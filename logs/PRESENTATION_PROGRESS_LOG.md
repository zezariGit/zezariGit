# Presentation Progress Log

Project: REAL_QR_FIND

This file is the cumulative presentation-ready project log. It is written so the user can use it to prepare status reports or presentation material.

## 2026-09-12 - 디자인 선선택 오류 수정
- 상품을 먼저 선택하지 않아도 디자인 선택 화면을 열 수 있도록 수정
- 디자인을 먼저 선택한 다음 상품을 고르면 해당 상품의 동일 디자인 데이터로 자동 연결
- 실제 브라우저에서 디자인 선선택, 상품 후선택, 금액 계산, 수량 및 다음 버튼 활성화까지 확인
- 회귀검사와 40개 라우트 운영 빌드 통과, 수정 커밋 `36c8342`

## 2026-09-12 - 상품 선택 상세화 및 서비스소개 관리
- 대상자 선택 시 구매 화면 뒤쪽을 흐리게 처리하고, 선택 대상자를 초록 체크·배경·굵은 이름으로 구분하는 즉시 선택창 적용
- 상품은 이미지·상품명·실제 가격을 표시하는 2열 카드, 디자인은 12간지 2열 카드 화면으로 구성
- 상품과 디자인 선택 전에는 `선택 완료`를 비활성화하고 선택 후 초록 테두리·체크 표시 및 구매 화면 반영
- 모든 상품구매 도움말 버튼을 `제품보기` 서비스 소개 화면으로 연결하고 뒤로가기와 맨 위 이동 버튼 제공
- 관리자에 `상품구매 서비스소개 관리` 메뉴를 신설해 세로형 상세 이미지를 업로드·교체·삭제하고 사용자 화면에 즉시 반영하도록 연동
- 상품구매 회귀검사, 보안검사, 40개 라우트 운영 빌드 및 로컬 브라우저 검증 통과
- 기능 커밋 `fb623a0` 생성, GitHub 및 Vercel 운영 배포는 대기 중

## 2026-09-12 - 상품 구매 선택 화면 개편
- 제공된 모바일 화면을 기준으로 상단 뒤로가기·도움말, 대상자, 상품, 디자인, 수량, 금액 요약, 다음 버튼을 재배치
- 가장 최근 등록 대상자를 기본 표시하고 이름은 굵게, 생년월일은 작은 글씨로 구분
- 상품과 디자인을 각각 선택할 수 있으며 두 항목 선택 전에는 수량과 다음 버튼을 회색 비활성화
- 기본 수량 1개 및 최소 수량 1개를 적용하고 선택 완료 후 수량 변경에 따라 결제예정금액 자동 갱신
- 기존 배송지·쿠폰·Toss 결제 단계로 정상 연결되는 것까지 로컬 미리보기 검증 완료
- 기능 커밋 `5fd03e9` 생성, GitHub 및 Vercel 운영 배포는 대기 중

## 2026-06-12 - Project Start and Documentation System Setup

### Request
- Start the real QR people-finding project.
- Codex should act as a full-stack developer.
- Keep two cumulative logs:
  - A technical handoff log for developers and AI agents.
  - A presentation log for the user.
- Produce official implementation deliverables by development phase.
- Accumulate image-generation prompts for diagrams or visuals when needed.

### Reflected Content
- Created the project rules document.
- Created the technical handoff log.
- Created the presentation progress log.
- Created the official deliverables folder.
- Created the image prompt archive.

### Time Spent
- Initial setup: approximately 5 minutes.

### Output
- `00_PROJECT_RULES.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`
- `deliverables/README.md`
- `deliverables/image_prompts/IMAGE_PROMPTS.md`

### Presentation Note
- The project now has a repeatable documentation system.
- Future requirements, implementation results, elapsed time, and outputs will be accumulated in these files.

## 2026-06-12 - VSCode/GitHub Account Check

### Request
- Check whether VSCode is configured with the GitHub account `zezari`.

### Reflected Content
- Checked global Git user settings.
- Checked whether the current project folder is a Git repository.
- Checked GitHub CLI login status.
- Checked VSCode user settings.

### Result
- The current project folder is not yet a Git repository.
- Global Git is currently set to:
  - Name: `soonsuboy`
  - Email: `soonsuboy10@gmail.com`
- GitHub CLI is not installed or not available from the terminal.
- VSCode settings did not show a visible GitHub account configuration.

### Time Spent
- Account check and log update: approximately 5 minutes.

### Output
- Updated `logs/DEV_HANDOFF_LOG.md`.
- Updated `logs/PRESENTATION_PROGRESS_LOG.md`.

### Presentation Note
- The current command-line Git identity is not `zezari`.
- Additional VSCode sign-in confirmation may be needed from the VSCode Accounts UI.

## 2026-06-12 - VSCode Profile Screenshot Clarification

### Request
- User showed a VSCode profile screenshot and asked whether it means the profile is set to `zezari`.

### Reflected Content
- Reviewed the screenshot context.
- Clarified the difference between VSCode GitHub profile login and terminal Git commit identity.

### Result
- The VSCode profile menu appears to show `zezariGit (GitHub)`.
- This means VSCode/Codex extension login appears to be using the `zezariGit` GitHub profile.
- However, terminal Git commit identity is still set to:
  - Name: `soonsuboy`
  - Email: `soonsuboy10@gmail.com`

### Time Spent
- Screenshot review and log update: approximately 3 minutes.

### Presentation Note
- VSCode login and Git commit author settings are separate.
- The project should set local Git identity before the first commit if commits must be associated with `zezari`.

## 2026-06-12 - Git Author Email Updated

### Request
- User provided the connected email `general@zezari.com` and asked to enter it directly in PowerShell.

### Reflected Content
- Updated global Git author name and email.

### Result
- Git global name is now `zezariGit`.
- Git global email is now `general@zezari.com`.

### Time Spent
- Git configuration update and verification: approximately 2 minutes.

### Presentation Note
- Future Git commits will use the `zezariGit / general@zezari.com` identity by default unless a project-specific Git setting overrides it.

## 2026-06-12 - GitHub Repository Connection

### Request
- User provided the repository URL `https://github.com/zezariGit/zezariGit.git`.
- User asked to apply the Git initialization and GitHub push script.

### Reflected Content
- Initialized the project folder as a Git repository.
- Created the first Git commit.
- Connected the project to the GitHub repository.
- Resolved an account mismatch in Windows Git credentials.
- Pushed the project to GitHub.

### Result
- The local project is now connected to GitHub.
- Branch `main` is tracking `origin/main`.
- The first project commit is available on the remote repository.

### Time Spent
- Repository initialization, authentication fix, push, and verification: approximately 15 minutes.

### Output
- GitHub remote:
  - `https://github.com/zezariGit/zezariGit.git`
- Initial commit:
  - `90dc4c7`
  - `Initial project setup`

### Presentation Note
- A Windows credential entry was still using the old `soonsuboy` account, which initially blocked the push.
- After updating Git Credential Manager to `zezariGit`, the project was successfully pushed to GitHub.

## 2026-06-12 - Vercel and Turso Connection Setup

### Request
- User said Vercel and Turso signup is complete and asked to connect them.

### Reflected Content
- Installed and logged in to Vercel CLI.
- Linked the local project folder to Vercel.
- Connected the GitHub repository to the Vercel project.
- Checked Turso CLI availability and Windows setup requirements.
- Prepared environment variable structure for Turso.

### Result
- Vercel connection is complete.
- Vercel account: `zezarigit`
- Vercel project: `real-qr-find`
- GitHub repository connected to Vercel: `zezariGit/zezariGit`
- Turso connection is pending because official Windows setup requires WSL and WSL is not currently installed.

### Time Spent
- Vercel installation, login, project link, Turso check, and documentation: approximately 20 minutes.

### Output
- `.gitignore`
- `.env.example`
- `deliverables/INTEGRATION_SETUP.md`
- Updated `deliverables/README.md`
- Updated project logs

### Presentation Note
- Hosting integration is ready through Vercel.
- Database integration path is prepared, but Turso requires either WSL setup or database credentials from the Turso dashboard.

## 2026-06-12 - Turso Environment Variable Setup

### Request
- User saved the Turso database URL and key in `env.txt`.
- User asked Codex to configure the environment variables.

### Reflected Content
- Read the Turso values from `env.txt`.
- Created local `.env.local`.
- Added the Turso variables to Vercel.
- Protected `env.txt` from Git commits by adding it to `.gitignore`.

### Result
- Local development has Turso environment variables configured.
- Vercel Production has Turso environment variables configured.
- Vercel Development has Turso environment variables configured.
- Vercel Preview is pending because Vercel requires a non-production preview branch or dashboard configuration.

### Time Spent
- Environment parsing, local setup, Vercel setup, verification, and documentation: approximately 10 minutes.

### Output
- Updated `.gitignore`.
- Updated `deliverables/INTEGRATION_SETUP.md`.
- Updated project logs.

### Presentation Note
- The database is now ready for local and production/development Vercel use.
- Secret values were intentionally excluded from Git and logs.

## 2026-06-12 - First Test Page

### Request
- Confirm whether GitHub, Vercel, and DB setup are complete.
- Create a simple page that prints `hellow zezari`.

### Reflected Content
- Confirmed GitHub connection.
- Confirmed Vercel connection.
- Confirmed Turso local, Production, and Development environment variable setup.
- Created a simple static web page.

### Result
- Added `public/index.html`.
- The page displays `hellow zezari`.
- Vercel build completed successfully.

### Time Spent
- Status check, page creation, build verification, and documentation: approximately 8 minutes.

### Output
- `public/index.html`
- Updated project logs.

### Presentation Note
- This is the first visible deployed screen for the project.
- It confirms that the project can move from local source code to Vercel deployment.

## 2026-06-12 - Public URL Renamed

### Request
- Change the public URL wording from `real-qr-find` to `zezari`.
- Confirm whether `zezari.com` can be used later.

### Reflected Content
- Renamed the Vercel project to `zezari`.
- Added the public alias `https://zezari.vercel.app`.
- Disabled Vercel deployment protection so the new URL opens publicly.
- Confirmed the page opens and displays `hellow zezari`.

### Result
- Current public URL:
  - `https://zezari.vercel.app`
- Previous URL still exists as an alias:
  - `https://real-qr-find.vercel.app`

### Time Spent
- Vercel project rename, alias setup, protection check, verification, and documentation: approximately 8 minutes.

### Future Domain Note
- `zezari.com` can be connected later if the domain is owned or controlled.
- It will require Vercel domain registration plus DNS setup at the domain provider.

## 2026-06-12 - Google Login Preparation

### Request
- Start the first feature: Google signup/login.
- Tell the user what information is needed from Google Cloud Console.
- User will write the needed values in `env.txt`.

### Reflected Content
- Defined the Google OAuth setup requirements.
- Created an authentication setup deliverable.
- Prepared the required redirect URLs for current Vercel URL and future custom domain.

### Required from Google Cloud
- Google OAuth Client ID.
- Google OAuth Client Secret.

### Required Console Settings
- App type: Web application.
- Production origin:
  - `https://zezari.vercel.app`
- Production redirect URI:
  - `https://zezari.vercel.app/api/auth/callback/google`
- Local origin:
  - `http://localhost:3000`
- Local redirect URI:
  - `http://localhost:3000/api/auth/callback/google`

### Time Spent
- OAuth setup review and documentation: approximately 8 minutes.

### Output
- `deliverables/AUTH_SETUP.md`
- Updated `deliverables/README.md`
- Updated project logs.

## 2026-06-12 - Google Login Implemented

### Request
- User completed Google Cloud Console setup and added the values to `env.txt`.
- User asked to create Google signup/login.

### Reflected Content
- Built the project as a Next.js app.
- Added Google login through NextAuth.
- Added a login page with:
  - Google login button
  - Logged-in user name/email display
  - Logout button
- Registered Google and NextAuth environment variables locally and in Vercel.
- Updated Vercel project settings from static/other to Next.js.

### Result
- Google login/signup implementation is now in the app.
- The callback route is:
  - `https://zezari.vercel.app/api/auth/callback/google`
- Local callback route is:
  - `http://localhost:3000/api/auth/callback/google`

### Time Spent
- Implementation, environment setup, build verification, Vercel project setting fix, and documentation: approximately 30 minutes.

### Output
- Next.js app files under `app/`.
- Auth config under `lib/auth.js`.
- Updated `.env.example`.
- Updated `deliverables/AUTH_SETUP.md`.
- Updated project logs.

### Presentation Note
- The first real user-facing feature is now implemented: Google-based signup/login.
- A full OAuth click-through should be tested by the project owner in the browser.

### Completion Update
- GitHub commit:
  - `851ffa1 Implement Google authentication`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Page opens successfully.
  - Google login button appears.
  - Google authentication provider route is active.

## 2026-06-13 - App Install Support and Login UI Update

### Request
- Remove `hellow`.
- Add the Google logo to the Google login button.
- Make the website installable like an app on Chrome desktop and mobile.

### Reflected Content
- Updated the login screen branding to `zezari`.
- Added Google logo to the login button.
- Added web app manifest.
- Added service worker.
- Added app icons for installation.
- Added install prompt support for desktop Chrome and Android.
- Added iPhone home-screen guidance.

### Result
- The site is now prepared as an installable PWA.
- Users can install it from supported browsers and open it from the desktop or phone home screen.

### Time Spent
- UI update, PWA setup, icon generation, build verification, and documentation: approximately 25 minutes.

### Output
- `public/manifest.webmanifest`
- `public/sw.js`
- `public/icons/icon-192.png`
- `public/icons/icon-512.png`
- `public/icons/maskable-512.png`
- `deliverables/PWA_SETUP.md`
- Updated project logs.

### Completion Update
- GitHub commit:
  - `b5ef241 Add PWA install support`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - `hellow` removed.
  - Google login text remains visible.
  - Manifest is available.
  - Service worker is available.
  - App icons are available.
  - Google login provider remains active.

## 2026-06-13 - Three-Page Service Introduction

### Request
- Add a three-page service introduction before the login screen.
- Make the pages swipe horizontally.
- Add `다시보지 않기` so the user can go directly to login later.

### Reflected Content
- Added an onboarding screen before login.
- Added three service pages:
  - QR 안심 서비스
  - 실종 발생 시 빠른 대응
  - 온라인 실종 공고
- Added swipe support on mobile.
- Added side-by-side phone panels on desktop.
- Added `다시보지 않기` using browser local storage.

### Result
- First-time logged-out users see the service introduction before login.
- Users who click `다시보지 않기` skip directly to the login screen on future visits from the same browser/device.

### Time Spent
- Onboarding UI, responsive layout, skip behavior, build verification, and documentation: approximately 25 minutes.

### Output
- `app/onboarding-gate.js`
- Updated `app/page.js`
- Updated `app/globals.css`
- Updated `public/sw.js`
- `deliverables/ONBOARDING_FLOW.md`

### Completion Update
- GitHub commits:
  - `6cc0bc2 Add onboarding before login`
  - `3f247d2 Render onboarding immediately`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Service introduction appears.
  - Three slide texts appear.
  - `다시보지 않기` appears.
  - `로그인 시작` appears.
  - Google login provider remains active.

## 2026-06-13 - Onboarding Flow Corrected

### Request
- User clarified that the onboarding should not show the capture-like three-phone layout.
- The service introduction should be three separate pages with images.
- Users should move from page 1 to page 2 to page 3, then reach the login page.
- `다시보지 않기` should immediately open the login page.

### Reflected Content
- Changed the onboarding to show one page at a time on both desktop and mobile.
- Removed the side-by-side desktop layout.
- Improved the visual design with one large illustration per page.
- Kept swipe and navigation controls.
- Kept `다시보지 않기` skip behavior.

### Result
- The onboarding now behaves as a true three-page intro before login.

### Time Spent
- Flow correction, CSS redesign, cache version update, and documentation: approximately 18 minutes.

### Completion Update
- GitHub commit:
  - `dfc60fb Correct onboarding to sequential pages`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Service introduction appears.
  - `다시보지 않기` appears.
  - Three slide titles appear.
  - Old side-by-side desktop layout was removed.
  - Google login remains active.

## 2026-06-13 - Guardian and Subject Management

### Request
- Treat logged-in users as guardians.
- Add guardian information entry/edit screen.
- Allow each guardian to register up to 4 target people.
- Allow photo upload per target.
- Save all information in the database.
- Let each guardian view and edit only their own information.

### Reflected Content
- Added Turso database tables for guardians and subjects.
- Added guardian profile form.
- Added target/person forms.
- Added photo upload support.
- Added edit support for saved information.
- Added server-side owner filtering by logged-in Google account.
- Added server-side maximum of 4 target people per guardian.
- Added password hashing so passwords are not stored as plaintext.

### Result
- After Google login, the user now sees a guardian dashboard.
- The guardian can save/update their own information.
- The guardian can add/update/delete their own target people.
- Subject information is stored in Turso.

### Time Spent
- DB schema, dashboard UI, server actions, photo upload, password hashing, build verification, and documentation: approximately 45 minutes.

### Output
- `app/dashboard.js`
- `app/actions.js`
- `lib/db.js`
- `lib/security.js`
- `deliverables/DATABASE_SCHEMA.md`

### Completion Update
- GitHub commit:
  - `38501f0 Add guardian and subject management`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Build succeeded.
  - Database schema was created in Turso.
  - Public page opens.
  - Google login remains active.
  - Logged-in dashboard should be checked after Google login.

## 2026-06-13 - Dashboard Status View

### Request
- After profile input is complete, logged-in guardians should go to a dashboard.
- The dashboard should show the guardian's four managed people.
- Each managed person should show their current status.
- User provided a reference dashboard capture.

### Reflected Content
- Added current status to managed people.
- Added dashboard view before the edit forms for completed guardian profiles.
- Added four management slots.
- Added status badges:
  - `문제없음`
  - `찾는중`
  - `QR활성화필요`
- Added quick action buttons similar to the reference.

### Result
- Completed guardians now land on a status dashboard after login.
- They can still edit guardian/subject information below the dashboard.

### Time Spent
- Dashboard flow update, DB status field, styling, and documentation: approximately 30 minutes.

### Completion Update
- GitHub commit:
  - `5d38213 Refine guardian status dashboard`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Build succeeded.
  - Subject status column exists in Turso.
  - Public page opens.
  - Google login remains active.
  - Latest PWA cache version is active.

## 2026-06-13 - Dashboard and Info Entry Split

### Request
- Separate the information entry screen from the dashboard.
- Style the information entry screen like the dashboard.
- Add a top-level menu with dashboard first, information entry second.

### Reflected Content
- Added top menu:
  - `대시보드`
  - `정보입력`
- Default logged-in screen is now the dashboard.
- Information forms are now only on the `정보입력` screen.
- If profile information is incomplete, the dashboard shows a prompt to move to information entry.

### Result
- Logged-in users now have two distinct screens:
  - Dashboard for status overview.
  - Information entry for profile and target-person edits.

### Time Spent
- Tab routing, screen separation, UI styling, cache update, and documentation: approximately 20 minutes.

### Completion Update
- GitHub commit:
  - `1e7fa08 Separate dashboard and info entry screens`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Build succeeded.
  - Public page opens.
  - Google login remains active.
  - Latest PWA cache version is active.

## 2026-06-13 - Admin Guardian Management

### Request
- Add an admin page.
- Allow admin to activate/deactivate guardians.
- Show guardians in a left-side grid.
- Show selected guardian's four managed people in a right-side grid.

### Reflected Content
- Added `/admin`.
- Added admin-only access based on Google login email.
- Added guardian active/inactive state.
- Added left guardian grid.
- Added right managed-person grid.
- Added activate/deactivate button.
- Inactive guardians now see a disabled-account notice.

### Result
- Admins can manage guardian activation and inspect each guardian's registered managed people.

### Time Spent
- Admin page, DB activation field, access control, UI styling, migration check, and documentation: approximately 35 minutes.

### Output
- `app/admin/page.js`
- `app/admin/actions.js`
- `lib/admin.js`
- `deliverables/ADMIN_SETUP.md`

### Completion Update
- GitHub commit:
  - `e244ebe Add admin guardian management`
- Production URL:
  - `https://zezari.vercel.app/admin`
- Verified:
  - Admin route opens.
  - Logged-out users see admin login gate.
  - Google login remains active.
  - Latest PWA cache version is active.

## 2026-06-14 - Gov Style Applied

### Request
- Apply the style from `css/gov-style.css` to all current pages.
- Make future pages use the same style.

### Reflected Content
- Connected `gov-style.css` globally.
- Updated existing custom UI classes to follow gov-style tokens.
- Applied the light-blue public-sector style to:
  - Login
  - Onboarding
  - Dashboard
  - Information entry
  - Admin page
- Updated PWA theme color.
- Added a style guide deliverable for future pages.

### Result
- The app now has a shared design base for current and future pages.

### Time Spent
- Global CSS integration, style overrides, PWA theme update, build verification, and documentation: approximately 25 minutes.

### Output
- `css/gov-style.css`
- `deliverables/UI_STYLE_GUIDE.md`
- Updated `app/layout.js`
- Updated `app/globals.css`
- Updated PWA metadata

### Completion Update
- GitHub commit:
  - `6259c7d Apply gov style globally`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Build succeeded.
  - Public page opens.
  - Admin page opens.
  - Latest PWA cache version is active.
  - Manifest theme color uses the gov-style blue.

## 2026-06-14 - QR Code Management

### Request
- Create about 30 QR codes and QR-linked unique strings.
- Store them in the database.
- Let admins view and activate/deactivate QR codes.
- Let admins generate more non-duplicated QR codes by desired count.
- Use the unique string as the final URL segment for the people-finding page.

### Reflected Content
- Added a new `qr_codes` database table.
- Seeded 30 initial active QR records.
- Added an admin QR management tab:
  - QR image
  - QR label
  - Unique string
  - Public find URL
  - Active/inactive status
  - Additional QR generation form
- Added the public route:
  - `/find/{unique-string}`
- Added active, inactive, and unknown QR display states.
- Added official QR management deliverable and presentation image prompt.

### Result
- Admins can now manage QR code inventory from the admin page.
- QR codes now point to a stable public people-finding URL pattern.
- The project is ready for the next stage: assigning QR codes to managed people and adding report/contact flow.

### Time Spent
- QR DB design, unique generation, admin UI, public page, seeding, verification, and documentation: approximately 45 minutes.

### Output
- `lib/db.js`
- `app/admin/page.js`
- `app/admin/actions.js`
- `app/find/[key]/page.js`
- `deliverables/QR_MANAGEMENT.md`
- `deliverables/DATABASE_SCHEMA.md`

### Verification
- Local build succeeded.

### Completion Update
- GitHub commit:
  - `6986c8f Add soonsuboy admin access`
- Production URL:
  - `https://zezari.vercel.app/admin`
- Verified:
  - Admin page opens.
  - Google login provider remains available.

## 2026-06-14 - Admin Role Management

### Request
- Add `관리자 관리` to the admin page.
- Let an admin grant administrator role to registered guardian users.

### Reflected Content
- Added a new admin menu tab:
  - `관리자 관리`
- Added DB admin role field:
  - `guardians.is_admin`
- Admin access now works through either:
  - Environment admin email list.
  - DB admin role granted to a registered guardian.
- The admin management screen lists registered users and lets admins grant or revoke DB admin role.
- Base admins configured by environment are protected from role removal in the UI.

### Result
- 관리자 페이지에서 보호자 사용자에게 관리자 권한을 직접 부여할 수 있게 되었습니다.
- 앞으로 새 운영자가 가입하면 기존 관리자가 화면에서 관리자 역할을 줄 수 있습니다.

### Time Spent
- DB role field, admin authorization, admin UI, migration, verification, and documentation: approximately 35 minutes.

### Output
- `app/admin/page.js`
- `app/admin/actions.js`
- `lib/db.js`
- `lib/admin.js`
- `deliverables/ADMIN_SETUP.md`
- `deliverables/DATABASE_SCHEMA.md`

### Verification
- Local build succeeded.
- Turso migration succeeded:
  - Registered users: 4
  - DB admins: 1
- Local `/admin?section=admins` route returned HTTP 200 and showed the login gate while logged out.

### Completion Update
- GitHub commit:
  - `7707d72 Add admin role management`
- Production URL:
  - `https://zezari.vercel.app/admin?section=admins`
- Verified:
  - Production admin-management route opens.
  - Logged-out users see the admin login gate.
  - Google login provider remains available.

## 2026-06-15 - Kakao/Naver Social Login Foundation

### Request
- Add Kakao and Naver login/signup in addition to Google.
- Show matching logo buttons.
- Prepare the base so login works once each platform key is entered.

### Reflected Content
- Added Google/Kakao/Naver provider foundation.
- Added three social login buttons:
  - Google
  - Kakao
  - Naver
- Providers activate automatically when both client ID and client secret are configured.
- Kakao/Naver buttons remain visible but disabled until their keys are entered.
- Updated the login screen and admin login gate from Google-only to social-login wording.
- Added environment variable placeholders and callback URL documentation.

### Result
- The app is ready for Kakao and Naver OAuth credentials.
- Once keys are entered and deployed, users can sign up/login through Kakao or Naver.

### Time Spent
- Provider setup, button UI, environment documentation, build verification, and logs: approximately 30 minutes.

### Output
- `lib/auth.js`
- `app/auth-actions.js`
- `app/page.js`
- `app/admin/page.js`
- `deliverables/AUTH_SETUP.md`

### Verification
- Local build succeeded.
- Current provider API shows Google only because Kakao/Naver credentials are not configured yet.
- Login gate displays Google, Kakao, and Naver buttons.

### Completion Update
- GitHub commit:
  - `f834a3e Add Kakao and Naver login foundation`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Production auth provider API opens.
  - Google provider is active.
  - Kakao/Naver buttons are visible but disabled until credentials are added.
  - PWA cache version updated to `zezari-v10`.

## 2026-06-15 - Toss Payments Subscription Button

### Request
- Start Toss Payments integration for subscription payment and future advertisement payment.
- First, add a subscription payment button next to `현재 상태` on the dashboard.
- After payment completes, show `구독중`.

### Reflected Content
- Added Toss Payments subscription payment foundation.
- Added `구독결제하기` button next to the dashboard `현재 상태` title.
- Added `구독중` status badge after successful subscription payment.
- Added server-side Toss API flow:
  - Billing key issue.
  - First subscription billing approval.
  - DB status update to active.
- Added success/fail redirect pages.
- Added subscription database table.
- Added Toss Payments setup deliverable.

### Result
- The dashboard now has the subscription payment entry point.
- The backend is ready to store subscription state and mark a guardian as subscribed after server-side Toss payment success.
- Toss keys still need to be visible in environment variables before the real test payment window can be opened.

### Time Spent
- Toss docs review, DB schema, button UI, API routes, callback pages, migration, verification, and documentation: approximately 50 minutes.

### Output
- `app/toss-subscription-button.js`
- `app/api/payments/toss/subscription/prepare/route.js`
- `app/payments/toss/subscription/success/page.js`
- `app/payments/toss/subscription/fail/page.js`
- `lib/toss-payments.js`
- `deliverables/TOSS_PAYMENTS_SETUP.md`

### Verification
- Local build succeeded.
- Turso `subscriptions` table was created.
- Prepare API requires login.
- Fail redirect page opens.
- Toss payment window was not tested because Toss keys were not detected in the saved environment.

### Completion Update
- GitHub commit:
  - `a550f63 Add Toss subscription payment foundation`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Subscription prepare API is deployed and requires login.
  - Toss fail page opens.
  - PWA cache version updated to `zezari-v11`.

## 2026-06-15 - Toss Test Keys Reflected

### Request
- In `env.txt`, under `// tosspayments`, use the first value as the Toss client key and the second value as the Toss secret key.
- Reflect those test keys so they can be changed later.

### Reflected Content
- Applied the first Toss value as `TOSS_CLIENT_KEY`.
- Applied the second Toss value as `TOSS_SECRET_KEY`.
- Updated local `.env.local`.
- Added both keys to Vercel Production and Development as encrypted environment variables.
- Redeployed production.

### Result
- The deployed app now has Toss Payments test keys configured.
- The subscription payment button can proceed to the Toss flow for logged-in users.
- Future key changes only require replacing environment variable values.

### Time Spent
- Environment reflection, Vercel setup, redeployment, and verification: approximately 15 minutes.

### Verification
- Local build succeeded.
- Production prepare API remains protected and returns login-required while logged out.

## 2026-06-15 - Subscription Options And Pause Resume

### Request
- Users should be able to pause/resume after subscribing.
- Subscription service should support:
  - 1 month
  - 3 months
  - 6 months
- Admin should be able to configure prices for each option in the admin page.

### Reflected Content
- Added 1/3/6-month subscription options.
- Added administrator price management:
  - `/admin?section=payments`
  - `결제 관리`
- Added user dashboard controls:
  - Plan selector before payment.
  - `구독결제하기`.
  - `구독중`.
  - `일시정지`.
  - `일시정지중`.
  - `재개`.
- Added DB support for subscription plan prices and subscription periods.
- Updated Toss payment preparation so price is loaded from the server-side database.

### Result
- Subscription pricing can now be changed from the admin page.
- Users can control app-level subscription pause/resume from the dashboard.
- The app is ready to test payment for 1/3/6-month options.

### Time Spent
- Plan pricing schema, admin UI, dashboard controls, Toss prepare update, migration, verification, and docs: approximately 45 minutes.

### Output
- `subscription_plans` DB table.
- `app/api/subscription/status/route.js`
- Updated dashboard subscription component.
- Updated admin payment-management screen.

### Verification
- Local build succeeded.
- Turso migration succeeded:
  - 1 existing subscription.
  - 3 subscription plans seeded.
- Local unauthenticated APIs return login-required.
- Local admin payment route opens and shows login gate while logged out.

### Completion Update
- GitHub commit:
  - `7e595ed Add subscription plans and pause resume`
- Production URL:
  - `https://zezari.vercel.app`
- Verified:
  - Subscription status API is deployed and requires login.
  - Subscription prepare API is deployed and requires login.
  - Admin payment route opens and requires admin login.
  - PWA cache version updated to `zezari-v12`.
- Turso seed result:
  - 30 total QR records
  - 30 active QR records
- Local public QR URL returned HTTP 200 and displayed the active QR confirmation.
- Admin QR URL returned HTTP 200 and showed the login gate while logged out.

### Completion Update
- GitHub commit:
  - `b3a15d3 Add QR code management`
- Production URL:
  - `https://zezari.vercel.app`
- Verified in production:
  - Main page opens.
  - Admin QR tab URL opens and shows the admin login gate when logged out.
  - A sample active QR finding URL opens.
  - PWA cache version was updated to `zezari-v9`.

## 2026-06-14 - Additional Admin Email

### Request
- Give admin access to:
  - `soonsuboy10@gmail.com`

### Reflected Content
- Added `soonsuboy10@gmail.com` to the default admin email list.
- Added Vercel `ADMIN_EMAILS` for Production and Development with both admin emails.
- Updated environment example documentation.

### Result
- Users logged in with `soonsuboy10@gmail.com` can access the admin page after the new production deployment.

### Time Spent
- Admin setting update, environment configuration, build verification, and documentation: approximately 10 minutes.

### Verification
- Local build succeeded.
## 2026-06-15 - 저장/조회 피드백 전역 적용

### 요구내용
- 저장하거나 조회할 때 완료된 느낌이 없다.
- 하단에 `저장되었습니다`, `수정되었습니다`, `필수값을 입력해주세요` 같은 메시지가 필요하다.
- 조회할 때 프로그래스바가 필요하다.
- 전체 화면에 적용한다.

### 반영내용
- 공통 제출 버튼 컴포넌트 추가.
- 버튼 클릭 후 처리 중에는 버튼 내부에 진행바 표시.
- 저장/수정/삭제/매칭/상태변경 완료 후 하단 토스트 메시지 표시.
- 서버 처리 실패 시 에러형 하단 토스트 표시.
- 사용자 화면과 관리자 화면 모두 토스트 표시 연결.
- QR 필터와 QR 매칭대상 조회 같은 조회 폼에도 진행바 적용.

### 산출물
- `app/form-submit-button.js`
- `app/status-toast.js`
- `deliverables/QR_MANAGEMENT.md`

### 검증
- 로컬 빌드 성공.

### 반영 시간
- 공통 컴포넌트, 서버 액션 리다이렉트, 버튼 교체, 스타일/문서 반영: 약 30분.

## 2026-06-15 - QR 매칭 모달 UX 정리

### 요구내용
- QR 카드 안의 보호자 조회/관리대상 조회 텍스트와 정렬이 지저분하다.
- 미매칭 QR에는 `매칭대상 조회` 버튼만 보여준다.
- 버튼 클릭 시 팝업을 띄워 미매칭 관리대상을 검색/선택한다.
- 팝업이 떠 있는 동안 메인 화면은 선택되지 않게 비활성화한다.
- 여러 팝업이 동시에 열려 헷갈리지 않게 한다.
- 매칭된 QR에는 `매칭대상 조회`를 숨기고 `매칭 해제`만 보여준다.
- 텍스트가 삐져나오는 부분은 인식되는 대로 개선한다.

### 반영내용
- QR 카드 내부 검색 폼 제거.
- 미매칭 QR 카드에 `매칭대상 조회` 버튼 추가.
- `assignQr` 기준 단일 모달 팝업으로 매칭 대상 검색 흐름 변경.
- 모달 배경 오버레이로 메인 화면 비활성화 효과 적용.
- 모달 안에서 보호자/관리대상 검색 후 미매칭 대상만 선택 가능.
- 매칭된 QR은 `매칭 해제`만 노출.
- QR 카드 그리드 폭, 버튼 정렬, 텍스트 줄바꿈/넘침 방지 CSS 개선.

### 검증
- 로컬 빌드 성공.

### 반영 시간
- UX 변경, 스타일 보정, 문서/로그 반영: 약 25분.

## 2026-06-15 - QR 매칭 검색 및 보호자 QR 확인 개선

### 요구내용
- QR 관리에서 관리대상 매칭 시 selectbox 대신 조회 버튼 방식으로 개선한다.
- 보호자 이름/관리대상 이름으로 조회해서 선택할 수 있게 한다.
- 이미 매칭된 관리대상은 조회되지 않아야 한다.
- 매칭 해제해야만 다시 조회될 수 있어야 한다.
- QR 관리에서 QR 이미지를 누르면 다운로드되게 한다.
- 매칭된 QR 코드를 보호자 사용자 화면에서도 확인할 수 있게 한다.

### 반영내용
- QR 카드별 `대상 조회` 검색 폼 추가.
- 검색 조건:
  - 보호자 이름/이메일
  - 관리대상 이름
- 검색 결과는 미매칭 관리대상만 표시.
- 검색 결과마다 `선택 매칭` 버튼 제공.
- QR 이미지를 클릭하면 `{QR코드}.png`로 다운로드되게 개선.
- 보호자 정보입력 화면의 관리대상 카드에 배정 QR 이미지, QR 코드, 활성 상태, 찾기 URL 표시.
- 보호자 대시보드 목록에도 QR 코드명을 표시.

### 산출물
- `deliverables/QR_MANAGEMENT.md`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/image_prompts/IMAGE_PROMPTS.md`

### 검증
- 로컬 빌드 성공.
- Turso 미매칭 대상 조회 성공.
- 현재 관리대상 2명은 모두 QR 매칭 상태라 미매칭 조회 결과는 0건.
- 보호자 관리대상 QR 조인 조회 성공:
  - QR 코드/공개키가 포함된 관리대상 2건 확인.

### 반영 시간
- 구현, 빌드, DB 검증, 문서/로그 반영: 약 35분.

## 2026-06-15 - QR 필터 및 수동 매칭 기능 추가

### 요구내용
- QR 관리에서 관리대상과 매칭된 QR을 필터링해서 볼 수 있게 한다.
- 활성/비활성 QR도 필터링할 수 있게 한다.
- 관리자가 수동으로 QR 매칭, 변경, 해제를 할 수 있게 한다.

### 반영내용
- QR 관리 상단에 필터 추가:
  - 매칭상태: 전체 / 관리대상 매칭됨 / 관리대상 미매칭
  - 활성상태: 전체 / 활성 / 비활성
- QR 통계 표시 확장:
  - 활성
  - 비활성
  - 매칭
  - 미매칭
- 각 QR 카드에 관리대상 선택 박스 추가.
- 각 QR 카드에 `매칭 저장` 또는 `매칭 변경` 버튼 추가.
- 이미 매칭된 QR에는 `매칭 해제` 버튼 추가.
- 관리대상을 다른 QR에 매칭하면 이전 QR 배정은 자동 해제되도록 처리.

### 산출물
- `deliverables/QR_MANAGEMENT.md`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/image_prompts/IMAGE_PROMPTS.md`

### 검증
- 로컬 빌드 성공.
- Turso 기준 QR 통계 조회 성공:
  - 전체 30개
  - 활성 30개
  - 비활성 0개
  - 매칭 2개
  - 미매칭 28개
  - 관리대상 선택 옵션 2개
- GitHub 반영:
  - `23381c1 Add QR admin filters and manual matching`
- 운영 배포 완료:
  - `https://zezari.vercel.app`
- 운영 경로 확인:
  - `/admin?section=qr&match=matched&active=active` 정상 응답.
  - 서버 오류 문구 없음.

### 반영 시간
- 구현, DB 조회 검증, 문서/로그 반영: 약 25분.

## 2026-06-15 - QR 관리 페이지 서버 오류 수정

### 요구내용
- QR 관리 페이지로 들어가면 `This page couldn't load` 서버 오류가 발생한다.

### 원인
- QR 관리 목록 조회 SQL에서 `created_at` 컬럼명을 테이블명 없이 사용했다.
- QR, 보호자, 관리대상 테이블이 모두 `created_at`을 가지고 있어 DB가 어떤 컬럼인지 판단하지 못했다.

### 반영내용
- QR 관리 목록 정렬 기준을 `q.created_at`, `q.code`로 명확히 지정했다.

### 검증
- 오류 SQL 재현 확인.
- 수정 SQL 실행 성공:
  - QR 30건 조회 성공.
- 로컬 빌드 성공.
- GitHub 반영:
  - `826b239 Fix QR admin query ordering`
- 운영 배포 완료:
  - `https://zezari.vercel.app`
- 운영 경로 확인:
  - `/` 정상 응답.
  - `/admin?section=qr` 비로그인 상태에서 관리자 로그인 화면 정상 응답.

### 반영 시간
- 원인 확인, 수정, 검증, 로그 기록: 약 10분.

## 2026-06-15 - QR/푸시 기능 운영 배포 완료

### 배포 결과
- GitHub 커밋:
  - `d263b87 Add subject QR matching and guardian push alerts`
- 운영 배포 URL:
  - `https://zezari.vercel.app`

### 운영 확인
- 공개 QR 페이지 접속 성공.
- `보호자에게 알리기` 버튼 표시 확인.
- 보호자 정보 영역 표시 확인.
- 푸시 공개키 API 설정 완료 확인.
- 알림 API 정상 응답 확인.
- 서비스워커 `zezari-v13` 및 푸시 핸들러 반영 확인.

### 참고
- 현재 푸시 전송 수는 0건이다.
- 이유: 보호자가 실제 기기/브라우저에서 `푸시 알림 켜기`를 아직 누르지 않았기 때문이다.

## 2026-06-15 - QR별 관리대상 매칭 및 보호자 푸시 알림

### 요구내용
- QR을 찍으면 연결되는 페이지에 보호자가 등록한 관리대상 정보가 나오게 한다.
- 관리대상별로 QR 코드를 매칭한다.
- 보호자 1명당 관리대상 최대 4명이므로 보호자당 최대 4개 QR이 배정될 수 있다.
- QR 접근 페이지에서 관리대상 정보와 보호자 연락처, 주소 등 기본정보를 보여준다.
- `보호자에게 알리기` 버튼을 눌러 로그인되어 있는 보호자에게 `{관리대상 이름}을 찾았습니다` 푸시 메시지를 보낸다.

### 반영내용
- 관리대상 저장 시 자동으로 미배정 QR 1개를 배정하도록 구현.
- 기존 관리대상 1명에게 기존 QR 1개를 마이그레이션으로 배정.
- 공개 QR 페이지를 실제 대상자/보호자 정보 표시 화면으로 확장.
- 공개 QR 페이지에 `보호자에게 알리기` 버튼 추가.
- 보호자 대시보드에 `푸시 알림 켜기` 버튼 추가.
- 브라우저 Web Push 구독 정보를 DB에 저장하는 API 추가.
- QR 알림 API가 보호자의 등록된 기기로 푸시 알림을 보내도록 구현.
- 관리자 QR 화면에서 각 QR의 배정 보호자/관리대상을 확인할 수 있게 개선.
- Vercel 운영/개발 환경에 VAPID 푸시 키 등록 완료.

### 산출물
- `deliverables/PUSH_NOTIFICATION_SETUP.md`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/QR_MANAGEMENT.md`
- `deliverables/image_prompts/IMAGE_PROMPTS.md`

### 검증
- 로컬 빌드 성공.
- 공개 QR 찾기 페이지 HTTP 200 확인.
- `보호자에게 알리기` 버튼 표시 확인.
- 푸시 공개키 API 설정 완료 확인.
- 알림 API는 정상 응답했으며, 현재 등록된 보호자 푸시 기기가 없어 전송 수는 0건.

### 반영 시간
- 설계, 구현, DB 마이그레이션, 환경변수 등록, 검증, 문서화: 약 45분.

## 2026-06-15 - 관리대상별 광고 기능 기반 구현

### 요구내용
- 대시보드의 관리대상별로 `광고` 버튼을 추가한다.
- 버튼 클릭 시 팝업을 띄우고, 팝업이 열려있는 동안 메인 화면은 비활성화한다.
- 팝업에서 광고기간과 광고지역을 사용자가 설정한다.
- 지정한 기간과 관리자 설정 일 단가에 따라 광고 금액을 산정한다.
- 광고중 상태에서는 `일시정지`, `광고끝내기` 기능을 제공한다.
- 관리자페이지에 광고탭을 추가한다.
- 관리자 광고탭에서 일자별 광고단가를 설정한다.
- 관리자 광고탭에서 사용자별 광고 진행사항을 그리드로 조회한다.
- Meta API는 추후 정보 수령 후 연동한다.

### 반영내용
- `ad_settings` 테이블 추가:
  - 광고 일 단가 관리.
  - 기본값 10,000원.
- `subject_ads` 테이블 추가:
  - 관리대상별 광고지역, 광고기간, 일수, 일 단가, 총액, 상태 저장.
  - Meta API 연동용 `meta_campaign_id`, `meta_status` 예약 필드 추가.
- 사용자 대시보드 개선:
  - 각 관리대상 카드에 `광고` 버튼 추가.
  - 광고 신청/관리 모달 추가.
  - 모달에서 기간 변경 시 예상 금액 즉시 계산.
  - 광고중/일시정지 상태에 따라 일시정지, 재개, 종료 버튼 표시.
- 관리자페이지 개선:
  - `광고 관리` 탭 추가.
  - 광고 일 단가 저장 폼 추가.
  - 사용자별 광고 진행사항 그리드 추가.
- 기존 저장/조회 피드백 패턴 유지:
  - 버튼 진행 상태.
  - 하단 완료/오류 메시지.

### 산출물
- `deliverables/ADVERTISING_SETUP.md`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/README.md`
- `deliverables/image_prompts/IMAGE_PROMPTS.md`

### 검증
- 로컬 빌드 성공.
- Turso 광고 테이블 생성 확인.
- 기본 광고 일 단가 10,000원 확인.
- 현재 광고 진행 데이터 0건 확인.
- 운영 배포 완료:
  - `https://zezari.vercel.app`
- 운영 경로 확인:
  - `/` 정상 응답.
  - `/admin?section=ads` 정상 응답.

### 반영 시간
- 설계, 구현, DB 확인, 문서/로그 반영: 약 35분.

## 2026-06-16 - 팝업 공통 닫기 버튼 및 배경 스크롤 잠금 개선

### 요구내용
- 모든 팝업의 닫기 버튼을 아래쪽에 구현한다.
- 광고 팝업이 떠 있을 때 휴대폰 터치로 뒤쪽 메인화면이 스크롤되지 않게 한다.
- 팝업이 떠 있을 때 뒤쪽 메인화면 버튼도 눌리지 않게 한다.
- 현재 팝업뿐 아니라 향후 만들어질 팝업에도 공통적으로 적용한다.

### 반영내용
- 공통 팝업 스크롤 잠금 컴포넌트 `ModalScrollLock` 추가.
- 팝업이 열리면 현재 스크롤 위치를 유지한 채 body를 고정하도록 구현.
- 팝업 표면 밖의 모바일 `touchmove` 이벤트를 차단하도록 구현.
- 공통 팝업 CSS 클래스 추가:
  - `.modal-backdrop`
  - `.modal-surface`
  - `.modal-footer`
  - `.modal-close-button`
- 광고 팝업 닫기 버튼을 하단으로 이동.
- QR 매칭 팝업 닫기 버튼을 하단으로 이동.
- 향후 팝업 구현 규칙을 `deliverables/UI_STYLE_GUIDE.md`에 기록.

### 산출물
- `deliverables/UI_STYLE_GUIDE.md`

### 검증
- 로컬 빌드 성공.
- `/` HTTP 200 확인.
- `/admin?section=qr&assignQr=test` HTTP 200 확인.
- 운영 배포 완료:
  - `https://zezari.vercel.app`
- 운영 경로 확인:
  - `/` 정상 응답.
  - `/admin?section=qr&assignQr=test` 정상 응답.

### 반영 시간
- 구현, 검증, 문서/로그 반영: 약 20분.

## 2026-06-16 - 모바일 대시보드 관리대상 카드 정렬 개선

### 요구내용
- 모바일 대시보드에서 관리대상 카드가 비대칭으로 보인다.
- 카드 내 요소를 대칭이 맞게 정렬한다.

### 반영내용
- 모바일 전용 카드 레이아웃을 조정했다.
- 카드 상단은 사진과 관리대상 정보를 2열로 배치했다.
- 카드 하단은 상태 배지와 광고 버튼을 같은 폭의 2열로 배치했다.
- 모바일 카드 padding, gap, 사진 크기를 조정해 카드별 정렬 기준을 통일했다.
- 빈 슬롯 카드의 상태 배지도 전체 폭 기준으로 정렬되도록 보정했다.

### 산출물
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- `/` HTTP 200 확인.
- 운영 배포 완료:
  - `https://zezari.vercel.app`
- 운영 경로 확인:
  - `/` 정상 응답.

### 반영 시간
- 구현, 검증, 로그 반영: 약 15분.

## 2026-06-16 - 사용자 통합 설명서 생성

### 요구내용
- 지금까지 관리된 요청사항과 반영내역 관리 파일, 전체 소스, 결과물을 기준으로 사용자 설명서를 만든다.
- 화면, 버튼 기능, 조회 데이터 설명을 포함한다.
- 보호자 사용자와 관리자 사용자가 모두 이해할 수 있는 통합 설명서로 만든다.
- Google Docs 문서로 만든다.

### 반영내용
- Google Docs 문서 `REAL_QR_FIND 사용자 통합 설명서`를 생성했다.
- 설명서에 다음 내용을 정리했다.
  - 서비스 개요와 사용 환경.
  - Google/Kakao/Naver 로그인 구조.
  - 앱 설치와 온보딩 흐름.
  - 보호자 대시보드와 정보입력 화면.
  - 관리대상 등록/수정/삭제, 사진 업로드, 상태 표시.
  - 구독 결제, 일시정지, 재개.
  - 광고 신청/관리 기반 기능.
  - QR 찾기 공개 페이지와 보호자 알림.
  - 관리자 보호자 관리, QR 관리, 관리자 관리, 결제 관리, 광고 관리.
  - 저장/조회 진행 표시와 하단 완료/오류 메시지.
  - 주요 데이터 항목 설명.
- 로컬 산출물 `deliverables/USER_MANUAL.md`를 추가해 Google Docs 링크와 유지관리 기준을 남겼다.
- 민감한 환경변수와 키 값은 문서에 포함하지 않았다.

### 산출물
- Google Docs:
  - https://docs.google.com/document/d/1DdcqFv79lcAj4eCuiXaOTsJmpTKWtvBRErWJsoAidEM
- `deliverables/USER_MANUAL.md`
- `deliverables/README.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- Google Docs 커넥터로 문서를 다시 읽어 제목, 본문, 주요 장 제목 스타일 적용을 확인했다.
- 현재 구현된 화면과 버튼 기준으로 설명서를 구성했다.
- 미구현 또는 설정 필요 항목은 준비/설정 필요 상태로 구분했다.

### 반영 시간
- 소스/로그 검토, 설명서 작성, Google Docs 생성, 검증, 산출물/로그 반영: 약 35분.

## 2026-06-16 - 사용자 설명서 실제 화면 스크린샷 추가

### 요구내용
- 사용자 설명서에 화면, 버튼, 주요 기능별 실제 화면 스크린샷을 넣는다.

### 반영내용
- 로컬 앱을 실행해 실제 화면을 캡처했다.
- Google Docs 설명서에 `21. 실제 화면 예시` 섹션을 추가했다.
- 다음 화면 이미지를 삽입했다.
  - 온보딩 첫 화면.
  - 소셜 로그인 화면.
  - 관리자 로그인 화면.
  - 미매칭 QR 공개 화면.
  - 매칭 완료 QR 공개 화면 개인정보 가림본.
- 매칭 완료 QR 화면은 보호자 연락처와 관리대상 정보가 노출될 수 있어 민감 영역을 가린 이미지로 삽입했다.
- 원본 및 삽입용 스크린샷 파일은 `deliverables/user_manual_screenshots/`에 저장했다.

### 산출물
- Google Docs:
  - https://docs.google.com/document/d/1DdcqFv79lcAj4eCuiXaOTsJmpTKWtvBRErWJsoAidEM
- `deliverables/USER_MANUAL.md`
- `deliverables/user_manual_screenshots/`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- Google Docs 커넥터로 문서를 다시 읽어 `21. 실제 화면 예시` 섹션과 이미지 5개가 삽입된 것을 확인했다.
- 캡션 5개가 문서에 표시되는 것을 확인했다.

### 남은 사항
- 보호자 대시보드와 관리자 내부 탭 화면은 실제 로그인 세션이 필요하므로, 로그인된 브라우저 상태에서 추가 캡처가 필요하다.

### 반영 시간
- 캡처, 개인정보 가림, Google Docs 이미지 삽입, 검증, 로그 반영: 약 35분.

## 2026-06-16 - QR 공개 페이지 안심번호 적용

### 요구내용
- QR 페이지에서 보호자의 실제 전화번호가 그대로 노출되지 않게 한다.
- 보호자 전화번호를 안심번호 방식으로 보여주고 싶다.

### 반영내용
- 보호자 테이블에 `safe_phone` 안심번호 컬럼을 추가했다.
- 기존 DB에도 `safe_phone` 컬럼이 자동 추가되도록 스키마 보정 로직을 반영했다.
- 보호자 정보입력 화면에 선택 입력값 `안심번호`를 추가했다.
- QR 공개 페이지에서 실제 전화번호 `phone`을 더 이상 조회하지 않도록 수정했다.
- QR 공개 페이지의 `연락처` 표시를 `안심번호`로 변경했다.
- 안심번호가 아직 없으면 `안심번호 준비중`으로 표시한다.
- Google Docs 사용자 설명서에도 원 전화번호 비공개 및 안심번호 표시 기준을 반영했다.

### 산출물
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/QR_MANAGEMENT.md`
- `deliverables/USER_MANUAL.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- Turso `guardians.safe_phone` 컬럼 존재 확인.
- 로컬 QR 공개 페이지 정상 응답 확인.
- QR 공개 페이지에서 `안심번호`와 `안심번호 준비중` 표시 확인.
- QR 공개 페이지에서 기존 `연락처` 라벨이 표시되지 않는 것 확인.

### 남은 사항
- 실제 전화 연결이 되는 안심번호는 통신사/ARS/안심번호 사업자 연동이 필요하다.
- 현재 반영은 실제 번호를 공개하지 않는 앱 기반 개인정보 보호 구조다.

### 반영 시간
- 스키마, 화면, QR 공개 페이지, 검증, 문서/로그 반영: 약 30분.

## 2026-06-19 - 로그인/가입 화면 리디자인

### 요구내용
- 3페이지 서비스 소개 이후 첨부 예시와 같은 로그인/가입 화면을 보여준다.
- 아이디, 비밀번호, 자동로그인, 비밀번호 찾기, 로그인 버튼, SNS 아이콘, 회원가입 문구를 구성한다.

### 반영내용
- 메인 로그인 화면을 예시 이미지와 유사한 모바일 중심 레이아웃으로 변경했다.
- 보호자가 저장한 아이디/비밀번호로 로그인할 수 있도록 Credentials 로그인 구조를 추가했다.
- Kakao, Naver, Google은 기존 SNS 로그인 흐름을 아이콘 버튼으로 표시한다.
- Apple 아이콘은 UI 자리만 준비했으며 실제 OAuth 연동은 추후 작업 항목이다.
- 자동로그인은 비밀번호를 저장하지 않고 아이디만 기억하는 방식으로 구현했다.
- 필수값 누락, 로그인 실패, 비밀번호 찾기 준비중, 회원가입 안내 메시지를 화면에서 보여준다.

### 산출물
- `app/page.js`
- `app/auth-actions.js`
- `app/globals.css`
- `lib/auth.js`
- `lib/db.js`
- `deliverables/AUTH_SETUP.md`
- `deliverables/USER_MANUAL.md`
- `deliverables/user_manual_screenshots/login_redesign.png`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- 로컬 메인 페이지 정상 응답 확인.
- 모바일 화면 기준 로그인 화면 스크린샷 캡처 완료.

### 반영 시간
- 구현, 빌드 검증, 화면 캡처, 문서/로그 반영: 약 35분.

## 2026-06-19 - 로그인 화면 운영 배포 반영

### 요구내용
- GitHub에 푸시했는지 확인한다.
- 운영 페이지에 새 로그인 화면이 반영되지 않은 원인을 확인한다.

### 반영내용
- 로컬 변경사항이 아직 GitHub/Vercel에 올라가지 않은 상태임을 확인했다.
- 로그인 리디자인 변경사항을 커밋하고 GitHub `main`에 푸시했다.
- Vercel production 배포를 직접 실행했다.
- `zezari.vercel.app` 별칭이 이전 배포를 보고 있어 새 배포로 다시 연결했다.

### 산출물
- GitHub commit:
  - `8c1b307 Redesign login screen`
- Production deployment:
  - `https://zezari-4l8f3rrua-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 검증
- Vercel 운영 빌드 성공.
- `https://zezari.vercel.app`에서 새 로그인 화면 요소 `.login-card` 확인.
- 운영 화면 캡처 성공.

### 반영 시간
- GitHub 푸시, 운영 배포, 별칭 수정, 검증: 약 15분.

## 2026-06-19 - 최초 방문자 회원가입 플로우 추가

### 요구내용
- 회원가입하지 않은 최초 방문자가 첨부 예시와 같은 과정을 거쳐 가입 정보를 입력하게 한다.
- 단계는 휴대폰 인증, 기본정보 입력, 회원가입 완료로 구성한다.
- 구현 후 GitHub에도 자동 푸시한다.

### 반영내용
- 로그인 화면의 `회원가입` 버튼을 누르면 회원가입 화면으로 전환되게 했다.
- `/?signup=1` 주소로도 회원가입 화면을 직접 열 수 있게 했다.
- 휴대폰 인증 화면을 추가했다.
  - 휴대폰 번호 입력.
  - 인증코드 받기.
  - 5자리 인증번호 입력.
  - 남은 시간 표시.
- 기본정보 입력 화면을 추가했다.
  - 이름.
  - 생년월일.
  - 인증된 휴대폰 번호.
  - 아이디.
  - 비밀번호.
  - 개인정보 수집 및 이용 동의.
  - 서비스 이용 약관 동의.
- 회원가입 완료 화면을 추가했다.
  - 대상자 등록하기.
  - 대시보드 바로가기.
- 로그인 전 보호자 계정을 생성하는 API를 추가했다.
- 보호자 DB에 생년월일, 휴대폰 인증 완료 시각, 필수 약관 동의 시각을 저장하도록 확장했다.
- 새 회원가입 화면은 이메일을 받지 않으므로, 대시보드 사용 완료 기준에서 이메일 필수 조건을 제외했다.

### 산출물
- `app/page.js`
- `app/auth-actions.js`
- `app/api/signup/guardian/route.js`
- `app/dashboard.js`
- `app/globals.css`
- `lib/db.js`
- `deliverables/AUTH_SETUP.md`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/USER_MANUAL.md`
- `deliverables/user_manual_screenshots/signup_phone_step.png`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- 로컬 회원가입 화면 정상 응답 확인.
- 회원가입 휴대폰 인증 화면 스크린샷 캡처 완료.
- 회원가입 API 필수값 검증 메시지 확인.

### 남은 사항
- 현재 휴대폰 인증은 테스트 모드이며, 실제 문자 발송은 SMS 사업자 연동이 필요하다.

### 반영 시간
- 화면/API/DB/문서/검증 반영: 약 55분.

### 운영 반영
- GitHub commit:
  - `f6dad88 Add direct guardian signup flow`
- Vercel production deployment:
  - `https://zezari-aoit7tb7g-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- `https://zezari.vercel.app/?signup=1`에서 회원가입 화면 확인.
- 회원가입 API 필수값 검증 응답 확인.

## 2026-06-19 - SNS 최초 로그인 회원가입 정보 입력 적용

### 요구내용
- 현재는 `회원가입` 버튼을 눌렀을 때만 가입정보를 입력받는다.
- Kakao, Naver, Google 등 SNS 간편로그인을 눌렀을 때도 최초 가입자라면 가입정보를 입력받게 한다.
- SNS에서 넘어온 이름/이메일 등이 있으면 미리 채운다.
- 이미 가입된 사용자는 대시보드로 바로 이동한다.

### 반영내용
- SNS 로그인 후 보호자 정보가 미완성인 경우 대시보드 대신 회원가입 정보 입력 화면을 먼저 보여주게 했다.
- SNS에서 생성된 보호자 이름과 이메일을 기본정보 입력 화면에 미리 채운다.
- 가입정보가 이미 완성된 보호자는 기존처럼 바로 대시보드로 이동한다.
- 로그인된 SNS 사용자의 가입정보를 저장하는 API를 추가했다.
- 현재 SNS 보호자 레코드에 휴대폰, 생년월일, 아이디, 비밀번호 해시, 약관동의, 휴대폰 인증시각을 저장한다.
- 뒤로가기 버튼은 로그인 화면으로 돌아갈 수 있도록 로그아웃 처리한다.

### 산출물
- `app/dashboard.js`
- `app/social-signup-completion.js`
- `app/api/signup/complete/route.js`
- `app/globals.css`
- `lib/db.js`
- `deliverables/AUTH_SETUP.md`
- `deliverables/USER_MANUAL.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- 빌드 결과에서 `/api/signup/complete` 라우트 생성 확인.

### 남은 사항
- SNS 가입완성 플로우의 휴대폰 인증도 현재는 테스트 모드이며 실제 문자 발송은 SMS 사업자 연동이 필요하다.

### 반영 시간
- 구현, 빌드 검증, 문서/로그 반영: 약 35분.

### 운영 반영
- GitHub commit:
  - `3763156 Require signup completion after SNS login`
- Vercel production deployment:
  - `https://zezari-fyig126f5-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- 운영 메인 페이지 정상 응답 확인.
- 로그인되지 않은 상태에서 SNS 가입완성 API 호출 시 401 응답 확인.

## 2026-06-19 - 사용자 정보 탭 분리 및 대상자 QR 등록완료 화면

### 요구내용
- 사용자페이지의 정보입력에서 보호자정보와 관리대상정보를 분리한다.
- 탭을 대시보드, 보호자정보, 관리대상정보로 구성한다.
- 관리대상정보 입력 화면을 첨부 화면처럼 구성한다.
- 관리대상별로 보호자가 메모/메시지를 입력할 수 있게 한다.
- 관리대상별로 보호자의 음성을 녹음하고 저장할 수 있게 한다.
- 신규 관리대상 등록 시 DB에 있는 미매칭 QR을 매칭하고 등록완료 페이지를 띄운다.
- 기존 관리대상 수정 시에는 등록완료 페이지를 보여주지 않는다.

### 반영내용
- 사용자 탭을 3개로 분리했다.
  - 대시보드
  - 보호자정보
  - 관리대상정보
- 기존 `tab=info`는 보호자정보 탭으로 호환 처리했다.
- 관리대상 등록/수정 화면을 모바일 폰 카드 형태로 개편했다.
- 관리대상별 보호자 메시지 입력란을 추가했다.
- 관리대상별 보호자 음성녹음 기능을 추가했다.
  - 녹음
  - 정지
  - 재생
  - 새 녹음 삭제
- 신규 관리대상 저장 시 자동 QR 매칭 후 등록완료 화면을 표시한다.
- 기존 관리대상 수정 시에는 관리대상정보 탭으로 돌아가며 완료 화면은 표시하지 않는다.
- QR 공개 페이지에서 보호자 메시지와 음성녹음을 표시/재생하게 했다.
- 대상자 DB에 메시지와 음성 저장 컬럼을 추가했다.

### 산출물
- `app/actions.js`
- `app/dashboard.js`
- `app/find/[key]/page.js`
- `app/globals.css`
- `app/page.js`
- `app/subject-voice-recorder.js`
- `lib/db.js`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/QR_MANAGEMENT.md`
- `deliverables/USER_MANUAL.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- `/find/[key]` 공개 QR 페이지 라우트 빌드 포함 확인.

### 남은 사항
- 브라우저가 MediaRecorder를 지원하지 않으면 음성녹음이 제한된다.
- 음성 파일은 현재 데이터 URL로 저장하며 서버 저장 제한은 2.5MB다.
- 기존 프로젝트 기준인 보호자당 최대 4명 제한은 유지했다.

### 반영 시간
- 탭 분리, 화면 개편, 음성녹음, QR 등록완료, 공개 QR 표시, 문서/로그 반영: 약 70분.

### 운영 반영
- GitHub commit:
  - `82d5d31 Split user info tabs and add subject QR completion`
- Vercel production deployment:
  - `https://zezari-7wwfatpt5-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- 운영 메인 페이지 정상 응답 확인.
- 운영 QR 공개 페이지 샘플 경로 정상 응답 확인.

## 2026-06-19 - 대상자 수정 화면 스타일 오류 수정

### 요구내용
- 대상자 수정 화면은 첨부 화면과 같은 폰 스타일일 필요가 없다.
- 현재 이름, 생년월일, 현재상태 값이 제대로 보이지 않고 입력도 어렵다.
- `대상자 수정` 글자 위의 불필요한 노치 디자인을 제거한다.

### 반영내용
- 기존 대상자 수정 카드는 일반 카드 스타일로 변경했다.
- 신규 대상자 등록 카드에만 폰 스타일을 유지했다.
- 대상자 수정 카드의 상단 노치 디자인을 제거했다.
- 입력 필드 글자 크기 상속 문제를 수정해 이름, 생년월일, 현재상태 값이 보이도록 했다.

### 산출물
- `app/dashboard.js`
- `app/globals.css`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.

### 반영 시간
- 원인 확인, 화면/CSS 수정, 빌드 검증, 로그 반영: 약 15분.

## 2026-06-19 - 마이페이지 추가 및 대상자 등록/수정 스타일 통일

### 요구내용
- 대상자 등록 화면도 대상자 수정 화면과 같은 스타일로 맞춘다.
- 로그아웃, 푸시알림켜짐 버튼을 마이페이지로 이동한다.
- 첨부 이미지와 유사한 정보 요약형 마이페이지를 만든다.
- 스타일은 현재 프로젝트 CSS 기준을 따른다.

### 반영내용
- 사용자 탭에 `마이페이지`를 추가했다.
- 헤더에 있던 푸시 알림 버튼과 로그아웃 버튼을 마이페이지로 이동했다.
- 마이페이지에 보호자 정보 요약을 추가했다.
- 마이페이지에 대표 관리대상 정보 요약을 추가했다.
- 마이페이지에 결제/구독 상태, 푸시 알림, 관리자 페이지 바로가기, 고객지원 메뉴, 로그아웃을 구성했다.
- 대시보드의 `내 정보` 바로가기는 마이페이지로 연결했다.
- 대상자 신규 등록 화면도 수정 화면과 같은 일반 카드 스타일로 변경했다.
- 신규 등록 후 QR 등록완료 화면은 기존대로 유지했다.

### 산출물
- `app/page.js`
- `app/dashboard.js`
- `app/globals.css`
- `deliverables/USER_MANUAL.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.

### 반영 시간
- 탭 추가, 마이페이지 UI 구성, 대상자 폼 스타일 통일, 빌드 검증, 로그 반영: 약 35분.

## 2026-06-19 - 마이페이지 상단 아이콘 분리 및 알림함 추가

### 요구내용
- 마이페이지를 탭에 놓지 않고 화면 오른쪽 위 사람 모양 아이콘으로 배치한다.
- 사람 모양 아이콘에 마우스를 올리면 `마이페이지` 설명이 보이게 한다.
- 화면 왼쪽 위에는 종 모양 아이콘을 배치한다.
- 종 아이콘에서 푸시알림으로 오는 메시지를 확인할 수 있게 한다.

### 반영내용
- 사용자 탭은 `대시보드`, `보호자정보`, `관리대상정보` 3개로 재정리했다.
- 마이페이지는 우측 상단 사람 아이콘 클릭 시 오버레이로 열리게 했다.
- 기존 `?tab=my` 주소는 호환을 위해 대시보드에서 마이페이지 오버레이를 열도록 유지했다.
- 좌측 상단 종 아이콘에 알림 목록, 읽지 않은 알림 수, 새로고침 기능을 추가했다.
- `보호자에게 알리기` 푸시 메시지를 `guardian_notifications` 테이블에 저장하도록 했다.
- 열린 앱 화면은 서비스워커 푸시 수신 시 알림 목록을 갱신한다.

### 산출물
- `app/dashboard.js`
- `app/page.js`
- `app/notification-bell.js`
- `app/api/notifications/route.js`
- `app/globals.css`
- `lib/db.js`
- `lib/push.js`
- `public/sw.js`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/PUSH_NOTIFICATION_SETUP.md`
- `deliverables/USER_MANUAL.md`

### 검증
- 로컬 빌드 성공.

### 반영 시간
- UI 위치 변경, 알림 저장/조회 API, 서비스워커 연동, 문서/로그 반영: 약 40분.

### 운영 반영
- GitHub commit:
  - `7c98dd4 Move my page to corner actions and add notification inbox`
- Vercel production deployment:
  - `https://zezari-itn02cnr3-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- 운영 메인 페이지 정상 응답 확인.
- 알림 API는 비로그인 상태에서 401을 반환해 로그인 보호가 적용된 것을 확인.

## 2026-06-19 - 마이페이지 스타일 정렬 기준 반영

### 요구내용
- 마이페이지가 프로젝트 전체 CSS와 어울리도록 수정한다.
- 향후 첨부 캡처는 화면 구성만 참고하고, 실제 스타일은 프로젝트 CSS를 기준으로 적용한다.

### 반영내용
- 마이페이지의 좁은 팝업 느낌을 줄이고 프로젝트 카드형 패널 스타일로 정리했다.
- 두꺼운 검은 구분선과 검은 아바타를 제거하고, 프로젝트 포인트 컬러와 얇은 테두리 기준으로 변경했다.
- 보호자 정보, 대상자 정보, 부가 정보, 고객지원 영역의 행 정렬과 링크 스타일을 통일했다.
- 모바일 화면에서 정보 행이 한 줄로 압축되어 삐져나오지 않도록 반응형 처리를 추가했다.
- 앞으로 캡처 이미지는 구성 참고용으로만 보고, 최종 화면은 프로젝트 공통 CSS 기준을 따른다는 기준을 기록했다.

### 산출물
- `app/globals.css`
- `deliverables/USER_MANUAL.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- CSS 차이 검사 통과.
- 로컬 빌드 성공.
- 로컬 메인 화면과 관리자 화면 HTTP 200 응답 확인.
- 인앱 브라우저 검증은 현재 세션에서 브라우저가 비활성 상태라 진행하지 못했다.

### 운영 반영
- GitHub commit:
  - `0ef97f1 Align my page with project style`
- Vercel production deployment:
  - `https://zezari-d7jkov9xq-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- 운영 메인 페이지 정상 응답 확인.
- 운영 관리자 페이지 정상 응답 확인.

### 반영 시간
- 마이페이지 CSS 정리, 반응형 보정, 로그 반영: 약 15분.

## 2026-06-19 - 카카오 우편번호 보호자 주소 입력 연동

### 요구내용
- 카카오 우편번호 서비스를 사용해서 보호자 정보에서 주소를 입력받고 싶다.

### 반영내용
- 보호자 정보 화면의 주소 입력 영역에 `주소 검색` 버튼을 추가했다.
- 버튼 클릭 시 카카오 우편번호 검색 팝업이 열리도록 구성했다.
- 사용자가 검색 결과를 선택하면 우편번호와 기본주소가 기존 보호자 주소 입력값에 자동 반영된다.
- 보호자는 자동 반영된 주소 뒤에 상세주소를 직접 이어서 입력할 수 있다.
- 별도 API 키 없이 카카오 공식 우편번호 스크립트를 필요할 때만 불러오도록 구현했다.
- 모바일에서는 주소 입력칸과 검색 버튼이 세로로 배치되도록 반응형 스타일을 적용했다.

### 산출물
- `app/kakao-postcode-address.js`
- `app/dashboard.js`
- `app/globals.css`
- `deliverables/USER_MANUAL.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- CSS 차이 검사 통과.
- 로컬 빌드 성공.
- 로컬 메인 화면 정상 응답 확인.
- 로컬 보호자 정보 탭 정상 응답 확인.
- 카카오 우편번호 팝업의 실제 선택 동작은 현재 세션에서 브라우저가 비활성 상태라 직접 시각 검증하지 못했다.

### 운영 반영
- GitHub commit:
  - `f9e049e Add Kakao postcode address search`
- Vercel production deployment:
  - `https://zezari-i1mxasybm-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- 운영 메인 페이지 정상 응답 확인.
- 운영 관리자 페이지 정상 응답 확인.

### 반영 시간
- 공식 가이드 확인, 주소 검색 컴포넌트 추가, 보호자 폼 연결, 스타일/로그 반영: 약 25분.

## 2026-06-19 - 상품 선택 페이지 및 관리자 상품 관리 추가

### 요구내용
- 대시보드에서 `구독결제하기` 버튼과 상단 구독 기간/가격 선택 박스를 제거한다.
- 기존 `상품 구매` 버튼을 새 상품 선택 페이지로 연결한다.
- 상품 선택 화면은 첨부 화면의 구성만 참고하고, 스타일은 프로젝트 CSS 기준으로 만든다.
- 상품은 관리자가 업로드한 이미지로 구성한다.
- 상품 선택 후 대상자를 선택하고, 구독기간을 선택하면 기간별 구독 결제로 진행한다.
- 상품 단독 구매는 이미 구독중인 고객만 선택할 수 있게 한다.

### 반영내용
- 대시보드 현재 상태 상단의 구독 결제 UI를 제거했다.
- 대시보드 `상품 구매` 버튼을 `/shop`으로 연결했다.
- `/shop` 상품 선택 페이지를 추가했다.
- 상품 선택 후 상세 화면에서 디자인, 수량, 대상자, 구독기간을 선택할 수 있게 구성했다.
- 상품 단독 구매 탭은 구독중 상태에서만 선택 가능하게 제한했다.
- 관리자 페이지에 `상품 관리` 메뉴를 추가했다.
- 관리자는 상품명, 설명, 이미지, 단독 구매 가격, 노출 여부, 정렬 순서를 관리할 수 있다.
- `products`, `product_orders` 테이블을 추가하고 기본 상품 4종을 시드하도록 구성했다.
- 구독 결제 준비 시 상품/대상자/수량 선택값을 주문 초안으로 저장하도록 했다.
- 상품 단독 구매 요청 저장 API를 추가했다.

### 산출물
- `app/shop/page.js`
- `app/shop-checkout-client.js`
- `app/api/products/orders/route.js`
- `app/api/payments/toss/subscription/prepare/route.js`
- `app/admin/page.js`
- `app/admin/actions.js`
- `app/dashboard.js`
- `app/globals.css`
- `lib/db.js`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/USER_MANUAL.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- 빌드 결과에서 `/shop` 및 `/api/products/orders` 라우트 생성 확인.
- 별도 로컬 빌드 서버에서 `/shop` HTTP 200 응답 확인.
- 별도 로컬 빌드 서버에서 `/admin?section=products` HTTP 200 응답 확인.

### 운영 반영
- GitHub commit:
  - `8cfd778 Add product shop flow`
- Vercel production deployment:
  - `https://zezari-31xgubkr0-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- 운영 메인 페이지 정상 응답 확인.
- 운영 상품 선택 페이지 정상 응답 확인.
- 운영 관리자 상품 관리 페이지 정상 응답 확인.

### 반영 시간
- DB 모델, 관리자 상품 관리, 사용자 상품 선택 화면, 결제 준비 연결, 문서/로그 반영: 약 70분.

## 2026-06-19 - 상품 주문 단계화 및 QR 활성화 게이트 적용

### 요구내용
- 상품 선택 후 화면에서 상품 단독구매 설명 영역은 기본 화면에서 제거한다.
- 상품 단독구매 설명은 `상품 단독 구매`를 눌렀을 때만 보이게 한다.
- `다음` 버튼을 누르면 선택한 상품의 상세/미리보기 페이지가 나오게 한다.
- 상세/미리보기 페이지의 버튼은 `결제하기`가 아니라 `주문정보입력`으로 표시한다.
- 주문정보입력 후 배송지와 결제방법을 선택하고 실제 Toss 결제를 진행한다.
- 결제 완료 후 주문 완료 페이지를 보여준다.
- 상품 수령 후 QR을 스캔하면 보호자가 로그인된 경우 QR 활성화 버튼을 제공한다.
- QR 활성화 시 실제 구독기간이 시작되게 한다.
- 활성화되지 않은 QR 공개 페이지에서는 관리대상 정보가 보이지 않게 한다.

### 반영내용
- 상품 구매 화면을 `선택 -> 미리보기 -> 주문정보입력/결제 -> 완료` 단계로 재구성했다.
- 상품 단독 구매 요약/설명은 단독 구매 탭을 선택했을 때만 표시되도록 변경했다.
- 상품 미리보기 화면에 관리자 업로드 상품 이미지를 기반으로 한 주문 미리보기를 추가했다.
- `주문정보입력` 버튼을 추가하고, 다음 단계에서 배송지와 결제방법을 입력받도록 했다.
- Toss 일반 상품 결제 준비/성공/실패 라우트를 추가했다.
- 구독 결제 성공 시 바로 구독기간을 시작하지 않고 `ready` 상태로 보관하도록 했다.
- QR 코드에 보호자 활성화 시각 `activated_at`을 추가했다.
- 보호자가 상품 수령 후 QR 페이지에서 활성화하면 QR 공개와 구독기간 시작이 동시에 처리되도록 했다.
- 활성화 전 QR 공개 페이지는 대상자/보호자 정보를 숨기고 안내 화면만 보여주도록 변경했다.
- 활성화 전 QR에서는 보호자에게 알리기 API도 호출되지 않게 제한했다.

### 산출물
- `app/shop-checkout-client.js`
- `app/api/payments/toss/product/prepare/route.js`
- `app/payments/toss/product/success/page.js`
- `app/payments/toss/product/fail/page.js`
- `app/payments/toss/subscription/success/page.js`
- `app/find/[key]/page.js`
- `app/api/find/[key]/notify/route.js`
- `app/actions.js`
- `app/globals.css`
- `lib/db.js`
- `lib/toss-payments.js`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/USER_MANUAL.md`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- CSS 차이 검사 통과.
- 로컬 빌드 성공.
- 빌드 결과에서 상품 결제 준비/성공/실패 라우트와 `/shop`, `/find/[key]` 라우트 생성 확인.
- 별도 로컬 빌드 서버에서 `/shop`, `/payments/toss/product/fail`, `/admin?section=products` HTTP 200 응답 확인.
- Toss 결제창 최종 승인 단계는 외부 결제 상호작용이 필요해 자동 완료 검증하지 못했다.

### 운영 반영
- GitHub commit:
  - `332206a Add staged product payment and QR activation`
- Vercel production deployment:
  - `https://zezari-4vta9rbcd-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- 운영 메인 페이지 정상 응답 확인.
- 운영 상품 선택 페이지 정상 응답 확인.
- 운영 상품 결제 실패 페이지 정상 응답 확인.
- 운영 관리자 상품 관리 페이지 정상 응답 확인.

### 반영 시간
- 구매 단계 재구성, Toss 상품 결제 라우트, QR 활성화 정책, 공개 페이지 정보 보호, 문서/로그 반영: 약 80분.

## 2026-06-19 - 사용자 탭 스타일 통일 및 대시보드 사진 확대

### 요구내용
- 사용자 페이지의 `대시보드`, `보호자정보`, `관리대상정보` 탭 스타일이 서로 달라 보인다.
- 세 탭의 스타일을 `관리대상정보` 기준으로 통일한다.
- 대시보드 관리대상 사진을 현재보다 약 1.7배 크게 보이게 한다.

### 반영내용
- 대시보드 현재상태 패널, 보호자정보 패널, 관리대상정보 헤더/카드의 폭과 카드 스타일을 통일했다.
- 대시보드의 기존 휴대폰 목업 느낌을 줄이고 관리대상정보와 같은 카드형 패널로 정리했다.
- 보호자정보 화면도 같은 카드 폭, 테두리, 그림자, 헤더 정렬 기준을 따르도록 변경했다.
- 대시보드 관리대상 사진 영역을 확대했다.
- 사진 확대에 맞춰 관리대상 카드의 그리드 폭, 최소 높이, 모바일 정렬을 보정했다.

### 산출물
- `app/globals.css`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- CSS 차이 검사 통과.
- 로컬 빌드 성공.
- 별도 로컬 빌드 서버에서 메인 페이지와 상품 페이지 HTTP 200 응답 확인.

### 운영 반영
- GitHub commit:
  - `a10b693 Unify guardian tab styling`
- Vercel production deployment:
  - `https://zezari-fmjo05wud-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 운영 검증
- 운영 메인 페이지 정상 응답 확인.
- 운영 상품 페이지 정상 응답 확인.

### 반영 시간
- 사용자 탭 공통 스타일 정리, 대시보드 사진 확대, 반응형 보정, 로그 반영: 약 20분.

## 2026-06-20 - 마이페이지 계정 관리 메뉴 확장

### 요구내용
- 마이페이지에 `결제 및 구독 현황` 링크를 추가하고 새 페이지를 만든다.
- 마이페이지에 `쿠폰함`을 추가하고 쿠폰 등록/조회 화면을 만든다.
- 마이페이지에 `결제수단`을 추가하고 사용자가 결제수단 표시 정보를 미리 입력할 수 있게 한다.
- 마이페이지에 `광고 대시보드` 항목을 추가하고 광고 상태를 조회할 수 있는 페이지를 만든다.
- 첨부 화면은 구성만 참고하고, 스타일은 현재 프로젝트의 CSS 기준으로 통일한다.

### 반영내용
- 마이페이지 부가 정보에 다음 링크를 추가했다.
  - 결제 및 구독 현황
  - 쿠폰함
  - 결제수단
  - 광고 대시보드
- `/account/billing` 페이지를 추가해 관리대상별 구독 상태와 최근 상품 결제 내역을 조회할 수 있게 했다.
- `/account/coupons` 페이지를 추가해 쿠폰 번호 등록, 사용가능 쿠폰, 사용완료 쿠폰 영역을 구성했다.
- `/account/payment-methods` 페이지를 추가해 카드사/별칭/끝 4자리/기본 여부 같은 안전한 표시 정보만 저장하도록 했다.
- `/account/ads` 페이지를 추가해 광고 전체/광고중/광고완료 필터와 광고 카드 목록을 구성했다.
- 쿠폰과 결제수단 표시 정보를 DB로 관리하기 위해 `guardian_coupons`, `guardian_payment_methods` 테이블을 추가했다.
- 전체 화면은 기존 카드형 패널, 버튼, 테두리, 색상 토큰을 사용해 프로젝트 스타일로 맞췄다.

### 산출물
- `app/account/account-ui.js`
- `app/account/billing/page.js`
- `app/account/coupons/page.js`
- `app/account/payment-methods/page.js`
- `app/account/ads/page.js`
- `app/actions.js`
- `app/dashboard.js`
- `app/globals.css`
- `lib/db.js`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- CSS 차이 검사 통과.
- 로컬 환경에서 새 계정 보호 라우트가 비로그인 상태에서 `/`로 정상 이동하는 것 확인.
- 운영 배포 후 메인 페이지 HTTP 200 확인.
- 운영 환경에서 새 계정 보호 라우트가 비로그인 상태에서 `/`로 정상 이동하는 것 확인.

### 운영 반영
- GitHub commit:
  - `f78ae48 Add account billing and utility pages`
- Vercel production deployment:
  - `https://zezari-hojnijdb8-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 반영 시간
- 마이페이지 링크 추가, 계정 하위 페이지 4개 생성, DB 스키마/액션 추가, 스타일 정리, 배포/검증: 약 45분.

## 2026-06-20 - 온라인 실종신고 대상자 선택 기능

### 요구내용
- 대시보드의 `실종신고` 버튼이 현재 `관리대상정보`로 이동하는 흐름을 새 실종신고 페이지로 변경한다.
- 첨부 화면을 참고해 “어떤 대상자를 찾고 계신가요?” 대상자 선택 화면을 만든다.
- 대상자 상태를 4가지로 재구성한다.
  - 상품구매필요
  - QR활성화필요
  - 안전
  - 찾는중
- 상태별 동작을 다르게 구성한다.
  - 상품구매필요: 선택 불가, 상품 구매 필요 경고와 상품구매 페이지 링크 안내.
  - QR활성화필요: 선택 불가, 상품 내 QR 활성화 필요 경고.
  - 안전: 온라인 실종 광고 신청 화면으로 이동.
  - 찾는중: 광고 대시보드로 이동.

### 반영내용
- `/missing-report` 신규 페이지를 만들었다.
- 대시보드 하단 `실종신고` 버튼을 `/missing-report`로 연결했다.
- 관리대상정보의 현재 상태 선택값을 새 4개 상태로 변경했다.
- 기존 `문제없음` 상태는 `안전`으로 정규화되도록 처리했다.
- 신규 대상자는 기본적으로 `상품구매필요` 상태가 되도록 변경했다.
- 상품 결제 완료 후 대상자가 `QR활성화필요`로 전환되도록 연결했다.
- QR 활성화 완료 후 대상자가 `안전`으로 전환되도록 연결했다.
- 관리자 화면과 QR 공개 페이지도 새 상태명이 표시되도록 맞췄다.
- 프로젝트 CSS 기준으로 대상자 선택 카드, 상태 배지, 경고 안내, 다음 버튼 스타일을 추가했다.

### 산출물
- `app/missing-report/page.js`
- `app/missing-report/missing-report-selector.js`
- `app/dashboard.js`
- `app/admin/page.js`
- `app/find/[key]/page.js`
- `app/globals.css`
- `lib/db.js`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- CSS 차이 검사 통과.
- 로컬 메인 페이지 정상 응답 확인.
- 로컬 `/missing-report`가 비로그인 상태에서 `/`로 정상 이동하는 것 확인.
- 운영 메인 페이지 정상 응답 확인.
- 운영 `/missing-report`가 비로그인 상태에서 `/`로 정상 이동하는 것 확인.

### 운영 반영
- GitHub commit:
  - `710c79d Add online missing report selector`
- Vercel production deployment:
  - `https://zezari-cixce9b8q-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 반영 시간
- 실종신고 페이지 생성, 상태 체계 변경, 결제/QR 활성화 상태 전환 연결, 배포/검증: 약 55분.

## 2026-06-20 - 전체 페이지 이동 속도 원인 분석

### 요구내용
- 페이지와 대시보드 탭 이동이 전체적으로 느린 원인을 파악한다.

### 분석결과
- 대시보드 탭이 일반 `<a>` 링크라 이동할 때마다 전체 문서를 다시 요청한다.
- 로그인 사용자 페이지는 캐시되지 않아 이동마다 Vercel 서버 함수가 실행된다.
- 공통 `getDashboardData()`가 모든 페이지에서 보호자, 대상자, QR, 광고, 구독, 요금제, 광고단가를 함께 조회한다.
- 대상자 사진과 음성이 base64로 DB에 저장되고 `SELECT s.*`로 매번 함께 조회된다.
- 현재 대상자 3명 기준 대상자 조회 payload는 약 59만 문자다.
- Turso 웜 상태의 대시보드 관련 순차 쿼리는 약 187ms, 초기 연결 조회는 약 1.2초가 측정됐다.
- 서버리스 인스턴스가 새로 뜰 때마다 DB 스키마 검사와 seed 쓰기를 실행한다.
- 한국 요청이 서울 엣지를 거친 뒤 미국 `iad1` 서버 함수에서 처리되어 원거리 지연이 추가된다.
- QR 이미지 생성은 웜 상태 약 5ms로 주원인이 아니다.

### 개선 우선순위
- 탭 이동을 클라이언트 상태 또는 Next.js `Link`로 전환한다.
- 페이지별 DB 조회를 분리하고 필요한 필드만 조회한다.
- 사진/음성을 파일 저장소로 이동하고 DB에는 URL만 저장한다.
- 스키마 마이그레이션과 seed를 배포 단계로 분리한다.
- 독립 DB 쿼리를 병렬 또는 batch 처리한다.
- Vercel 함수와 Turso 지역을 한국 사용자와 가까운 곳으로 맞춘다.

### 측정 및 결론
- 비로그인 운영 메인 TTFB는 반복 측정 시 약 0.24-0.30초였다.
- 따라서 Vercel 요금제 자체보다 로그인 후 데이터 조회 구조와 전체 새로고침 방식이 핵심 원인이다.
- 이번 단계에서는 원인 분석만 수행했고 애플리케이션 소스는 변경하지 않았다.

### 분석 시간
- 코드 추적, 운영 TTFB 측정, Turso 쿼리/데이터 크기 측정, 개선 우선순위 정리: 약 25분.

## 2026-06-20 - 페이지 이동 성능 개선 구현

### 요구내용
- 분석된 페이지 이동 지연 원인을 실제 소스에서 개선한다.
- GitHub 푸시와 Vercel 운영 배포까지 완료한다.

### 반영내용
- 대시보드, 관리자, 계정, 상품, 실종신고의 주요 내부 링크를 Next.js `Link`로 변경했다.
- 화면 이동 중 즉시 표시되는 전역 로딩 진행 UI를 추가했다.
- 페이지별로 필요한 데이터만 조회하도록 `getDashboardData` 옵션을 분리했다.
- 보호자/대상자/구독/요금제/광고단가 조회를 Turso batch 1회로 통합했다.
- 공통 조회에서 사진과 음성 base64를 제외했다.
- 대상자 사진은 인증된 별도 API에서 제공하고 브라우저가 1일 캐시하도록 변경했다.
- 사진 URL에 대상자 수정시각을 버전값으로 사용해 수정 시 자동 갱신되게 했다.
- 서버리스 냉간 시작 때 전체 DB 마이그레이션을 반복하지 않도록 스키마 버전 테이블을 추가했다.
- 저장, 구독, 광고, 알림, 쿠폰, 결제수단 액션도 보호자 최소 정보만 조회하도록 변경했다.

### 개선 측정
- 대상자 3명 기준 공통 데이터:
  - 약 59만 문자에서 약 3천 문자로 감소
  - 약 99.4% 감소
- 웜 대시보드 DB 조회:
  - 약 187ms에서 약 41ms로 감소
  - 약 78% 감소
- 로컬 로그인 대시보드 HTML 약 21KB.
- 사진 API 인증/캐시 응답 정상 확인.
- 운영 웜 로그인 대시보드 응답 약 0.42~0.73초.

### 산출물
- `app/api/subjects/[id]/photo/route.js`
- `app/loading.js`
- `app/page.js`
- `app/dashboard.js`
- `app/globals.css`
- `app/shop/page.js`
- `app/missing-report/page.js`
- `app/missing-report/missing-report-selector.js`
- `app/account/*`
- `app/admin/page.js`
- `lib/db.js`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 운영 반영
- GitHub commits:
  - `d02c8f7 Optimize authenticated navigation performance`
  - `4a5c16b Batch dashboard database reads`
- Vercel production deployment:
  - `https://zezari-ri40e4t5m-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 잔여사항
- Vercel 함수가 계속 미국 `iad1`에서 실행되어 첫 냉간 요청은 약 2초대가 남는다.
- 프로젝트 요금제의 서울 리전 지원 여부가 확인되면 Vercel/Turso 리전 정렬을 추가 검토한다.

### 반영 시간
- Link 전환, DB 조회 분리/batch, 사진 API/캐시, 스키마 버전 처리, 배포 및 성능 검증: 약 90분.

## 2026-06-20 - 알림 좌우 스와이프 삭제

### 요구내용
- 상단 종 아이콘의 알림 메시지를 왼쪽 또는 오른쪽으로 밀어서 삭제할 수 있게 한다.

### 반영내용
- 알림 행에 좌우 포인터/터치 스와이프 제스처를 추가했다.
- 72px 이상 이동하면 삭제되고, 기준 미만이면 원위치로 돌아온다.
- 좌우 어느 방향으로 밀어도 빨간 삭제 영역과 삭제 애니메이션이 표시된다.
- 세로 알림 목록 스크롤은 계속 사용할 수 있도록 가로 방향 제스처만 판별한다.
- 화면에서 먼저 제거한 뒤 API가 실패하면 알림을 복원하고 오류 메시지를 표시한다.
- 인증된 보호자 본인의 알림만 삭제하는 DELETE API와 DB 함수를 추가했다.

### 산출물
- `app/notification-bell.js`
- `app/api/notifications/route.js`
- `app/globals.css`
- `lib/db.js`
- `logs/DEV_HANDOFF_LOG.md`
- `logs/PRESENTATION_PROGRESS_LOG.md`

### 검증
- 로컬 빌드 성공.
- 로컬 임시 알림 삭제 API HTTP 200 및 DB 잔여 0건 확인.
- 운영 임시 알림 삭제 API HTTP 200 및 DB 잔여 0건 확인.
- 테스트 알림은 검증 직후 삭제했다.

### 운영 반영
- GitHub commit:
  - `597700e Add swipe to delete notifications`
- Vercel production deployment:
  - `https://zezari-piy5bot3y-zezari.vercel.app`
- Production alias:
  - `https://zezari.vercel.app`

### 반영 시간
- 스와이프 제스처, 낙관적 삭제/복원, 보호자 소유권 확인 API, 배포 및 검증: 약 40분.

## 2026-06-20 - 주소검색 오류 개선 및 상세주소 연동

### 요구내용
- 반응하지 않는 카카오 주소검색 버튼을 정상화한다.
- 보호자 주소에 상세주소 입력란을 추가한다.
- 상품 주문 배송지에서도 기본주소와 상세주소를 함께 선택하고 결제 주문에 저장한다.

### 반영내용
- 잘못 사용한 `window.kakao.Postcode`를 실제 서비스 객체인 `window.daum.Postcode`로 수정했다.
- 모바일/PWA의 팝업 차단 영향을 줄이기 위해 주소검색을 새 창 대신 화면 내 모달로 제공한다.
- 주소 선택 후 상세주소 입력란으로 자동 이동하며, 로딩·성공·오류 상태를 표시한다.
- 보호자 기본주소와 상세주소를 분리 저장하고 마이페이지 및 활성화된 QR 공개 페이지에는 합쳐서 표시한다.
- 상품 주문 화면에 동일한 주소검색과 상세주소 입력을 적용하고, 구독/단독구매 결제 준비 데이터에 모두 포함했다.
- Turso DB에 보호자 및 주문 상세주소 컬럼을 추가하고 스키마 버전을 2로 올렸다.

### 산출물
- 주소검색 공통 컴포넌트 및 모달 UI
- 보호자 상세주소 저장 기능
- 주문 배송지 상세주소 저장 및 결제 API 연동
- DB 스키마 버전 2

### 검증 및 운영 반영
- 로컬/Vercel 프로덕션 빌드 성공.
- 카카오 우편번호 스크립트 HTTP 200 확인.
- 인증된 보호자 화면에서 주소검색 버튼과 상세주소 필드 확인.
- 운영 공개/인증 페이지 HTTP 200 및 DB 상세주소 컬럼 생성 확인.
- GitHub commit: `6a456f1`
- Vercel deployment: `https://zezari-qy3k3xtve-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`

### 반영 시간
- 오류 분석, 주소검색 모달 전환, 상세주소 DB/결제 연동, 빌드·배포·검증: 약 30분.

## 2026-06-20 - 관리자 주문/배송 관리

### 요구내용
- 일반 쇼핑몰처럼 관리자가 주문을 조회하고 배송상태, 택배사, 송장번호를 관리한다.

### 반영내용
- 관리자 페이지에 `주문/배송` 메뉴를 추가했다.
- 전체 주문과 결제완료, 배송준비, 배송중, 배송완료 건수를 요약한다.
- 주문번호, 보호자, 대상자, 상품, 송장번호 검색과 결제/배송상태 필터를 제공한다.
- 주문상품, 금액, 결제수단, 주문자, 수령인, 연락처, 기본주소와 상세주소를 조회한다.
- 배송상태, 택배사, 송장번호, 관리자 메모를 저장한다.
- 발송일과 배송완료일을 자동 기록한다.
- 결제 전 배송처리를 차단하고, 배송중/완료 처리에는 택배사와 송장번호를 필수로 받는다.
- 배송중 또는 배송완료 시 보호자 종 알림을 생성한다.
- 보호자 결제내역에도 배송상태와 송장번호를 표시한다.

### 산출물
- `deliverables/ADMIN_ORDER_SHIPPING.md`
- 관리자 주문/배송 화면 및 서버 액션
- DB 스키마 버전 3
- 발표용 주문·배송 흐름 이미지 생성 프롬프트

### 검증 및 운영 반영
- 로컬 및 Vercel 프로덕션 빌드 성공.
- 임시 주문으로 배송상태, 택배사, 송장번호, 메모 저장을 검증하고 테스트 데이터 삭제 확인.
- 운영 관리자 주문/배송 페이지 HTTP 200 및 물류 컬럼 생성 확인.
- GitHub commit: `16302dd`
- Vercel deployment: `https://zezari-ozrcklodp-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`

### 반영 시간
- 주문/배송 DB, 관리자 UI, 사용자 배송조회, 검증 및 배포: 약 60분.

## 2026-06-20 - 관리자 주문/배송 탭 가시성 개선

### 요구내용
- 관리자페이지에서 주문/배송 관리 탭이 보이지 않는 문제를 수정한다.

### 반영내용
- 주문/배송 탭을 보호자 관리 다음 두 번째 위치로 이동했다.
- 가로 스크롤 메뉴를 화면 폭에 따라 자동 줄바꿈되는 그리드 메뉴로 변경했다.
- 모바일과 좁은 화면에서도 모든 관리자 메뉴가 화면 안에 표시된다.

### 검증 및 운영 반영
- 운영 관리자 인증 화면 HTTP 200.
- 주문/배송 탭 존재 및 두 번째 배치 확인.
- GitHub commit: `7dae21c`
- Vercel deployment: `https://zezari-gsj4yjnj4-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`

### 반영 시간
- 원인 확인, 메뉴 재배치, 반응형 메뉴 수정, 재배포 및 검증: 약 15분.

## 2026-06-23 - 구버전 SNS 로그인·토스 연동 이관

### 요구내용
- `reference`의 구버전 zezari 구현을 참고해 카카오·네이버 로그인과 토스 결제를 현재 사이트에 반영한다.
- 로컬 반영과 테스트를 먼저 진행한다.

### 반영내용
- 구버전 WordPress SQL에서 OAuth와 결제 설정 구조를 분석했다.
- 카카오 Client Secret을 필수로 요구하던 현재 코드 오류를 수정했다.
- 카카오 `profile_nickname` 범위와 네이버 ID/Secret 구성을 로컬에 반영했다.
- 상품 단독 구매에 카드, 실시간 계좌이체, 가상계좌를 제공한다.
- 구독은 카드 빌링키 결제만 허용한다.
- 결제 성공 시 보호자 소유권, 주문번호, 결제금액, 승인금액, 승인상태를 검증한다.
- 중복 콜백에서 결제를 다시 승인하지 않도록 보강했다.
- 운영키와 개인정보가 있는 `reference/`를 Git 제외 처리했다.
- Vercel 업로드에서도 제외되도록 `.vercelignore`를 추가하고 94개 앱 파일만 포함한 깨끗한 후보 배포를 생성했다.

### 검증
- 로컬 빌드 성공.
- 카카오·네이버 제공자 노출 및 각 공식 인증 호스트 이동 확인.
- 카카오 scope와 새 NextAuth 콜백 경로 확인.
- 계좌이체·가상계좌 주문 준비 성공 및 미지원 결제수단 HTTP 400 차단 확인.
- 다른 보호자의 상품주문·구독 콜백 접근 차단 확인.
- 임시 주문 2건 삭제 후 잔여 0건 확인.
- 운영 Vercel에서 Google·Kakao·Naver 제공자와 공식 인증 호스트 이동 확인.

### 산출물
- `deliverables/REFERENCE_AUTH_TOSS_INTEGRATION.md`
- 구버전 이관 발표용 이미지 생성 프롬프트

### 운영 전 주의
- Kakao/Naver 콘솔에 `zezari.vercel.app` 콜백 URL 등록이 필요하다.
- 구버전 Toss 실결제 키는 명시적 승인 전까지 적용하지 않고 기존 테스트 키를 유지한다.

### 운영 반영
- Vercel Production/Development에 Kakao·Naver 환경변수 등록 완료.
- GitHub commit: `6fb9a79`
- Vercel deployment: `https://zezari-2rut2jo77-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`

### 반영 시간
- 구버전 분석, 보안 보강, 로컬·외부 DB 검증, 환경변수 등록, 배포 및 운영 검증: 약 90분.

## 2026-06-24 - 네이버 신규키·토스 라이브키 운영 전환

### 요구내용
- `.env.local`의 네이버 신규키를 유지한다.
- 기존 토스 키를 주석 처리하고 준비된 라이브키를 활성화한다.
- 로컬 기능 테스트 후 GitHub와 Vercel 운영 환경에 반영한다.

### 반영내용
- 네이버 신규 Client ID/Secret은 값 변경 없이 유지했다.
- 로컬 토스 기존 키는 주석 처리하고 라이브 Client/Secret을 활성화했다.
- Vercel Production에 네이버 신규키와 토스 라이브키를 등록했다.
- Vercel Development는 네이버 신규키를 반영하고 토스 테스트키를 유지했다.
- 키와 시크릿 값은 Git 및 문서에 기록하지 않았다.

### 검증결과
- Next.js 프로덕션 빌드 성공.
- 로컬·운영 네이버 로그인 시작 URL과 신규 Client ID 사용 확인.
- 토스 라이브 시크릿의 읽기 전용 API 인증 성공.
- 운영 페이지 및 인증 제공자 API HTTP 200 확인.
- 실제 승인이나 과금은 발생시키지 않았다.

### 운영반영
- 배포: `https://zezari-171s2oo07-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 상태: Ready

### 반영시간
- 환경 전환, 안전 검증, 빌드, 배포, 운영 확인: 약 35분.

## 2026-06-24 - 관리자 세로 메뉴·숨김 기능

### 요구내용
- 관리자 페이지 상단의 가로 탭을 왼쪽 세로 메뉴로 변경한다.
- 세로 메뉴를 숨기거나 다시 펼칠 수 있게 한다.

### 반영내용
- 보호자, 주문/배송, QR, 관리자, 결제, 상품, 광고 관리 메뉴를 왼쪽에 세로 배치했다.
- 메뉴 숨기기/펼치기 아이콘 버튼을 추가했다.
- 접힘 상태를 저장해 새로고침과 메뉴 이동 후에도 유지되게 했다.
- 메뉴를 접으면 작업 영역이 자동으로 넓어지게 했다.
- 모바일에서는 메뉴와 본문을 위아래 한 열로 배치했다.
- 기존 관리자 기능과 URL, 데이터 조회 방식은 그대로 유지했다.

### 검증결과
- 프로덕션 빌드 성공.
- 펼침 216px, 접힘 56px 동작 확인.
- 활성 메뉴 강조와 접힘 상태 저장 확인.
- 모바일 본문 순서와 가로 넘침 없음 확인.
- 운영 DB와 분리한 임시 DB로 로그인 화면 검증 후 테스트 파일 삭제.

### 산출물
- `deliverables/ADMIN_SIDEBAR_NAVIGATION.md`
- 발표자료용 펼침/접힘 비교 이미지 생성 프롬프트 포함.

### 반영시간
- 구조 확인, 구현, 반응형 보정, 화면 검증, 문서화: 약 35분.

### GitHub·운영반영
- GitHub 기능 커밋: `85e6e3b`
- Vercel 배포: `https://zezari-80jqysx8b-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 배포 상태: Ready

## 2026-06-24 - 관리자 운영 대시보드

### 요구내용
- 관리자 메뉴 최상단에 대시보드를 추가한다.
- 전체 회원, 관리대상자, 활성 QR, 실종신고, 광고 진행, 월매출, 상품매출, 구독매출을 카드로 표시한다.
- 첨부 화면은 구성만 참고하고 프로젝트 CSS 스타일을 유지한다.

### 반영내용
- 관리자 기본 화면과 메뉴 첫 항목을 대시보드로 변경했다.
- 8개 운영 지표 카드를 추가했다.
- 월 선택기로 과거 월의 상품·구독 매출을 조회할 수 있게 했다.
- 월매출은 상품매출과 구독매출의 합으로 계산한다.
- 구독 주문과 최신 구독 결제 기록의 중복 집계를 방지했다.
- 보호자 관리 링크와 선택 보호자 복귀 경로를 새 메뉴 구조에 맞게 수정했다.
- 데스크톱 4열, 중간 화면 2열, 모바일 1열로 반응형 처리했다.

### 검증결과
- 프로덕션 빌드 성공.
- 임시 DB에서 8개 카드의 기대값과 실제 표시값 일치.
- 조회 월 변경에 따른 매출 변경 확인.
- 대시보드 기본 진입과 보호자 관리 이동 확인.
- 데스크톱·모바일에서 텍스트 잘림과 가로 넘침 없음 확인.
- 테스트 DB와 캡처 파일 삭제 완료.

### 산출물
- `deliverables/ADMIN_DASHBOARD_METRICS.md`
- 지표 정의, 매출 집계 기준, 운영 참고사항, 발표용 이미지 생성 프롬프트 포함.

### 반영시간
- 지표 정의, 구현, 경로 보정, 집계 테스트, 화면 검증, 문서화: 약 45분.

### GitHub·운영반영
- GitHub 기능 커밋: `620e0dd`
- Vercel 배포: `https://zezari-kartwiqqy-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 배포 상태: Ready
- 운영 관리자 인증 상태에서 대시보드 HTTP 200 및 8개 지표 표시 확인.

## 2026-06-24 - 관리자 대시보드 최근현황

### 요구내용
- 8개 지표 카드 아래에 최근현황 영역을 추가한다.
- 신규가입자, 신규주문, QR 활성화 요청, 실종신고, 고객문의 최신 4건을 각각 카드로 표시한다.
- 각 카드에 더보기 링크를 제공한다.

### 반영내용
- 최근현황 카드 5개를 데스크톱 5열로 배치했다.
- 각 데이터는 최신순 4건으로 제한했다.
- 더보기를 보호자, 주문/배송, QR, 광고, 고객문의 관리 화면에 연결했다.
- 날짜·시간을 한국시간 `YYYY.MM.DD HH:mm` 형식으로 통일했다.
- 고객문의 DB 테이블과 관리자 조회 화면을 준비했다.
- 모바일은 카드가 한 열로 표시되게 했다.

### 검증결과
- 프로덕션 빌드 성공.
- 항목별 5건 중 최신 4건만 표시되고 가장 오래된 데이터가 제외되는지 확인.
- 더보기 5개 경로 모두 HTTP 200 확인.
- 고객문의 목록 테스트 데이터 5건 조회 확인.
- 스키마 버전 4 확인.
- 데스크톱·모바일에서 가로 넘침과 텍스트 잘림 없음 확인.
- 테스트 DB와 캡처 파일 삭제 완료.

### 산출물
- `deliverables/ADMIN_DASHBOARD_RECENT_STATUS.md`
- 데이터 기준, 화면 연결, 스키마 준비, 검증 결과, 발표용 이미지 생성 프롬프트 포함.

### 반영시간
- 데이터 매핑, 스키마, UI, 링크, 최신 4건 테스트, 반응형 검증, 문서화: 약 50분.

### GitHub·운영반영
- GitHub 기능 커밋: `68cbe18`
- Vercel 배포: `https://zezari-m5i4jao30-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 배포 상태: Ready
- 운영 대시보드·고객문의 화면 HTTP 200 확인.
- Turso 스키마 버전 4와 고객문의 테이블 생성 확인.

## 2026-06-24 - 보호자·관리대상자 관리자 화면 분리

### 요구내용
- 기존 보호자와 관리대상자가 섞인 화면을 두 메뉴로 분리한다.
- 두 화면을 왼쪽 선택 그리드와 오른쪽 상세정보로 구성한다.
- 보호자는 배송지, 대상자, 구독, 결제, 광고, 관리메모를 확인한다.
- 관리대상자는 기본정보, 보호자 메시지, 음성, 부가정보를 확인한다.

### 반영내용
- 보호자 관리와 관리대상자 관리 메뉴를 분리했다.
- 보호자 6개 열과 관리대상자 6개 열의 행 전체 선택 그리드를 만들었다.
- 보호자 검색·상태 필터와 관리대상자 검색·상태·QR 필터를 추가했다.
- 보호자 상세에 배송지, 대상자 링크, 구독, 결제, 광고, 메모를 구성했다.
- 관리대상자 상세에 사진, 기본정보, 메시지, 음성, QR과 등록정보를 구성했다.
- 관리자 메모 저장을 추가하고 DB 스키마를 버전 5로 갱신했다.
- 목록에서는 사진·음성 원본을 제외해 조회 성능을 보호했다.
- 데스크톱 2열, 모바일 1열로 반응형 처리했다.

### 검증결과
- 프로덕션 빌드 성공.
- 보호자·관리대상자 목록과 선택 상세 데이터 일치 확인.
- 관리메모 저장과 성공 메시지 확인.
- 보호자 활성상태 및 관리대상자 QR 상태 필터 확인.
- 보호자 메시지·음성·QR 공개 링크 확인.
- 데스크톱 6개 열 전체 표시 및 모바일 가로 넘침 없음 확인.
- 테스트 DB와 캡처 파일 삭제 완료.

### 산출물
- `deliverables/ADMIN_GUARDIAN_SUBJECT_MANAGEMENT.md`
- 보호자·관리대상자 화면별 데이터 계약, 스키마, 검증, 이미지 생성 프롬프트 포함.

### 반영시간
- 데이터 설계, 스키마, 서버 액션, 두 화면, 필터, 화면 보정, 테스트, 문서화: 약 75분.

### GitHub·운영반영
- GitHub 기능 커밋: `aec4921`
- Vercel 배포: `https://zezari-kuey3f4bf-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 배포 상태: Ready
- 운영 보호자·관리대상자·주문 화면 HTTP 200 확인.
- Turso 스키마 버전 5와 관리자 메모 컬럼 확인.

## 2026-06-24 - 주문/배송 관리 그리드 개편

### 요구내용
- 큰 주문 카드를 표 형태로 바꾸고 여러 주문을 한눈에 비교한다.
- 주문을 선택해 택배사, 송장번호, 메모 등 배송 상세를 입력하고 저장한다.

### 반영내용
- 화면을 `주문 현황 + 왼쪽 주문 그리드 + 오른쪽 상세정보`로 재구성했다.
- 그리드에서 주문번호, 주문자·대상자, 상품, 금액, 결제·배송상태, 주문일을 조회한다.
- 선택 행은 강조되고 선택 주문은 URL에 유지된다.
- 상세정보에서 상품·결제정보, 수령인, 기본·상세주소를 확인한다.
- 배송상태, 택배사, 송장번호, 관리자 메모를 입력·저장한다.
- 결제 전 주문은 배송 처리 상태를 제한하고, 송장 필수 검증은 기존 운영 규칙을 유지했다.
- 데스크톱은 좌우 2열, 태블릿·모바일은 위아래 1열로 구성했다.

### 검증결과
- 프로덕션 빌드 성공.
- 주문 3건 그리드와 선택 상세 데이터 일치 확인.
- 배송정보 저장 후 DB 반영 확인.
- 결제 대기 주문 상태 제한과 배송 중 필터 확인.
- 데스크톱·모바일 페이지 가로 넘침 없음 확인.

### 산출물
- `deliverables/ADMIN_ORDER_GRID_MANAGEMENT.md`
- 화면 구성, 저장 규칙, 반응형 기준, 검증 결과, 발표용 이미지 생성 프롬프트 포함.

### 반영시간
- 화면·데이터 개편, 저장 테스트, 반응형 검증, 문서화: 약 35분.

### GitHub·운영반영
- GitHub 기능 커밋: `b3715ec`
- Vercel 배포: `https://zezari-h8yvds49o-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 배포 상태: Ready
- 인증된 운영 주문/배송 화면 HTTP 200, 주문 그리드·7개 열·상세 패널 확인.

## 2026-06-24 - 토스 결제위젯 라이브 연동

### 요구내용
- `.env.local`의 토스 결제위젯 신규 키 2개를 실제 상품 결제에 반영한다.

### 반영내용
- 기존 구독 빌링 키와 신규 상품 결제위젯 키를 분리했다.
- 주문정보 화면에 토스가 직접 제공하는 결제수단과 필수약관 위젯을 표시한다.
- 사용자가 결제하기를 누르면 최신 배송지·상세주소로 결제대기 주문을 생성하고 토스 결제 인증을 시작한다.
- 결제 성공 후 서버가 위젯 시크릿 키로 승인하고 주문번호·금액·승인상태가 일치할 때만 결제완료 처리한다.
- 실제 결제수단을 주문정보에 저장한다.
- 카드번호와 계좌정보는 서비스 DB에 저장하지 않는다.
- Vercel 운영·개발 환경에 신규 위젯 키를 암호화 등록했다.

### 검증결과
- 프로덕션 빌드 성공.
- 실제 토스 라이브 결제위젯의 결제수단·필수약관 렌더링 확인.
- 모바일 결제화면과 결제하기 버튼 활성화 확인.
- 주문 준비 API의 결제대기·금액·상세주소 저장 확인.
- 실제 금전 거래는 자동 테스트하지 않고 사용자의 소액 실결제 확인 단계로 남겼다.

### 산출물
- `deliverables/TOSS_PAYMENT_WIDGET_LIVE_INTEGRATION.md`
- 키 분리, 상품·구독 결제 흐름, 보안 규칙, 검증, 발표용 이미지 생성 프롬프트 포함.

### 반영시간
- 결제 구조 분석, 위젯·승인 구현, 로컬 SDK 검증, Vercel 환경설정, 문서화: 약 55분.

### GitHub·운영반영
- GitHub 기능 커밋: `2abfee9`
- Vercel 배포: `https://zezari-1ms1dpbib-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 배포 상태: Ready
- 운영 상품 결제화면에서 토스 라이브 결제수단·필수약관 위젯과 결제하기 버튼 활성화 확인.
- 운영 검증 중 주문 생성과 실제 금전 거래는 실행하지 않음.

## 2026-06-24 - 자동결제 없는 선불 이용권 전환

### 요구내용
- 토스 자동결제 빌링 계약 대신 1개월, 3개월, 6개월 이용권을 한 번씩 결제한다.
- 최초 이용기간은 주문 대상자의 QR을 활성화한 날부터 시작한다.
- 추가 구매는 기존 만료일 뒤에 연장하고, 일시정지 기간은 이용기간에서 차감하지 않는다.
- 이용권 만료나 일시정지 상태에서는 공개 QR에서 개인정보를 보여주지 않는다.

### 반영내용
- 구독 빌링키 발급과 자동결제 API를 제거하고 토스 결제위젯 일회성 승인으로 통일했다.
- 최초 결제 완료 상태를 `QR 활성화 대기`로 저장한다.
- 결제 때 선택한 관리대상자의 QR만 이용권을 시작할 수 있게 검증한다.
- 1·3·6개월 달력 월 연장, 월말 날짜 보정, 기존 만료일 기준 갱신을 적용했다.
- 일시정지 후 재개하면 실제 정지 시간만큼 만료일을 연장한다.
- 만료일이 지났거나 없는 활성 데이터는 만료 처리한다.
- 공개 QR은 준비·정지·만료 상태에서 대상자명, 보호자명, 연락처, 주소를 숨긴다.
- 마이페이지 결제 및 이용권 현황에 구매, 일시정지, 재개, QR 활성화 대기 상태를 표시한다.

### 검증결과
- 프로덕션 빌드 성공.
- 이용권 생명주기 DB 시나리오 7개 통과.
- QR 대상 일치와 개인정보 차단 경계 시나리오 5개 통과.
- 실제 토스 결제수단·약관 위젯 로딩과 결제 버튼 활성화 확인.
- 데스크톱 1440px, 모바일 390px 가로 넘침 없음.
- QR 활성화, 일시정지, 재개 화면 흐름 확인.
- 실제 금전 거래는 실행하지 않음.

### 산출물
- `deliverables/PREPAID_PASS_PAYMENT.md`
- 상태 전이, 결제 보안, QR 개인정보 정책, 운영 체크리스트, 발표용 이미지 생성 프롬프트 포함.

### 반영시간
- 결제 구조 전환, 기간 계산, 개인정보 보호, 화면·DB 테스트, 문서화: 약 75분.

### GitHub·운영반영
- GitHub 기능 커밋: `c786b3d`
- Vercel 배포: `https://zezari-aio9zrzb1-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 배포 상태: Ready
- 운영 Turso 스키마 버전 6과 1·3·6개월 이용권 이름 확인.
- 운영 마이페이지의 선불 이용권 문구, 결제위젯 설정 API 200, 토스 결제수단·약관 iframe 2개, 결제 버튼 활성화 확인.
- 운영 검증 중 주문 생성과 실제 금전 거래는 실행하지 않음.

## 2026-06-25 - 관리자 광고관리 그리드 전환

### 요구내용
- 관리자페이지의 광고관리 화면을 카드 형태가 아닌 그리드 형태로 바꾼다.
- 그리드에는 광고 번호, 관리대상, 보호자, 상태, 광고지역, 광고기간, 광고비, 클릭수를 보여준다.
- 그리드 위쪽에 광고승인, 광고정지, 광고재개, 광고상세정보 버튼을 둔다.
- 추후 Meta 광고 API 연동 때 사용할 수 있는 구조로 만든다.

### 반영내용
- 광고관리 화면을 기존 카드형 목록에서 운영자용 그리드와 오른쪽 상세 패널로 변경했다.
- 광고번호/관리대상/보호자/연락처 통합 검색과 상태 필터를 추가했다.
- 그리드에서 광고를 선택하면 URL에 선택 광고가 반영되고 상세정보가 표시된다.
- 선택된 광고 1건에 대해 광고승인, 광고정지, 광고재개 버튼이 동작하도록 서버 액션을 추가했다.
- 상태에 맞지 않는 버튼은 비활성화되도록 처리했다.
- 광고 상태 라벨을 광고중, 정지중, 승인대기, 만료로 정리했다.
- Meta API 리포팅을 대비해 `subject_ads.click_count` 컬럼을 추가하고 기본값을 0으로 관리한다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과.

### 산출물
- `deliverables/ADMIN_AD_GRID_MANAGEMENT.md`
- 기존 `ADVERTISING_SETUP.md`, `DATABASE_SCHEMA.md`, 발표용 이미지 프롬프트를 최신화했다.

### 반영시간
- 화면 전환, 상태 액션, DB 클릭수 컬럼, CSS, 문서화, 빌드 검증: 약 35분.

### GitHub·운영반영
- GitHub 기능 커밋: `c4288df`
- Vercel 배포: `https://zezari-q8466w9ph-zezari.vercel.app`
- 운영 주소: `https://zezari.vercel.app`
- 배포 상태: Ready
- 운영 주소 HTTP 200 확인.

## 2026-06-25 - 관리자 실종신고 관리 메뉴 추가

### 요구내용
- 관리자화면에 실종신고 메뉴를 추가한다.
- 신고일시, 대상자, 보호자, 신고상태, 광고상태, 발견여부를 그리드로 표현한다.

### 반영내용
- 관리자 왼쪽 메뉴에 `실종신고 관리`를 추가했다.
- `/admin?section=missing` 화면을 새로 구성했다.
- 신고일시 기간 필터와 대상자/보호자 통합 검색을 추가했다.
- 그리드 컬럼을 신고일시, 대상자, 보호자, 신고상태, 광고상태, 발견여부로 구성했다.
- 대상자와 보호자 이름을 누르면 각각 기존 관리자 상세 화면으로 이동한다.
- 관리자 대시보드 최근현황의 `실종신고 현황` 더보기 링크를 새 실종신고 관리 화면으로 연결했다.
- 현재는 전용 실종신고 테이블이 없으므로 관리대상 현재 상태(`찾는중`)와 최근 광고 이력을 조합해 화면을 구성한다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과.

### 산출물
- `deliverables/ADMIN_MISSING_REPORT_MANAGEMENT.md`
- 발표용 이미지 프롬프트 아카이브 갱신.

### 반영시간
- 메뉴 연결, 데이터 조회, 그리드 UI, CSS, 문서화, 빌드 검증: 약 30분.

## 2026-06-25 - QR 위치공유 및 관리자 위치공유 관리

### 요구내용
- QR 공개 페이지에서 위치공유 버튼을 제공한다.
- 사용자가 위치공유를 누르면 휴대폰/브라우저 위치권한을 요청하고, 획득한 위치를 저장한다.
- 보호자에게 관리대상자가 어느 위치에서 발견되었다는 푸시 알림을 보낸다.
- 알림에는 카카오 또는 네이버 지도 위치 링크가 포함되면 좋다.
- 관리자페이지에 위치공유관리 메뉴를 추가하고 공유 이력을 그리드로 조회한다.

### 반영내용
- QR 공개 페이지에 `위치공유` 버튼과 선택 입력값 `발견자 연락처`, `위치 설명`을 추가했다.
- 브라우저 Geolocation API로 위도, 경도, 정확도를 받아 서버 API로 전송한다.
- 서버에서 QR 활성/보호자 활성화/활성 이용권 조건을 다시 검증한 뒤 `location_shares` 테이블에 저장한다.
- 저장된 위치로 카카오 지도 링크와 네이버 지도 링크를 생성한다.
- 보호자 푸시 알림과 알림 이력에 지도 링크를 포함한다.
- 관리자 왼쪽 메뉴에 `위치공유 관리`를 추가했다.
- 관리자 화면은 공유일시, 대상자, 보호자, 발견자 연락처, 주소, 위도, 경도 그리드와 오른쪽 상세 패널로 구성했다.
- 상세 패널에는 지도 미리보기, 지도 링크, 좌표, 정확도, 대상자/보호자 바로가기, 안심번호, 발견자 연락처를 표시한다.
- 보호자 원본 전화번호는 위치공유 이력에 저장하지 않고 안심번호 스냅샷만 저장하도록 했다.

### 검증결과
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- 프로덕션 빌드 성공.
- 로컬 Next dev 서버 `3010` 포트에서 `/`, `/admin?section=locations` HTTP 200 확인.
- 신규 API `POST /api/find/__codex_missing_key__/location`가 없는 QR에 대해 HTTP 400과 `등록되지 않은 QR입니다.` 메시지를 반환하는 것 확인.
- 현재 환경에는 `agent-browser` CLI가 없어 브라우저 자동 시각 검증은 수행하지 못함.
- 실제 휴대폰 위치권한 허용/거부 흐름은 배포 후 실기기 브라우저에서 추가 확인 필요.

### 산출물
- `deliverables/LOCATION_SHARE_MANAGEMENT.md`
- `deliverables/DATABASE_SCHEMA.md` 갱신
- `deliverables/image_prompts/IMAGE_PROMPTS.md` 갱신

### 반영시간
- 위치공유 API, DB, 푸시, 관리자 그리드, 공개 QR 버튼, CSS, 문서화, 빌드 검증: 약 40분.

## 2026-06-25 - 위치공유 알림 지도 링크 클릭 개선

### 요구내용
- 네이버지도 공유링크가 알림메시지로 오지만 클릭되지 않는 문제를 개선한다.

### 반영내용
- 위치공유 푸시 알림의 클릭 대상 URL을 네이버 지도 우선, 카카오 지도 대체로 변경했다.
- 앱 안의 종 알림 목록에서 본문에 포함된 `https://...` URL을 자동으로 클릭 가능한 링크로 바꾸었다.
- 알림 항목에 `지도 열기` 버튼을 추가했다.
- 스와이프 삭제 기능이 알림 링크 클릭을 방해하지 않도록 링크 이벤트 전파를 막았다.
- 서비스워커가 외부 지도 URL을 기존 앱 화면에 억지로 이동시키지 않고 새 창으로 열도록 수정했다.
- 서비스워커 캐시 버전을 `zezari-v15`로 올렸다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).

### 반영시간
- 원인 분석, 푸시 URL 수정, 종 알림 링크 렌더링, 서비스워커 클릭 처리, 문서화, 빌드 검증: 약 20분.

## 2026-06-26 - 위치공유 알림 카카오맵 우선 전환

### 요구내용
- 실제 휴대폰 위치공유에서는 카카오맵이 정확하게 위치를 표시했다.
- 위치공유 푸시 알림 클릭 링크를 네이버 지도 우선에서 카카오맵 우선으로 변경한다.
- 종 알림의 지도 링크 명칭도 카카오맵 기준으로 변경한다.

### 반영내용
- 위치공유 푸시 알림의 클릭 대상 URL을 `카카오맵 -> 네이버 지도 -> 앱 홈` 순서로 변경했다.
- 알림 이력 본문은 카카오 링크가 있을 때 `카카오맵 링크`로 표시한다.
- 앱 안의 종 알림 목록에서 `map.kakao.com` 링크 라벨을 `카카오맵`으로 표시한다.
- 지도 버튼은 카카오맵 URL일 때 `카카오맵 열기`로 표시한다.
- 네이버 지도 링크와 DB 컬럼은 관리자 확인 및 fallback 용도로 유지한다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).

### 반영시간
- 푸시 링크 우선순위 변경, 알림 라벨 변경, 문서화, 빌드 검증: 약 15분.

## 2026-06-26 - 관리자 결제관리 및 엑셀 다운로드

### 요구내용
- 관리자페이지에 결제관리 메뉴를 만들고 결제번호, 보호자, 대상자, 구분, 결제수단, 결제금액, 결제일을 그리드로 조회한다.
- 관리자페이지에 있는 각 메뉴의 그리드에 엑셀다운로드 기능을 추가한다.

### 반영내용
- 결제관리 화면에 결제내역 그리드를 추가했다.
- 결제내역은 상품 결제, 이용권 결제, 광고 금액 이력을 통합 조회한다.
- 결제 구분 필터를 `전체`, `이용권`, `상품`, `광고`로 구성했다.
- 통합 검색은 결제번호, 보호자, 전화번호, 이메일, 대상자, 구분, 결제수단 기준으로 동작한다.
- 이용권 옵션 가격 설정은 결제내역 아래에 유지했다.
- 관리자 화면의 주요 목록에 엑셀 호환 CSV 다운로드 버튼을 추가했다.
- 다운로드 대상은 보호자, 관리대상자, QR, 관리자 사용자, 상품, 주문/배송, 광고, 실종신고, 위치공유, 고객문의, 결제내역이다.
- 다운로드 파일은 화면에서 현재 조회된 데이터 기준으로 생성된다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).

### 산출물
- `deliverables/ADMIN_PAYMENT_EXPORT_MANAGEMENT.md`

### 반영시간
- 결제조회 SQL, 관리자 그리드, CSV 다운로드 공통 버튼, 각 그리드 데이터 매핑, CSS, 문서화, 빌드 검증: 약 45분.

## 2026-06-28 - 관리자 왼쪽 메뉴 스타일 개선

### 요구내용
- 관리자페이지의 왼쪽 관리메뉴를 첨부한 참고 이미지처럼 진한 세로 메뉴 스타일로 변경한다.
- 기존 숨김 기능은 유지한다.
- 로그아웃 버튼과 사용자화면 버튼을 왼쪽 관리메뉴 안에 넣는다.

### 반영내용
- 관리자 사이드바 상단을 `제자리 관리자` 브랜드와 방패 아이콘으로 구성했다.
- 메뉴 배경을 진한 네이비로 변경하고, 메뉴 텍스트와 아이콘을 흰색 계열로 정리했다.
- 선택된 메뉴는 보라색 배경과 연한 테두리로 강조되도록 변경했다.
- 각 메뉴에 선형 아이콘을 추가했다.
- 메뉴 순서를 운영 메뉴 흐름에 맞게 정리했다.
- `사용자 화면`, `로그아웃`을 사이드바 하단 빠른 작업 영역으로 이동했다.
- 본문 헤더 오른쪽에 있던 중복 버튼은 제거했다.
- 기존 메뉴 숨김/펼치기 버튼과 접힘 상태 저장 방식은 유지했다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/ADMIN_SIDEBAR_NAVIGATION.md`
- `deliverables/image_prompts/IMAGE_PROMPTS.md` 갱신

### 반영시간
- 사이드바 구조 변경, 아이콘 추가, CSS 개선, 중복 버튼 제거, 문서화, 빌드 검증: 약 25분.

## 2026-06-28 - 관리자 운영형 대시보드 개편

### 요구내용
- 관리자 대시보드를 첨부파일과 같이 변경한다.
- 스타일뿐 아니라 기능적으로도 현황, 그래프, 최근현황, 주문/광고/구독/매출 현황을 한 화면에 확인할 수 있게 한다.

### 반영내용
- 상단 현황 카드를 5개로 재구성했다.
  - 전체 보호자/전체 대상자
  - 신규 보호자/신규 대상자
  - 활성 QR/미활성 QR
  - 광고 진행중/광고 심사반려
  - 일 매출/월 매출
- 최근 30일 운영 추이 그래프를 추가했다.
  - 매출
  - 주문
  - 위치공유/광고 활동
- 최근 현황 영역을 표 형태로 변경했다.
  - 신규 가입자
  - 신규 주문
  - 최근 활동 알림
  - 주의 대상 현황
- 하단 운영 영역을 추가했다.
  - 주문 현황 흐름
  - 최근 실종 광고
  - 광고 진행 현황 도넛 그래프
  - 구독 현황 도넛 그래프
  - 매출 현황 표
- DB 집계 함수에 전일 대비, 전월 대비, 광고매출, 환불금액, 위험대상 카운트, 최근 30일 추이 데이터를 추가했다.

### 검증결과
- 프로덕션 빌드 성공.
- 직접 Node import 검증은 Next.js의 확장자 없는 ESM import 해석 차이로 수행하지 못했다.

### 산출물
- `deliverables/ADMIN_DASHBOARD_OPERATIONS_REVAMP.md`
- `deliverables/image_prompts/IMAGE_PROMPTS.md` 갱신

### 반영시간
- 데이터 집계 확장, 화면 구성, 그래프/표/도넛 스타일, 문서화, 빌드 검증: 약 60분.

## 2026-06-28 - 관리자 대시보드 최근 활동 알림 표시 개선

### 요구내용
- 최근 활동 알림의 내용이 긴 URL과 문장 때문에 표가 과도하게 세로로 늘어나는 문제를 개선한다.
- 내용은 10글자까지만 보여주고 이후는 `...`으로 표시한다.

### 반영내용
- 관리자 대시보드 `최근 활동 알림` 표의 `내용` 컬럼에 10글자 말줄임 처리를 적용했다.
- 원문 내용은 마우스 오버 시 확인할 수 있도록 툴팁으로 유지했다.
- 긴 텍스트가 표 셀의 높이를 늘리지 않도록 CSS를 보강했다.

### 검증결과
- 프로덕션 빌드 성공.
- 운영 배포 성공.
- `https://zezari.vercel.app`, `https://zezari.vercel.app/admin` 응답 200 확인.
- Vercel 오류 로그 확인 결과 신규 오류 없음.

### 반영시간
- 화면 표시 로직 및 CSS 보정, 로그 기록: 약 10분.

## 2026-06-29 - 관리자 보호자관리 화면 운영형 개편

### 요구내용
- 보호자관리 화면을 첨부파일처럼 구성한다.
- 현황 카드, 조회 조건, 보호자 그리드, 상세정보 패널을 한 화면에서 볼 수 있게 한다.

### 반영내용
- 보호자관리 상단에 현황 카드 4개를 추가했다.
  - 전체 보호자
  - 신규 보호자
  - 대상자 등록
  - 대상자 미등록
- 조회 조건을 확장했다.
  - 검색어
  - 상태
  - 구분
  - 가입일 시작/종료
- 보호자 목록을 고밀도 그리드 형태로 재구성했다.
  - 이름, 아이디, 연락처, 생년월일, 보호자상태, 보호자 구분, 가입일, 관리 항목 표시
- 오른쪽 상세정보 패널을 재구성했다.
  - 기본정보
  - 대상자 링크
  - 구독/주문 링크
  - 광고 링크
  - 활동 내역 링크
  - 대상자/구독/결제/광고 요약
  - 관리메모 저장
  - 탈퇴 처리/재활성화
- 보호자 구분은 현재 DB 기준으로 관리자, VIP, 일반으로 계산한다.
- 보호자 성별은 현재 스키마에 없으므로 `-`로 표시한다.
- 보호자 CSV 다운로드 항목을 화면 구성에 맞게 확장했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- 로컬 dev 서버에서 `/admin?section=guardians` 응답 200 확인.
- Playwright 시각 검증은 로컬 브라우저 바이너리가 없어 수행하지 못했다.
- 운영 배포 성공.
- `https://zezari.vercel.app`, `https://zezari.vercel.app/admin?section=guardians` 응답 200 확인.
- Vercel 오류 로그 확인 결과 신규 오류 없음.

### 산출물
- `deliverables/GUARDIAN_ADMIN_OPERATIONS_LAYOUT.md`
- `deliverables/image_prompts/IMAGE_PROMPTS.md` 갱신

### 반영시간
- 데이터 조회 확장, 화면 구성, CSS, 문서화, 로컬 검증: 약 55분.

## 2026-06-29 - 보호자 상세 카드 내부 탭 전환

### 요구내용
- 보호자관리 오른쪽 상세정보 카드 상단 항목은 링크가 아니라 탭이다.
- 탭을 누르면 다른 페이지로 이동하지 않고 같은 카드 안에서 정보가 바뀌어야 한다.
- 첨부파일의 탭별 화면 구성을 반영한다.

### 반영내용
- 오른쪽 상세 카드의 `기본정보`, `대상자`, `구독/주문`, `광고`, `활동 내역`을 카드 내부 탭으로 변경했다.
- 탭 클릭 시 같은 카드 안에서 아래 내용만 바뀌도록 구현했다.
- 탭별 구성:
  - 기본정보: 보호자 기본정보, SNS 로그인, 관리메모, 탈퇴 처리/재활성화
  - 대상자: 관리대상 카드 목록과 상태, 상세보기 링크
  - 구독/주문: 주문 내역, 구독 내역, 결제 내역
  - 광고: 광고명, 상태, 기간, 지역, 예산, 클릭수
  - 활동 내역: 보호자 알림 기반 활동 타임라인
- 탭 화면에 필요한 대상자, 주문, 광고, 알림 데이터를 추가 조회하도록 DB 함수를 보강했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- 로컬 dev 서버에서 `/admin?section=guardians` 응답 200 확인.
- 운영 배포 성공.
- `https://zezari.vercel.app`, `https://zezari.vercel.app/admin?section=guardians` 응답 200 확인.
- Vercel 오류 로그 확인 결과 신규 오류 없음.

### 반영시간
- 상세 카드 탭 전환, 조회 데이터 보강, CSS, 문서화, 로컬 검증: 약 35분.

## 2026-06-29 - 보호자 상세 탭 내부 스크롤 적용

### 요구내용
- 보호자관리 상세 카드의 `구독/주문`, `활동 내역` 탭은 데이터가 많아질수록 화면 전체가 길어질 수 있다.
- 해당 탭 내부에 스크롤바를 만들어 고정된 크기 안에서 데이터를 확인할 수 있게 한다.

### 반영내용
- `구독/주문` 탭에 고정 높이 내부 스크롤을 적용했다.
- `활동 내역` 탭에 고정 높이 내부 스크롤을 적용했다.
- 스크롤 영역은 상세 카드 안에서만 움직이도록 처리했다.
- 모바일 화면에서는 더 작은 높이로 조정되도록 반응형 CSS를 추가했다.
- 보호자관리 산출물에 긴 데이터 탭의 내부 스크롤 기준을 기록했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).

### 반영시간
- CSS 안정화, 문서화, 로컬 검증: 약 10분.

## 2026-06-29 - 관리자 대상자관리 운영형 개편

### 요구내용
- 관리자페이지의 `대상자관리` 메뉴를 첨부파일처럼 구성한다.
- 오른쪽 상세 카드는 보호자관리처럼 내부 탭으로 구성한다.
- 정보가 많아지는 탭은 내부 스크롤바로 고정 크기 안에서 조회할 수 있게 한다.
- 보호자관리 상세 카드도 모든 탭에 내부 스크롤을 적용한다.

### 반영내용
- 대상자관리 상단에 현황 카드 8개를 추가했다.
  - 전체 대상자
  - 신규 대상자
  - 안전
  - 상품구매필요
  - QR미활성화
  - 찾는 중
  - 비활성
  - 삭제
- 조회 조건을 확장했다.
  - 검색어
  - 대상자 상태
  - QR 상태
  - 구독 상태
  - 등록일 시작/종료
- 대상자 목록을 고밀도 그리드로 변경했다.
  - 대상자 번호, 대상자, 보호자, 대상자상태, QR 상태, 구독 상태, 등록일, 관리
- 오른쪽 상세 카드에 내부 탭을 구성했다.
  - 기본 정보
  - 보호자
  - QR
  - 광고
  - 구독/주문
  - 활동 내역
- 선택 대상자의 QR 이미지를 생성해 QR 탭에 표시한다.
- 선택 대상자의 주문, 광고, 보호자 구독, 활동 이력을 상세 탭에서 조회한다.
- 보호자관리와 대상자관리의 상세 카드 모든 탭에 고정 높이 내부 스크롤을 적용했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- 실제 Turso 대상자 목록 조인 SQL이 정상 조회되는 것을 확인했다.

### 산출물
- `deliverables/ADMIN_GUARDIAN_SUBJECT_MANAGEMENT.md` 갱신

### 반영시간
- 데이터 조회 확장, 대상자관리 UI 개편, CSS, 문서화, 로컬 검증: 약 50분.

## 2026-06-29 - 관리자 QR관리 운영형 개편

### 요구내용
- 관리자페이지의 `QR관리` 메뉴를 첨부파일처럼 구성한다.
- 오른쪽 상세정보 카드는 탭 구조를 고려해 구성한다.
- 대상자관리 상세 QR 탭의 긴 QR 번호가 세로로 쪼개지지 않도록 가로 스크롤을 적용한다.
- QR관리 상세정보 카드에도 가로/세로 스크롤을 적용한다.

### 반영내용
- QR관리 상단에 현황 카드 4개를 추가했다.
  - 전체 QR
  - 사용중 QR
  - 미사용
  - 폐기
- 조회 조건을 확장했다.
  - 검색어
  - QR상태
  - 활성화 상태
  - 활성화일 시작/종료
- QR 목록을 고밀도 그리드로 변경했다.
  - QR 썸네일, QR 번호, 대상자명, 보호자명, 상태, 활성화상태, 생성일, 활성화일, 만료일, 관리
- 오른쪽 상세 카드에 내부 탭을 구성했다.
  - 기본 정보
  - 이력 정보
  - 관리
- QR 라이프사이클 상태를 추가했다.
  - 사용중
  - 미사용
  - 폐기
- QR 메모 저장, QR 폐기, 폐기 QR 복구 기능을 추가했다.
- 대상자관리 상세 QR 탭과 QR관리 상세 카드의 긴 텍스트에 가로 스크롤을 적용했다.
- QR관리 상세 탭은 고정 높이 안에서 가로/세로 스크롤이 가능하도록 구성했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- Turso `qr_codes` 테이블에 QR 상태/폐기일/관리메모 컬럼을 추가했다.
- 실제 Turso QR 목록 조인 SQL이 정상 조회되는 것을 확인했다.

### 산출물
- `deliverables/QR_MANAGEMENT.md` 갱신

### 반영시간
- QR 상태 모델, 화면 개편, CSS, 문서화, 로컬 검증: 약 60분.

## 2026-06-29 - 관리자 주문관리 배송정보 화면 개선

### 요구내용
- 관리자페이지의 `주문관리` 메뉴를 첨부파일처럼 구성한다.
- 상세카드 아래쪽에 배송정보 입력 영역을 두고 관리자가 택배사와 송장번호를 저장할 수 있게 한다.
- 구매자는 마이페이지 주문내역에서 송장번호를 확인할 수 있어야 한다.

### 반영내용
- 주문관리 상단에 6개 현황 카드를 구성했다.
  - 전체 주문
  - 결제 완료
  - 배송중
  - 배송완료
  - 취소
  - 환불
- 검색 조건을 확장했다.
  - 검색어
  - 상품명
  - 주문상태
  - 배송상태
  - 주문기간
- 주문 목록을 고밀도 그리드로 변경했다.
  - 주문번호, 주문일시, 대상자/보호자, 연락처, 상품, 수량, 금액, 결제방법, 주문상태, 배송상태, 관리
- 오른쪽 상세정보 카드를 주문 기본 정보, 대상자 정보, 주문 상품, 결제정보, 배송정보, 관리자 배송정보 입력 순서로 구성했다.
- 관리자 배송정보 입력에서 배송상태, 택배사, 송장번호, 관리자 메모를 저장한다.
- 사용자 마이페이지 `결제 및 이용권 현황`의 최근 결제 내역에 배송조회 정보 블록을 추가했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- 주문 조회 필터와 사용자 배송조회 정보 블록이 소스에 반영된 것을 확인했다.
- Vercel 운영 배포 성공.
- `https://zezari.vercel.app` 운영 alias가 새 배포를 바라보는 것을 확인했다.
- 운영 URL HTTP 확인 결과:
  - `/` 200
  - `/admin?section=orders` 200
  - `/account/billing` 200
- 최근 Vercel 로그에서 오류 항목은 확인되지 않았다.

### 산출물
- `deliverables/ADMIN_ORDER_GRID_MANAGEMENT.md` 갱신

### 반영시간
- 주문관리 화면 개편, 주문 조회 필터 확장, 사용자 송장정보 표시, CSS, 문서화: 약 45분.

## 2026-06-29 - 관리자 구독관리 화면 개편

### 요구내용
- 관리자페이지의 `구독관리` 메뉴를 첨부파일처럼 다시 구성한다.
- 오른쪽 상세정보 카드는 2개 탭으로 구성한다.
- 상세정보 카드와 긴 데이터는 세로/가로 스크롤을 고려해서 만든다.

### 반영내용
- 관리자 왼쪽 메뉴에 `구독 관리`를 추가했다.
- 구독관리 상단에 4개 현황 카드를 구성했다.
  - 전체 구독
  - 구독중
  - 일시정지
  - 취소 / 환불
- 조회 조건을 추가했다.
  - 검색어
  - 구독상품
  - 구독상태
  - 결제상태
  - 구독기간
- 구독 목록을 고밀도 그리드로 변경했다.
  - 구독번호, 대상자(보호자), 구독상품, 구독기간, 다음결제일, 금액, 구독상태, 결제상태, 관리
- 현재 조회 결과를 CSV로 내려받을 수 있게 했다.
- 오른쪽 상세정보 카드를 내부 탭으로 구성했다.
  - 기본정보: 대상자정보, 구독정보, 관리자 메모
  - 결제내역: 최근 구독 결제/주문 내역
- 관리자 메모를 `subscriptions.admin_memo` 컬럼에 저장하도록 했다.
- 구독 목록, 상세카드, 탭 패널, 긴 구독번호/결제번호에 가로/세로 스크롤 처리를 적용했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- Turso `subscriptions.admin_memo` 컬럼 확인 및 추가 완료.
- Turso 구독 요약/구독 주문 조인 SQL이 정상 조회되는 것을 확인했다.

### 산출물
- `deliverables/ADMIN_SUBSCRIPTION_MANAGEMENT.md` 생성
- `deliverables/image_prompts/IMAGE_PROMPTS.md`에 구독관리 발표용 이미지 생성 프롬프트 추가

### 반영시간
- 구독관리 화면 개편, 조회/상세 데이터 구성, 스크롤 CSS, 문서화, 로컬 검증: 약 55분.

## 2026-06-29 - 관리자 광고관리 상세카드 3탭 개편

### 요구내용
- 관리자페이지의 `광고관리` 메뉴를 첨부파일처럼 다시 구성한다.
- 오른쪽 상세정보 카드는 총 3개 탭으로 구성한다.

### 반영내용
- 광고관리 화면을 운영형 구성으로 재정리했다.
  - 현황 카드
  - 조회 조건
  - 광고 그리드
  - 오른쪽 상세정보 카드
- 상단 현황 카드를 추가했다.
  - 진행 중
  - 심사 대기
  - 심사반려
  - 중단
  - 만료
  - 월 광고비
  - 년 광고비
- 조회 조건을 확장했다.
  - 검색어
  - 광고 상태
  - 심사 상태
  - 광고 지역
  - 광고 기간
- 광고 목록 그리드 컬럼을 확장했다.
  - 선택, 광고 번호, 대상자, 광고상태, 심사상태, 광고지역, 광고기간, 일예산, 총예산, 광고비(%), 활성화 일시, 관리
- 오른쪽 상세정보 카드를 내부 탭으로 구성했다.
  - 기본 정보: 광고상태, 심사상태, 예산, 광고지역, 광고기간, Meta 정보, 광고 소재, 메모
  - 광고 성과: 도달수, 노출수, 클릭수, 클릭비용, CTR, 지출금액, 연락 수
  - 지출 내역: 지출일시, 광고기간, 광고지역, 광고금액, 결제수단, 영수증 링크
- 향후 Meta 광고 API 연동을 위해 광고 성과 컬럼을 추가했다.
  - 노출수
  - 도달수
  - 연락수
  - 지출금액
  - 관리자 메모
- 광고 CSV 다운로드에 심사상태, 예산, 소진율, 활성화일시를 포함했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- Turso `subject_ads` 컬럼 확인 및 추가 완료.
- Turso 광고 요약/보호자/대상자 조인 SQL이 정상 조회되는 것을 확인했다.

### 산출물
- `deliverables/ADMIN_AD_GRID_MANAGEMENT.md` 갱신
- `deliverables/image_prompts/IMAGE_PROMPTS.md`에 광고관리 3탭 화면 프롬프트 추가

### 반영시간
- 광고관리 화면 개편, 조회/상세 데이터 구성, 스크롤 CSS, 문서화, 로컬 검증: 약 60분.

## 2026-06-29 - 관리자 결제관리 상세카드 및 취소/환불 팝업 개편

### 요구내용
- 관리자페이지의 `결제관리` 화면을 첨부파일처럼 다시 구성한다.
- 오른쪽 상세정보 카드의 `취소/환불` 버튼은 팝업으로 구성한다.

### 반영내용
- 결제관리 화면을 운영형 구성으로 재정리했다.
  - 현황 카드
  - 조회 조건
  - 결제 그리드
  - 오른쪽 상세정보 카드
  - 취소/환불 팝업
- 상단 현황 카드를 추가했다.
  - 총 매출액
  - 주문 매출
  - 구독 매출
  - 광고 매출
  - 취소/환불 비용
- 조회 조건을 확장했다.
  - 검색어
  - 거래구분
  - 결제상태
  - 기간
- 결제 목록 그리드 컬럼을 첨부 기준으로 변경했다.
  - 선택, 주문/구독/광고 번호, 거래구분, 대상자(보호자), 거래일시, 금액, 결제상태, 관리
- 오른쪽 상세정보 카드에 결제 정보와 대상자정보를 구분해 표시했다.
- `취소/환불` 버튼을 누르면 팝업에서 거래구분, 환불금액, 환불사유를 입력하고 접수할 수 있게 했다.
- `payment_refunds` 테이블을 추가해 관리자 취소/환불 접수 이력을 저장한다.
- 환불금액은 이미 접수된 금액을 포함해 결제금액을 넘지 못하도록 검증한다.
- 현재 구현은 운영 기록과 관련 상태 변경까지 수행하며, 실제 토스 취소/환불 API 호출은 운영 승인 후 별도로 붙일 수 있게 분리했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- Turso `payment_refunds` 테이블 확인 및 생성 완료.
- Turso 주문/구독/광고 결제 요약 SQL이 정상 조회되는 것을 확인했다.

### 산출물
- `deliverables/ADMIN_PAYMENT_EXPORT_MANAGEMENT.md` 갱신
- `deliverables/image_prompts/IMAGE_PROMPTS.md`에 결제관리/환불팝업 프롬프트 추가

### 반영시간
- 결제관리 화면 개편, 취소/환불 팝업, DB/액션 연결, 문서화, 로컬 검증: 약 55분.

## 2026-06-30 - 관리자 쿠폰관리 화면 추가

### 요구내용
- 관리자페이지에 첨부파일과 같은 `쿠폰관리` 메뉴와 화면을 만든다.

### 반영내용
- 관리자 왼쪽 메뉴에 `쿠폰 관리`를 추가했다.
- `/admin?section=coupons` 화면을 추가했다.
- 조회 조건을 구성했다.
  - 검색어
  - 할인 유형
  - 상태
- 쿠폰 목록 그리드를 구성했다.
  - 선택
  - 쿠폰번호
  - 쿠폰 코드
  - 할인 유형
  - 할인 금액
  - 할인 내용
  - 유효 기간
  - 발행 수
  - 사용 수
  - 상태
  - 관리
- 오른쪽 상세카드를 쿠폰 등록/수정 폼으로 구성했다.
  - 쿠폰코드 랜덤생성/직접입력
  - 할인유형
  - 할인금액
  - 할인내용
  - 유효기간
  - 최소/최대 금액
  - 적용 서비스
  - 발행수량
  - 1인당 사용 제한
  - 메모
  - 사용 가능/사용 불가능 상태
- `+ 새 쿠폰` 버튼과 쿠폰 CSV 다운로드를 추가했다.
- `coupons` 원장 테이블을 추가했다.
- 사용자 쿠폰함에서 쿠폰 등록 시 관리자 원장에 있는 쿠폰인지 검증하도록 개선했다.
- 쿠폰 상태, 유효기간, 발행수량을 검증해 사용할 수 없는 쿠폰은 등록되지 않게 했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- Turso `coupons` 테이블 확인 완료.
- Turso `guardian_coupons.coupon_id` 컬럼 확인 완료.

### 산출물
- `deliverables/ADMIN_COUPON_MANAGEMENT.md` 생성
- `deliverables/image_prompts/IMAGE_PROMPTS.md`에 쿠폰관리 발표용 이미지 생성 프롬프트 추가

### 반영시간
- 쿠폰관리 화면, DB 원장, 사용자 쿠폰 검증, 문서화, 로컬 검증: 약 50분.

## 2026-06-30 - 관리자 알림관리 화면 추가 및 쿠폰 라디오 간격 개선

### 요구내용
- 쿠폰관리 상세카드에서 라디오 버튼 옆 글자가 잘리거나 겹치는 현상을 개선한다.
- 관리자페이지에 `알림관리` 메뉴와 화면을 추가한다.
- 가입한 보호자에게 푸시 알림/메시지를 보낼 수 있게 한다.
- `+ 새 메시지` 버튼을 누르면 오른쪽 사이드 팝업에서 메시지를 작성하고 저장/발송할 수 있게 한다.

### 반영내용
- 쿠폰 등록/수정 카드의 라디오 그룹 간격을 넓혀 `랜덤생성`, `직접 입력`, `사용 가능`, `사용 불가능` 문구가 겹치지 않게 했다.
- 관리자 왼쪽 메뉴에 `알림 관리`를 추가했다.
- `/admin?section=notifications` 화면을 추가했다.
- 조회 조건을 구성했다.
  - 발송 채널
  - 발송 상태
  - 검색어
  - 기간
- 알림 목록 그리드를 구성했다.
  - 알림번호
  - 제목
  - 발송 채널
  - 발송 대상
  - 발송 상태
  - 발송 수
  - 성공 수
  - 실패 수
  - 발송 일시
  - 관리
- `+ 새 메시지` 버튼을 추가하고, 오른쪽 사이드 팝업으로 메시지 작성 폼을 띄우도록 했다.
- 작성 폼에는 발송채널, 발송대상, 제목, 내용, 미리보기, 저장, 발송 버튼을 구성했다.
- `admin_messages` 테이블을 추가해 관리자 메시지 원장을 저장한다.
- 발송 시 보호자 알림함에 메시지를 저장하고, 푸시 구독이 있는 보호자에게 브라우저 푸시를 전송한다.
- 그리드 엑셀 다운로드용 CSV 내보내기를 추가했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- Turso `admin_messages` 테이블과 컬럼 확인 완료.
- Turso 알림 메시지 건수 조회 확인 완료.

### 산출물
- `deliverables/ADMIN_NOTIFICATION_MANAGEMENT.md` 생성
- `deliverables/image_prompts/IMAGE_PROMPTS.md`에 알림관리 발표용 이미지 생성 프롬프트 추가

### 반영시간
- 쿠폰 라디오 간격 수정, 알림관리 화면/DB/푸시 발송 액션, 문서화, 로컬 검증: 약 55분.

## 2026-06-30 - 알림관리 카카오톡 채널 및 메시지 템플릿 관리 추가

### 요구내용
- 알림관리에서 발송채널에 `카카오톡`을 추가한다.
- 카카오톡으로도 메시지를 보낼 수 있게 한다.
- 메시지 템플릿을 관리할 수 있는 관리자 메뉴를 추가한다.
- 자동 메시지는 첨부파일처럼 제목과 내용만 수정 가능하게 한다.

### 반영내용
- 알림관리 검색/작성 화면의 발송채널에 `카카오톡`을 추가했다.
- 관리자 메시지 발송 액션이 `푸시 알림`과 `카카오톡` 채널을 구분해서 처리하도록 확장했다.
- 카카오톡 발송은 외부 카카오/Biz 메시지 API 환경변수가 설정되면 호출하도록 구성했다.
- 카카오 설정값이 없으면 발송 실패로 집계하되, 관리자 메시지 원장과 보호자 알림함 기록은 남기도록 했다.
- `message_templates` 테이블을 추가했다.
- 기본 템플릿을 시드했다.
  - QR 미활성화 안내
  - 구독 / 갱신
  - 실종광고 종료
  - 취소/환불
- 관리자 왼쪽 메뉴에 `메시지 템플릿`을 추가했다.
- `/admin?section=message-templates` 화면을 추가했다.
- 템플릿 목록 그리드를 구성했다.
  - 이벤트
  - 설명
  - 발송채널
  - 내용
  - 발송 대상
  - 자동 메시지
  - 관리
- 오른쪽 상세카드에서 템플릿을 등록/수정할 수 있게 했다.
- 자동 잠금 템플릿은 제목과 내용만 수정 가능하게 했다.
- 템플릿 CSV 다운로드를 추가했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- Turso `message_templates` 테이블 확인 완료.
- 기본 메시지 템플릿 4건 확인 완료.
- 로컬 알림관리 화면 HTTP 200 확인.
- 로컬 메시지 템플릿 화면 HTTP 200 확인.

### 산출물
- `deliverables/ADMIN_MESSAGE_TEMPLATE_MANAGEMENT.md` 생성
- `deliverables/ADMIN_NOTIFICATION_MANAGEMENT.md` 갱신
- `deliverables/image_prompts/IMAGE_PROMPTS.md`에 메시지 템플릿 발표용 이미지 생성 프롬프트 추가

### 반영시간
- 카카오톡 채널 기초 연동, 템플릿관리 화면/DB/액션, 문서화, DB/로컬 검증: 약 65분.

## 2026-06-30 - 회원가입 휴대폰 인증번호 실연동

### 요구내용
- 현재 회원가입 휴대폰 인증은 임시 방식이므로 실제 서버 기반 인증번호 흐름으로 바꾼다.
- 구버전 `reference/wp`의 회원가입 인증 흔적을 참고한다.
- 일반 회원가입과 SNS 최초 가입완성 모두 휴대폰 인증을 필수로 적용한다.

### 반영내용
- 구버전 자료 확인 결과, 워드프레스 플러그인 소스는 없고 SQL에 MShop 회원/문자/본인인증 플러그인 사용 흔적이 있음을 확인했다.
- 현재 Next.js/Turso 구조에 맞춰 서버에서 6자리 인증번호를 발급하고 DB에는 해시만 저장하도록 구성했다.
- `phone_verifications` 테이블을 추가했다.
- 인증번호 발송 API와 인증번호 검증 API를 추가했다.
- 인증 성공 시 15분 유효한 1회용 인증 토큰을 발급하도록 했다.
- 회원가입 완료 API는 인증 토큰 없이는 처리되지 않도록 강화했다.
- 일반 회원가입 화면과 SNS 가입완성 화면에서 테스트 인증코드 표시를 제거했다.
- SMS 공급자 환경변수 구조와 개발용 우회 코드 정책을 문서화했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- 로컬 임시 DB에서 인증번호 API 시나리오 확인 완료.
  - 잘못된 전화번호 실패.
  - 개발용 우회 코드 기준 발송 성공.
  - 잘못된 인증번호 실패.
  - 올바른 인증번호 검증 성공.
  - 검증 토큰 기반 회원가입 성공.
  - 토큰 재사용/다른 전화번호 사용 실패.
- GitHub commit: `b7ed5f6 Add server phone verification for signup`
- GitHub push to `origin/main` 완료.
- Vercel 운영 배포: `https://zezari-ql0y3boes-zezari.vercel.app`
- 배포 상태: Ready.
- `https://zezari.vercel.app` 별칭이 이전 배포를 보고 있어 최신 배포로 재연결했다.
- 운영 확인:
  - `https://zezari.vercel.app` HTTP 200 확인.
  - 잘못된 휴대폰 번호로 `POST /api/signup/phone/send` 호출 시 정상 라우트의 검증 오류가 반환됨을 확인.

### 산출물
- `deliverables/AUTH_PHONE_VERIFICATION.md` 생성
- `deliverables/AUTH_SETUP.md` 갱신
- `deliverables/README.md` 갱신

### 반영시간
- 서버 인증 API, DB 스키마, 가입 화면 연결, SMS 어댑터, 문서화, 로컬 검증: 약 55분.

## 2026-06-30 - 회원가입 인증번호 입력칸 정렬 개선

### 요구내용
- 인증번호 입력칸 6개 중 마지막 칸이 다음 줄로 내려가 미관상 좋지 않다.
- 6개 입력칸을 모두 한 줄에 보이게 줄여준다.
- 실제 문자 발송에 필요한 SMS 환경변수는 어느 사이트에서 받아야 하는지 안내한다.

### 반영내용
- 인증번호 입력 영역을 5칸 그리드에서 6칸 그리드로 수정했다.
- 모바일 폭에서도 한 줄 유지가 가능하도록 입력칸 간격, 모서리, 글자 크기를 조정했다.
- 일반 회원가입과 SNS 최초 가입완성 화면 모두 같은 CSS를 사용하므로 함께 반영된다.

### 검증결과
- 프로덕션 빌드 성공.

### 안내내용
- SMS 키는 사용자가 선택한 문자 발송 업체의 콘솔에서 발급받아야 한다.
- 우선 검토할 수 있는 국내 문자 발송 업체는 SOLAPI/CoolSMS, NHN Cloud SMS, Naver Cloud SENS다.

### 반영시간
- 화면 정렬 수정, 빌드 확인, SMS 업체 안내 정리: 약 15분.

## 2026-07-07 - 상품 디자인별 이미지/상세페이지 관리

### 요구내용
- 현재 상품 이미지를 하나 업로드하면 모든 디자인 선택지가 같은 이미지로 표시된다.
- 팔찌 안에서도 강아지, 고양이 등 각 디자인은 서로 다른 상품 디자인으로 관리되어야 한다.
- 관리자가 디자인별 이미지를 따로 업로드하고, 디자인별 상세페이지 이미지도 따로 등록할 수 있어야 한다.
- 사용자가 주문할 때 선택한 디자인이 주문/결제/관리자 조회에 함께 연동되어야 한다.

### 반영내용
- 상품 분류는 기존 `products`로 유지하고, 디자인 단위는 새 `product_designs` 테이블로 분리했다.
- 각 디자인에 선택 이미지, 상세페이지 이미지, 디자인명, 설명, 디자인별 가격, 노출상태, 정렬순서를 저장할 수 있게 했다.
- 주문 테이블에 `design_id`를 추가해 사용자가 선택한 디자인을 주문에 저장한다.
- 관리자 상품관리 화면에서 상품 대표 이미지와 디자인별 이미지를 따로 업로드할 수 있게 구성했다.
- 사용자 상품 주문 화면에서 디자인 선택 버튼은 디자인별 이미지를 보여주고, 다음 단계의 상세 미리보기는 디자인별 상세 이미지를 우선 표시한다.
- 토스 상품 단독구매와 이용권 포함 상품구매 준비 API 모두 `designId`를 함께 저장하도록 변경했다.
- 관리자 주문관리, 결제관리, 구독/주문 관련 조회, 사용자 마이페이지 주문내역에서 디자인명이 함께 보이도록 보강했다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과(Windows 줄바꿈 경고만 표시).
- 일반 Node ESM 직접 DB 검증은 프로젝트 내부 import 해석 방식 차이로 막혔고, Next/Turbopack 빌드 검증을 기준으로 확인했다.

### 산출물
- `deliverables/PRODUCT_DESIGN_CATALOG.md` 생성.
- `deliverables/README.md` 갱신.

### 반영시간
- DB 스키마, 관리자 업로드 UI, 사용자 주문 흐름, 주문 조회 표시, 문서화, 빌드 검증: 약 75분.

## 2026-07-08 - 상품 디자인 추가 방식 및 체크박스 UI 개선

### 요구내용
- 관리자 상품관리 화면에서 `사용자 화면에 노출`, `삭제` 등의 체크박스가 너무 커서 글자가 넘어간다.
- 디자인이 무조건 4개 입력되는 방식이 아니라, 디자인 추가 버튼을 누를 때마다 하나씩 추가되게 한다.
- 디자인 추가 시 썸네일과 상세페이지 이미지를 업로드할 수 있어야 한다.
- 사용자 상품/디자인 선택 화면에는 관리자가 설정한 디자인 개수와 사진이 그대로 반영되어야 한다.

### 반영내용
- 상품관리 폼을 `ProductAdminCatalogForm` 클라이언트 컴포넌트로 분리했다.
- 기존처럼 빈 디자인 행을 항상 보여주지 않고 `+ 디자인 추가` 버튼을 누를 때만 신규 디자인 입력 행이 생기게 했다.
- 신규 디자인 행마다 선택 이미지와 상세페이지 이미지 업로드 입력을 제공한다.
- 상품관리 영역의 체크박스는 전역 input 스타일을 따르지 않도록 16px 크기로 별도 지정했다.
- 체크박스 라벨 텍스트는 줄바꿈되도록 처리해 화면 밖으로 넘치지 않게 했다.
- 과거 기본 시드로 만들어진 빈 디자인 4개는 실제 관리자가 설정한 디자인처럼 보이지 않도록 조회 단계에서 제외했다.
- 사용자 상품 디자인 선택 화면은 관리자에 의해 설정된 활성 디자인만 표시한다.
- 등록된 디자인이 없으면 가짜 디자인 버튼을 표시하지 않고 상품 대표 이미지로 주문 흐름을 진행한다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/PRODUCT_DESIGN_CATALOG.md` 갱신.

### 반영시간
- 관리자 폼 분리, 디자인 추가 버튼, 체크박스 CSS, 디자인 조회 필터, 사용자 디자인 선택 화면 수정, 빌드 검증: 약 45분.

## 2026-07-08 - 상품 디자인 추가 서버 오류 개선

### 요구내용
- 관리자 상품관리에서 디자인을 추가하면 `This page couldn't load` 서버 오류 화면이 표시된다.

### 원인분석
- Vercel 운영 로그에서 `POST /admin` 요청이 `Body exceeded 1 MB limit` 오류로 실패하는 것을 확인했다.
- 새 이미지 업로드 파일과 함께 기존 상품/디자인 이미지 base64 데이터가 hidden input으로 다시 전송되어 Server Action 기본 용량 제한을 초과했다.

### 반영내용
- Server Action 요청 허용량을 `8mb`로 설정했다.
- 기존 상품 대표 이미지와 디자인 이미지를 hidden input으로 전송하지 않도록 제거했다.
- 새 파일을 올리지 않은 경우 서버가 DB에 저장된 기존 이미지를 그대로 유지하도록 저장 로직을 수정했다.
- 이미지 파일 1개당 1MB 제한은 유지했다.
- 상품 디자인 산출물 문서에 업로드 방식과 제한 사항을 기록했다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/PRODUCT_DESIGN_CATALOG.md` 갱신.

### 반영시간
- 운영 로그 확인, 원인 분석, 소스 수정, 문서화, 빌드 검증: 약 25분.

## 2026-07-08 - 상품관리 카드 여백 개선

### 요구내용
- 관리자 상품관리에서 특정 상품에 디자인을 추가하면 팔찌, 목걸이, 키링 등 다른 상품 카드에 큰 여백이 생긴다.
- 하나의 상품 디자인 추가가 다른 상품의 디자인/입력 배치에 영향을 주지 않게 한다.

### 원인분석
- 상품 카드는 CSS grid로 배치되어 같은 행의 가장 큰 카드 높이에 맞춰 늘어났다.
- 카드와 폼 내부 grid도 늘어난 높이를 따라가면서 디자인이 적은 상품의 입력 영역이 아래로 밀려 보였다.

### 반영내용
- 상품관리 그리드의 카드 정렬을 상단 기준으로 변경했다.
- 상품 카드와 상품 입력 폼의 내부 정렬도 상단 기준으로 고정했다.
- 디자인이 많은 상품만 세로로 길어지고, 다른 상품 카드의 내부 입력 위치는 유지되도록 개선했다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/PRODUCT_DESIGN_CATALOG.md` 갱신.

### 반영시간
- 레이아웃 원인 분석, CSS 수정, 문서화, 빌드 검증: 약 15분.

## 2026-07-08 - 사용자 결제 쿠폰 할인 적용

### 요구내용
- 사용자 상품 주문/결제 진행 시 등록한 쿠폰이 있다면 쿠폰을 선택할 수 있게 한다.
- 선택한 쿠폰의 정액/정률 정보만큼 할인한 금액이 실제 결제금액으로 설정되게 한다.

### 반영내용
- 상품 주문정보입력 단계에 `쿠폰 선택` 영역을 추가했다.
- 보호자가 쿠폰함에 등록한 사용가능 쿠폰 중 현재 주문에 적용 가능한 쿠폰만 표시한다.
- 쿠폰 적용 조건은 상태, 유효기간, 최소 주문금액, 적용 서비스 범위를 기준으로 검증한다.
- 정률 쿠폰은 주문금액에 비율을 적용하고, 최대 할인금액이 있으면 상한을 적용한다.
- 정액 쿠폰은 주문금액을 초과해 할인되지 않도록 처리한다.
- 주문 요약에 상품금액, 쿠폰할인, 배송비, 총 결제금액을 표시한다.
- 결제 준비 API에서도 서버 기준으로 쿠폰을 다시 검증하고 최종 결제금액을 계산한다.
- 주문 테이블에 쿠폰 적용 전 금액, 할인금액, 쿠폰 ID/코드/명칭/할인라벨을 저장한다.
- 결제가 성공한 뒤에만 쿠폰을 사용완료 처리하고 쿠폰 사용수를 증가시킨다.
- 100% 할인 등 최종 결제금액이 0원인 경우 Toss 결제위젯 없이 주문완료 처리한다.
- 사용자 마이페이지 결제내역에 쿠폰 할인금액을 표시한다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과.
- 주문 INSERT 컬럼/파라미터 개수 확인.

### 산출물
- `deliverables/ADMIN_COUPON_MANAGEMENT.md` 갱신.

### 반영시간
- 쿠폰 흐름 분석, DB 확장, 결제 화면 UI, 서버 검증/할인 계산, 0원 완료 처리, 문서화, 빌드 검증: 약 70분.

## 2026-07-08 - 쿠폰 적용 후 Toss 약관 위젯 중복 오류 개선

### 요구내용
- 쿠폰 적용 후 결제 화면이 `결제수단 준비중` 상태에 머문다.
- Toss Payments에서 `하나의 약관 위젯만 사용할 수 있어요` 오류가 표시된다.

### 원인분석
- 쿠폰 선택으로 결제금액이 변경될 때 Toss 결제수단 위젯과 약관 위젯을 다시 렌더링하고 있었다.
- Toss 약관 위젯은 한 결제 흐름에서 하나만 사용할 수 있어 중복 렌더링 시 오류가 발생했다.

### 반영내용
- 주문 화면 진입 시 Toss 결제수단/약관 위젯은 한 번만 렌더링하도록 수정했다.
- 쿠폰 선택, 수량 변경, 이용기간 변경으로 금액이 바뀌면 위젯을 다시 만들지 않고 `setAmount`로 결제금액만 갱신한다.
- 주문 단계로 다시 진입할 때 남아 있을 수 있는 Toss 컨테이너 내용을 초기화하도록 보강했다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/ADMIN_COUPON_MANAGEMENT.md` 갱신.

### 반영시간
- 원인 분석, 결제위젯 effect 분리, 컨테이너 초기화, 문서화, 빌드 검증: 약 20분.

## 2026-07-09 - Meta 마케팅 API 환경변수 반영 및 광고 버튼 연동

### 요구내용
- `.env.local`의 `#메타 접근 정보` 아래에 작성한 Meta 정보를 Vercel과 GitHub에 반영한다.
- 관리자페이지 광고관리의 버튼을 Meta 마케팅 API 호출과 연결한다.

### 반영내용
- Vercel 운영/개발 환경에 Meta 관련 키를 반영했다.
  - `META_APP_ID`
  - `META_APP_SECRET`
  - `META_ACCESS_TOKEN`
  - `META_AD_ACCOUNT_ID`
- `META_PAGE_ID`는 값이 없어 반영 대상에서 제외했다.
- GitHub secrets는 현재 PC에 GitHub CLI가 없고 `GITHUB_TOKEN`도 없어 자동 반영하지 못했다.
- 서버 전용 Meta API 연동 파일 `lib/meta-marketing.js`를 추가했다.
- 관리자 광고관리 버튼과 Meta 캠페인 상태를 연결했다.
  - `광고승인`: Meta 캠페인 생성 또는 활성화
  - `광고정지`: Meta 캠페인 일시정지
  - `광고재개`: Meta 캠페인 재활성화
- 보호자 화면의 광고 일시정지/재개/종료도 이미 Meta 캠페인이 연결된 경우 함께 상태가 변경되도록 했다.
- Meta에서 반환되는 캠페인 ID는 `subject_ads.meta_campaign_id`에 저장한다.
- Meta 연동 상태는 `subject_ads.meta_status`에 저장한다.
- 기존 DB 호환을 위해 광고 테이블에 Meta 관련 컬럼 보강 마이그레이션을 추가했다.

### 검증결과
- 프로덕션 빌드 성공.
- Meta 광고계정 조회 테스트 성공.
- 실제 캠페인 생성 테스트는 실제 Meta 캠페인 객체가 만들어질 수 있어 수행하지 않았다.

### 산출물
- `deliverables/ADVERTISING_SETUP.md` 갱신.
- `deliverables/ADMIN_AD_GRID_MANAGEMENT.md` 갱신.

### 반영시간
- 환경변수 확인/반영, API 어댑터 구현, 광고 버튼 연결, DB 보강, 문서화, 빌드 검증: 약 55분.

## 2026-07-09 - 광고승인 Meta Invalid Parameter 오류 개선

### 요구내용
- 관리자 광고관리에서 `광고승인` 버튼을 누르면 `Invalid parameter code=100 type=OAuthException` 오류가 발생한다.

### 원인분석
- Meta Marketing API 호출은 정상적으로 도달하고 있었다.
- 실제 캠페인을 만들지 않는 검증 요청으로 확인한 결과, Meta가 `is_adset_budget_sharing_enabled` 값을 명시하라고 응답했다.
- 현재 1차 연동은 캠페인만 생성하고 광고세트 예산 공유 기능은 사용하지 않으므로 값을 `false`로 지정해야 한다.

### 반영내용
- Meta 캠페인 생성 요청에 `is_adset_budget_sharing_enabled=false`를 추가했다.
- Meta에서 상세 오류 안내를 내려주는 경우 화면에도 더 구체적인 메시지가 보이도록 오류 메시지 처리를 개선했다.
- 광고 연동 산출물 문서를 갱신했다.

### 검증결과
- 프로덕션 빌드 성공.
- Meta 캠페인 생성 검증 요청 성공.
- 실제 캠페인 생성 테스트는 불필요한 Meta 캠페인을 만들 수 있어 수행하지 않았다.

### 산출물
- `deliverables/ADVERTISING_SETUP.md` 갱신.

### 반영시간
- 오류 확인, Meta 검증, 파라미터 수정, 문서화, 빌드 검증: 약 20분.

## 2026-07-10 - 광고지역 지도 선택 기능 반영

### 요구내용
- 관리대상별 광고 신청 시 광고지역을 텍스트로 직접 입력하지 않게 한다.
- 지도에서 광고 중심 위치를 선택하고 Meta API 위치 타겟팅에 사용할 수 있게 한다.

### 반영내용
- 광고 신청 팝업의 `광고지역` 입력칸을 지도 선택 방식으로 변경했다.
- 보호자는 지도에서 위치를 클릭하거나 현재 위치 권한을 허용해 광고 중심점을 설정할 수 있다.
- 광고 반경을 1km, 3km, 5km, 10km, 20km 중 선택할 수 있게 했다.
- 광고 신청 저장 시 지역명, 위도, 경도, 반경, 위치 선택 출처를 DB에 저장한다.
- 서버에서 지도 위치가 선택되지 않으면 광고 신청이 저장되지 않도록 검증한다.
- 관리자 광고 상세카드와 보호자 광고 대시보드에서 반경 정보가 보이도록 했다.
- 향후 Meta 광고세트 생성 시 바로 사용할 수 있도록 `custom_locations` 타겟팅 변환 헬퍼를 추가했다.

### 참고사항
- Meta Targeting Search API는 현재 토큰/앱에서 `API access blocked`가 응답되어 사용자 흐름에 의존하지 않도록 구성했다.
- 현재 Meta 연동은 캠페인 생성/상태변경 단계이며, 지도 좌표는 다음 단계인 광고세트 생성 시 타겟팅 값으로 사용된다.

### 검증결과
- 프로덕션 빌드 성공.
- 공백 검사 통과.

### 산출물
- `deliverables/ADVERTISING_SETUP.md` 갱신.

### 반영시간
- Meta 위치 타겟팅 확인, DB 확장, 지도 UI, 서버 검증, 관리자/사용자 표시, 문서화, 빌드 검증: 약 60분.

## 2026-07-10 - Meta API Access Blocked 승인 오류 처리

### 요구내용
- 사용자가 지도에서 광고지역을 설정한 뒤 관리자가 `광고승인`을 누르면 `API access blocked. code=200 type=OAuthException` 오류가 발생한다.

### 원인분석
- 지도 좌표 저장 문제는 아니며, Meta 캠페인 생성 요청이 Meta 쪽 앱/토큰 권한 상태에서 차단되고 있었다.
- 동일 토큰으로 캠페인 검증 요청을 해도 `API access blocked`가 발생했다.
- Meta API 권한 검수/앱 권한이 완료되기 전까지는 실제 캠페인 생성이 불가능할 수 있다.

### 반영내용
- Meta API가 권한 차단 오류를 반환해도 관리자 내부 승인 흐름은 막히지 않게 했다.
- 관리자 승인/정지/재개 요청은 로컬 DB 상태를 저장하고, Meta 상태는 `Meta 권한 승인 필요`로 표시한다.
- 보호자 광고 일시정지/재개/종료도 같은 방식으로 화면 오류 없이 처리되도록 보강했다.
- Meta 오류 코드와 타입을 내부에서 보존하도록 개선했다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/ADVERTISING_SETUP.md` 갱신.

### 반영시간
- 오류 분리, Meta 검증 확인, 로컬 승인 fallback, 화면 상태 라벨, 문서화, 빌드 검증: 약 30분.

## 2026-07-10 - 광고 미리보기 및 결제 진입 사전작업

### 요구내용
- Meta API 키가 정상 발급되기 전까지 광고 기능의 사전작업을 완료한다.
- 지도와 기간 선택 후 `광고신청저장`이 아니라 `확인` 버튼을 누르게 한다.
- 팝업 안에서 실종자 광고 미리보기 화면으로 넘어간다.
- 미리보기에는 관리대상 사진, 이름, 나이, 성별, 보호자 메시지, QR코드, 관리대상정보 페이지 링크를 표시한다.
- 하단 `결제하기` 버튼으로 결제 페이지로 넘어간다.
- 결제 완료 후에는 같은 구성을 이미지화해 Meta 광고 소재로 등록할 수 있게 한다.

### 반영내용
- 광고 신청 팝업을 `지도/기간 선택`과 `광고 미리보기` 2단계로 분리했다.
- 첫 단계 버튼명을 `확인`으로 변경했다.
- 미리보기 단계에 실종자 찾기 형태의 광고 포스터 UI를 추가했다.
- 관리대상 사진, 이름, 나이, 성별, 보호자 메시지, QR코드, 관리대상정보 링크가 자동 반영되도록 했다.
- `결제하기` 버튼을 누르면 광고 신청 데이터가 `준비중` 상태로 저장되고 `/ads/checkout/[id]` 결제 진입 화면으로 이동한다.
- 결제 진입 화면에는 결제예정금액, 광고지역, 광고반경, 광고기간, 일 단가, 광고 소재 미리보기를 표시한다.
- 실제 Toss 광고 결제 버튼은 외부 결제/API 승인 전까지 비활성 대기 상태로 두었다.
- 추후 이미지 캡쳐/Meta 소재 등록을 위해 광고 소재 DOM에 `data-ad-creative` 식별자를 추가했다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/ADVERTISING_SETUP.md` 갱신.

### 반영시간
- 팝업 단계 분리, 광고 미리보기 UI, 결제 진입 페이지, DB/액션 연결, 문서화, 빌드 검증: 약 45분.

## 2026-07-10 - 광고 미리보기 문구 변경 및 지도 지역검색 추가

### 요구내용
- 광고 미리보기의 안내 문구를 변경한다.
  - 기존: `QR 안심 서비스 / QR코드로 관리대상 정보를 확인해 주세요.`
  - 변경: `발견즉시 연락부탁드립니다 / qr을 스캔하시면 보호자에게 연락할 수 있습니다`
- 광고지역을 지도에서 선택할 때 상단에 지역 검색 기능을 추가한다.
- `논현동`처럼 입력하면 관련 지역 리스트가 나오고, 선택하면 지도가 해당 위치로 이동해야 한다.

### 반영내용
- 광고 미리보기와 광고 결제 진입 화면의 안내 문구를 변경했다.
- 광고지역 지도 선택 영역에 지역 검색 입력창과 검색 버튼을 추가했다.
- 검색 결과 리스트에서 지역을 선택하면 광고 중심 위치가 해당 좌표로 설정되고 지도가 이동한다.
- 서버 API `GET /api/maps/search`를 추가했다.
- 현재는 API 키 없이 사용할 수 있는 OpenStreetMap Nominatim을 서버에서 호출하도록 구성했다.
- 검색 결과는 한국 지역 우선, 한국어 표시 우선으로 반환한다.

### 참고사항
- 현재 검색 API는 사전작업 단계에서 무키 방식으로 구현했다.
- 운영 트래픽이 커지면 같은 API 라우트 내부를 Kakao/Naver/Google Places 같은 상용 검색 API로 교체하면 된다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/ADVERTISING_SETUP.md` 갱신.

### 반영시간
- 문구 변경, 지도 검색 API/UI, CSS, 문서화, 빌드 검증: 약 30분.

## 2026-07-10 - 광고 결제 페이지 구성 및 Toss 결제 연결

### 요구내용
- 광고 결제 페이지를 첨부 화면처럼 구성한다.
- 비용 안내, 예상노출수, 안내사항, 결제수단, 하단 결제 버튼을 보여준다.
- Toss 결제를 실제로 연결한다.

### 반영내용
- `/ads/checkout/[id]`를 모바일 카드형 광고 결제 화면으로 변경했다.
- 비용 안내 영역에 기간 비용, 범위 비용, 총 광고금액을 표시한다.
- 예상노출수 영역을 추가했다.
- 광고 결제 안내사항 영역을 추가했다.
- 결제수단 영역에 Toss Payments 결제위젯을 연결했다.
- 하단 버튼은 `13,000원 결제하기`처럼 실제 광고 금액을 포함해 표시된다.
- 광고 전용 Toss API를 추가했다.
  - `POST /api/payments/toss/ad/prepare`
  - `/payments/toss/ad/success`
  - `/payments/toss/ad/fail`
- Toss 결제 성공 시 서버에서 결제승인을 검증하고 광고 결제정보를 저장한다.
- `subject_ads`에 광고 결제 필드를 추가했다.
  - Toss 주문번호
  - 결제키
  - 결제수단
  - 결제완료일시
- 광고 결제 완료 후에도 광고 상태는 `준비중`으로 유지해 관리자 승인/Meta 등록 흐름과 분리했다.
- 관리자 결제관리에서 광고 결제도 결제대기/결제완료로 구분되도록 했다.

### 검증결과
- 프로덕션 빌드 성공.

### 산출물
- `deliverables/ADVERTISING_SETUP.md` 갱신.
- `deliverables/DATABASE_SCHEMA.md` 갱신.

### 반영시간
- 광고 결제 UI, Toss 결제 API/콜백, DB 스키마, 관리자 결제관리 반영, 문서화, 빌드 검증: 약 75분.

## 2026-07-19 - 로그인 화면 Apple 버튼을 Facebook으로 변경

### 요구내용
- 로그인 화면의 Apple 버튼을 Facebook 버튼으로 교체한다.
- 메타 개발자센터의 Facebook Login 안내를 참고한다.

### 반영내용
- 기존 Apple 아이콘 placeholder를 제거하고 Facebook 아이콘 버튼으로 교체했다.
- Facebook OAuth Provider를 NextAuth에 추가했다.
- 환경변수는 다음 둘 중 하나의 조합을 사용할 수 있게 했다.
  - `FACEBOOK_CLIENT_ID`, `FACEBOOK_CLIENT_SECRET`
  - `META_APP_ID`, `META_APP_SECRET`
- 로그인 화면의 SNS 버튼 순서는 Kakao, Naver, Google, Facebook으로 구성했다.
- Facebook 최초 로그인 사용자는 기존 SNS 회원가입 완료 화면으로 이동해 휴대폰 인증과 기본정보 입력을 진행한다.
- 인증/설정 문서에 Facebook 콜백 URL을 추가했다.

### 참고사항
- 현재 프로젝트는 NextAuth 기반 세션 구조이므로 브라우저 단독 Facebook SDK 로그인 대신 NextAuth OAuth 방식으로 구현했다.
- Meta Developer Center의 Valid OAuth Redirect URIs에 아래 주소가 필요하다.
  - `https://zezari.vercel.app/api/auth/callback/facebook`

### 검증결과
- 프로덕션 빌드 성공.
- 공백 오류 검사 통과.
- Vercel 운영 환경에 `META_APP_ID`, `META_APP_SECRET`이 이미 설정되어 있음을 확인했다.

### 산출물
- `deliverables/AUTH_SETUP.md` 갱신.

### 반영시간
- Facebook 로그인 버튼/Provider 반영 및 문서화: 약 25분.

## 2026-07-19 - Meta 앱 키 운영환경 갱신

### 요구내용
- Meta 개발자센터에서 확인한 값을 `.env.local`에 저장한 뒤 실제 운영환경 설정을 이어서 진행한다.

### 반영내용
- 비밀값을 출력하지 않고 Meta 환경변수의 설정 여부만 검사했다.
- 로컬에 새로 저장된 `META_APP_ID`, `META_APP_SECRET`을 확인했다.
- Vercel Production과 Development 환경의 두 값을 새 값으로 갱신했다.
- 키와 토큰은 GitHub 소스 및 로그에 기록하지 않았다.

### 현재상태
- Facebook 로그인 키는 운영 재배포가 가능한 상태다.
- Meta 광고 API 검증에 필요한 `META_ACCESS_TOKEN`, `META_AD_ACCOUNT_ID`는 로컬 파일에 아직 없다.
- Vercel에는 같은 이름의 암호화 변수가 존재하지만 CLI로 실제 값을 다시 읽을 수 없어 로컬 API 검증에는 사용할 수 없다.

### 검증결과
- Vercel Production/Development 환경변수 갱신 성공.
- 민감정보 출력 및 Git 커밋 없음.

### 반영시간
- 설정 확인 및 Vercel 환경변수 갱신: 약 10분.

## 2026-07-22 - 운영 서비스 전반 기능 점검

### 요구내용
- `https://zezari.vercel.app`에 로그인된 상태를 기준으로 전체 기능을 꼼꼼하게 테스트한다.

### 반영내용
- VS Code에 열린 로그인 탭의 제어 연결을 시도했으나 Codex 제어 브라우저로 노출되지 않는 것을 확인했다.
- 계정 쿠키를 추출하거나 우회하지 않고 다음 운영 검증을 읽기 전용으로 수행했다.
  - 홈/공개 화면과 로그인 공급자
  - Google, Kakao, Naver, Facebook OAuth 시작 주소
  - 잘못된 아이디/비밀번호 거절
  - 비로그인 API 권한 차단
  - 잘못된 입력값 검증
  - 주소 검색과 위경도 반환
  - PWA manifest, 서비스워커, 앱 아이콘
  - Web Push 공개키
  - 운영 빌드와 Vercel 로그
  - Turso 테이블/집계/참조 무결성
  - QR 도메인과 개인정보 비노출
  - 상품 디자인, 주문 금액, 쿠폰 데이터 점검
- 공식 테스트 산출물 `deliverables/PRODUCTION_FUNCTION_TEST_2026-07-22.md`를 생성했다.

### 정상 확인
- 프로덕션 빌드 성공.
- DB 스키마 버전 18 일치, 주요 테이블 22개 모두 존재.
- 고아 데이터, 중복 QR 키, QR 보호자 불일치, 대상자 4명 초과 모두 0건.
- QR 30개의 연결 도메인이 모두 `zezari.vercel.app`으로 저장됨.
- 네 가지 SNS 로그인 모두 각 공급자의 HTTPS 인증 화면으로 정상 시작됨.
- 테스트 과정에서 Vercel 5xx 오류 없음.

### 발견사항
- 운영 SMS 환경변수가 없어 신규 회원의 실제 휴대폰 인증이 완료될 수 없음.
- 카카오 메시지 발송 환경변수가 없어 관리자 카카오 채널 발송은 사용할 수 없음.
- 활성 디자인 17개 중 옵션 이미지 업로드는 2개, 상세페이지 이미지는 0개임.
- 과거 주문 19건은 총 결제금액은 있으나 상품금액/할인금액 분리가 이관되지 않음.
- Next.js 16에서 제거된 `next lint` 명령을 사용해 lint 스크립트가 실행되지 않음.
- 잘못된 QR의 알림 API는 404, 위치 API는 400으로 응답 코드가 다름.

### 제한사항
- 실제 결제, 광고 승인, QR 활성화, 메시지 발송, 저장/수정/삭제는 수행하지 않았다.
- 로그인된 VS Code 탭을 Codex가 제어할 수 없어 화면 버튼과 관리자 메뉴의 실제 클릭 검증은 후속 점검이 필요하다.

### 반영시간
- 운영 HTTP/API, OAuth, DB, 환경설정, 로그, 빌드 점검 및 보고서 작성: 약 45분.

## 2026-07-24 - 날짜 표기 통일 및 관리자 테스트 구독 설정

### 요구내용
- 관리자 구독관리의 복잡한 ISO 기간을 년-월-일로 간단하게 표시한다.
- 다른 화면의 날짜 표기도 같은 기준으로 통일한다.
- 실제 결제 없이 관리자가 구독 상품, 상태, 이용 기간을 임의 설정해 기능을 테스트할 수 있게 한다.

### 반영내용
- 공통 날짜 모듈 `lib/date-format.js`를 추가했다.
- 날짜는 `YYYY-MM-DD`, 일시는 `YYYY-MM-DD HH:mm`으로 통일했다.
- DB UTC 일시는 한국 시간으로 변환하고 생년월일 같은 날짜값은 날짜 이동 없이 표시한다.
- 관리자, 보호자 대시보드, 마이페이지, 광고, 실종신고, 공개 QR, 상품 주문, 알림 화면의 날짜 표시를 같은 방식으로 변경했다.
- 관리자 구독 상세 카드에 `테스트 구독 설정`을 추가했다.
  - 1개월, 3개월, 6개월 상품 선택
  - 이용중, QR 활성화 대기, 일시정지, 기간 만료, 해지 상태 선택
  - 시작일과 종료일 직접 지정
  - 서버 관리자 권한 및 기간 유효성 검증
  - 결제·주문 생성 없이 구독 레코드에만 반영
- 선택한 종료일은 한국 시간 기준 해당 날짜 마지막까지 유효하도록 저장한다.

### 공식 산출물
- `deliverables/ADMIN_SUBSCRIPTION_MANAGEMENT.md` 갱신

### 검증결과
- Next.js 프로덕션 빌드 성공.
- 코드 공백 오류 없음.
- ISO/DB UTC/한국 종료일 날짜 변환 확인.
- 로컬 관리자 구독관리 주소 HTTP 200 확인.
- 제어 가능한 로그인 브라우저가 없어 관리자 저장 버튼의 자동 클릭 검증은 배포 후 수동 확인 항목으로 남겼다.

### 반영시간
- 분석, 구현, 검증, 문서화 및 배포 준비: 약 35분.

## 2026-07-24 - 설치형 앱 알림센터·알림음·아이콘 배지

### 요구내용
- Android 또는 iPhone에 설치한 zezari 앱의 알림센터로 푸시를 수신한다.
- 알림 시 간단한 소리를 재생한다.
- 읽지 않은 메시지 개수를 앱 아이콘에 표시한다.

### 반영내용
- 기존 Web Push를 Android/iOS 설치형 PWA의 OS 알림센터와 연결된 상태로 보강했다.
- 서버에서 읽지 않은 보호자 알림 전체 건수를 계산해 푸시 데이터에 포함한다.
- 백그라운드 푸시 수신 시 앱 아이콘 배지를 실제 미확인 건수로 설정한다.
- 앱 내부 종 알림을 열거나 OS 알림을 누르면 읽음 처리와 배지 제거를 함께 수행한다.
- 밀어서 알림을 삭제하면 같은 OS 알림도 닫도록 연동했다.
- 알림은 `silent: false`로 표시하며 Android 지원 기기에는 짧은 진동 패턴을 요청한다.
- 웹 푸시는 별도 알림 음원 지정이 불가능하므로 휴대폰의 기본 알림음을 사용한다.
- iPhone 일반 브라우저 탭에서는 홈 화면 설치 앱으로 열어야 한다는 안내를 표시한다.
- iOS 설치 앱 식별을 안정화하기 위해 manifest에 앱 ID를 추가했다.

### 지원조건
- Android: 설치형 PWA와 알림 권한 필요, 배지 모양과 숫자 표시는 제조사 런처 설정에 따라 다를 수 있다.
- iPhone/iPad: iOS/iPadOS 16.4 이상, 홈 화면 설치, 설치 아이콘으로 실행, 알림 권한 허용 필요.
- 무음모드와 집중모드에서는 소리가 나지 않을 수 있다.

### 공식 산출물
- `deliverables/PUSH_NOTIFICATION_SETUP.md` 갱신

### 검증결과
- Next.js 프로덕션 빌드 성공.
- 서비스워커 문법 검사 성공.
- 로컬 서비스워커 HTTP 200, 캐시 v16, 배지 및 알림음 허용 코드 확인.
- 비로그인 알림 API가 HTTP 401로 계속 보호되는 것 확인.
- 실제 알림음과 홈 화면 배지는 Android/iPhone 실기기 최종 확인 필요.

### 반영시간
- 플랫폼 조사, 구현, 검증 및 문서화: 약 35분.

## 2026-07-24 - Android 푸시 미수신 장애 개선

### 장애내용
- Android에 설치한 앱에서 관리자 Push 발송 후 알림음과 알림센터 메시지가 모두 나타나지 않았다.

### 운영 진단결과
- Vercel 운영 VAPID 환경변수는 모두 존재했다.
- Turso의 기기 Push 구독은 0건이었다.
- 최근 관리자 Push 발송은 대상 9명, 성공 0명, 실패 9명이었다.
- 알림 메시지는 DB에 저장됐지만 전송할 Android 기기 endpoint가 없었다.

### 원인
- 기존 화면이 실제 기기 구독과 DB 저장 여부를 확인하지 않고 브라우저 알림 권한만 허용되어 있으면 `푸시 알림 켜짐`으로 표시했다.
- 켜짐 상태의 버튼이 비활성화되어 사용자가 기기 연결을 복구할 수도 없었다.

### 개선내용
- 앱을 열 때 권한, 서비스워커, Push 구독, VAPID 키, Turso endpoint 저장을 모두 확인한다.
- 이미 권한을 허용한 사용자도 누락된 기기 구독을 자동 복구한다.
- 연결 전에는 `푸시 알림 연결하기`, 연결 후에는 `테스트 알림 보내기` 버튼을 제공한다.
- 수동 연결 시 실제 테스트 알림을 즉시 발송한다.
- 관리자 발송 성공이 0건이면 완료 메시지 대신 실패 원인을 표시한다.
- 서버에 기기 저장 성공과 Push 공급자 실패 로그를 추가했다.

### 검증결과
- Next.js 빌드 성공.
- VAPID 공개키 설정 확인.
- 테스트 Push API 비로그인 접근 HTTP 401 확인.
- Android 실기기에서는 배포 후 앱 재실행 및 재연결 테스트가 필요하다.

### 반영시간
- 운영 진단, 구현, 검증 및 문서화: 약 35분.

## 2026-07-24 - 위치공유 Push에 발견자 연락처 표시

### 요구내용
- 관리대상 공개 페이지에서 위치를 공유할 때 입력한 발견자 전화번호를 보호자 Push 알림에 먼저 표시한다.
- 전화번호 옆에 공유 위치를 표시한다.

### 반영내용
- 기존 DB에는 발견자 연락처가 정상 저장되고 있음을 확인했다.
- 위치공유 API에서 누락된 `finderContact`를 Push 발송 함수까지 전달했다.
- 알림 본문을 `연락처: 전화번호 · 위치: 위치설명` 순서로 변경했다.
- 전화번호 미입력 시 `연락처 미입력`으로 표시한다.
- 알림 클릭 시 카카오맵으로 이동하는 기존 기능은 유지했다.

### 공식 산출물
- `deliverables/LOCATION_SHARE_MANAGEMENT.md` 갱신

### 검증결과
- Next.js 프로덕션 빌드 성공.
- 위치 저장, 지도 링크, 관리자 위치공유 이력 구조에는 변경 없음.

### 반영시간
- 분석, 구현, 빌드 검증 및 문서화: 약 10분.

## 2026-07-24 - 공개 QR 페이지 보호자 음성 재생

### 요구내용
- 관리대상 공개 페이지에서 보호자가 녹음한 음성을 재생할 수 있는 버튼을 제공한다.
- 심신안정 목적을 알 수 있는 버튼 문구를 사용한다.

### 반영내용
- 기존 브라우저 기본 오디오 컨트롤을 모바일용 전용 버튼으로 변경했다.
- 기본 버튼명은 `보호자 음성 재생(심신안정용)`이다.
- 재생 중에는 `보호자 음성 일시정지`로 변경된다.
- 재생 상태와 음원 오류를 버튼 아래에 표시한다.
- 보호자 음성이 없는 대상에게는 버튼을 표시하지 않는다.
- 활성 QR, 보호자 QR 활성화, 활성 이용권 조건을 그대로 유지했다.

### 공식 산출물
- `deliverables/PUBLIC_GUARDIAN_VOICE_PLAYBACK.md`

### 검증결과
- Next.js 프로덕션 빌드 성공.
- 운영 DB에서 음성이 등록된 활성 대상 한 건을 읽기 전용으로 확인했다.
- 로컬 공개 페이지 HTTP 200 및 음성 재생 버튼 렌더링 확인.

### 반영시간
- 분석, 구현, 실제 데이터 기반 페이지 검증 및 문서화: 약 20분.

## 2026-07-24 - 보호자 음성 버튼 표시 보완

### 확인내용
- 운영 배포는 최신 상태였으나 확인 대상 `매칭테스트`에는 저장된 보호자 음성이 없었다.
- 최초 구현은 저장된 음성이 있을 때만 버튼을 표시해 기능이 누락된 것처럼 보였다.

### 반영내용
- 공개 관리대상 페이지에 보호자 음성 재생 버튼을 항상 표시한다.
- 음성이 없으면 버튼을 비활성화하고 `보호자 음성이 등록되지 않았습니다.`를 안내한다.
- 음성이 등록되면 기존 재생, 일시정지, 상태 및 오류 안내 기능이 활성화된다.

### 반영시간
- 운영 데이터 확인, 표시 방식 개선, 검증 및 배포: 약 15분.

## 2026-07-24 - 상품 디자인 상세 이미지 원본 비율 표시

### 요구내용
- 세로로 긴 상품 상세페이지 이미지를 짧은 고정 프레임에 축소하지 않는다.
- 업로드한 이미지의 가로세로 비율에 따라 상세페이지 높이가 자연스럽게 늘어나게 한다.

### 반영내용
- 상품 및 디자인 선택용 썸네일의 고정 크기는 유지했다.
- 사용자 주문 미리보기의 디자인 상세 이미지는 화면 가로폭에 맞추고 높이를 자동 계산한다.
- 관리자 상품관리의 상세페이지 이미지 미리보기에서도 4:3 고정 프레임을 제거했다.
- 긴 세로 이미지는 잘림이나 세로 압축 없이 전체 내용이 이어서 표시된다.

### 공식 산출물
- `deliverables/PRODUCT_DESIGN_CATALOG.md` 갱신.

### 반영시간
- 분석, CSS 분리, 반응형 검증, 문서화 및 배포: 약 20분.

## 2026-07-24 - QR 비활성 기간 구독 보정

### 요구내용
- 관리자가 대상자별 QR을 활성화·비활성화한다.
- 구독 중 QR이 비활성화된 기간은 구독기간에서 보호한다.
- 1일 이내의 잠깐 비활성화는 구독기간에 반영하지 않는다.

### 반영내용
- 대상자관리 상세 카드의 QR 탭에 상태 변경 버튼을 추가했다.
- 활성 구독 QR의 비활성 시작시각을 QR별로 기록한다.
- 재활성화 시 24시간 이하는 0일, 24시간 초과는 완료 일수만큼 구독 종료일을 연장한다.
- 보호자 단위 구독 상태는 유지해 같은 보호자의 다른 대상자 서비스가 함께 중단되지 않게 했다.
- 보정 시작일시, 누적 보정일, 최근 보정 결과를 관리자 화면과 활동 이력에서 확인한다.
- 중복 클릭으로 같은 비활성 기간이 두 번 반영되지 않도록 상태 조건을 적용했다.

### 공식 산출물
- `deliverables/QR_SUBSCRIPTION_HOLD.md`
- `deliverables/ADMIN_GUARDIAN_SUBJECT_MANAGEMENT.md` 갱신
- `deliverables/QR_MANAGEMENT.md` 갱신

### 검증결과
- 24시간 전후 경계값 5개 시나리오 통과.
- Next.js 프로덕션 빌드 성공.
- Git 공백 오류 검사 성공.

### 반영시간
- 데이터 구조 분석, 구현, 중복 반영 방지, UI, 테스트, 문서화 및 배포: 약 55분.

## 2026-07-24 - 비즈콜 050 안심번호 자동 연동

### 요구내용
- 보호자가 입력한 연락받을 전화번호를 관리대상 공개 페이지에 그대로 노출하지 않는다.
- 비즈콜이 발급한 050 안심번호를 공개하고 실제 보호자 전화로 연결한다.
- 첨부된 비즈콜 HTTPS 연동규격서를 기준으로 구현한다.

### 반영내용
- 비즈콜 규격서 20페이지를 확인해 자동 발급과 기존 번호 재연결 API를 구현했다.
- 일반 회원가입, SNS 최초 가입, 보호자 연락처 수정 시 안심번호를 자동 동기화한다.
- 보호자 정보 화면의 수동 안심번호 입력칸을 자동 발급 상태 표시로 변경했다.
- 관리자 보호자 상세 화면에 `안심번호 발급/재연결` 버튼을 추가했다.
- 공개 QR 페이지에는 발급 상태가 정상인 050 번호만 표시한다.
- 050 번호를 누르면 휴대폰 전화 앱으로 연결된다.
- 비즈콜 설정 누락이나 장애가 발생해도 보호자 실제 번호는 대체 노출하지 않는다.
- 발급 전 또는 실패 상태에는 `안심번호 준비중`을 표시한다.

### 개인정보 보호
- 보호자 실제 전화번호와 050 번호를 DB에서 분리 관리한다.
- API 인증값과 계정 정보는 서버 환경변수로만 관리한다.
- 공개 페이지는 `active` 상태의 안심번호만 조회한다.

### 공식 산출물
- `deliverables/BIZCALL_SAFE_PHONE_INTEGRATION.md`
- `deliverables/DATABASE_SCHEMA.md` 갱신
- `deliverables/README.md` 갱신
- 안심번호 연동 흐름 이미지 생성 프롬프트 누적

### 검증결과
- 비즈콜 규격서 주요 API 페이지 렌더링 및 육안 확인.
- Next.js 프로덕션 빌드 성공.
- 비즈콜 요청 경로, 폼 형식, 전화번호 정규화, MD5/Base64 인증값, 050 표시 형식 모의 검증 통과.
- Turso 스키마 버전 `20` 및 안심번호 상태 컬럼 생성 확인.
- 활성 QR 공개 페이지 HTTP 200, `안심번호 준비중` 표시, 보호자 원 전화번호 미노출 확인.
- 실제 번호 발급 시험은 비즈콜 계약 계정의 API 기본 URL과 Interface ID 등록 후 진행한다.

### 반영시간
- 규격 분석, 개인정보 보호 설계, 구현, 관리자 기능, 빌드 검증 및 문서화: 약 45분.

## 2026-07-24 - 관리자 휴대폰 인증 예외

### 요구내용
- 이미 가입되고 관리자 권한이 있는 계정은 로그인 시 휴대폰 인증번호를 요구하지 않는다.

### 원인
- 해당 계정은 관리자 권한이 활성화되어 있었지만 일반 보호자용 아이디, 비밀번호, 휴대폰 정보가 비어 있었다.
- 기존 화면은 이 값을 기준으로 가입 미완료로 판단해 SNS 가입정보 입력 화면을 표시했다.

### 반영내용
- 로그인 후 관리자 권한을 보호자 가입완료 조건보다 먼저 판단한다.
- 환경변수 관리자와 DB 관리자 모두 휴대폰 인증 및 가입정보 보완 화면을 건너뛴다.
- 일반 보호자의 휴대폰 인증 필수 정책은 유지한다.
- 관리자 예외로 휴대폰 자체가 인증 완료 처리되지는 않는다.
- 비활성 계정 차단 정책은 그대로 유지한다.

### 공식 산출물
- `deliverables/AUTH_PHONE_VERIFICATION.md` 갱신
- 관리자 로그인 분기 이미지 생성 프롬프트 누적

### 반영시간
- 계정 상태 확인, 로그인 분기 수정, 검증, 문서화 및 배포: 약 15분.

## 2026-07-24 - 공개 관리대상 페이지 개인정보 최소화

### 요구내용
- 보호자에게 알리기 버튼과 보호자 이름, 이메일, 주소를 제거한다.
- 안심번호만 보호자 연락정보로 표시한다.
- 발견자 연락처와 위치설명 입력을 제거하고 위치공유 버튼만 둔다.
- 관리대상 사진을 약 1.7배 큰 라운드 사각형 프레임으로 변경한다.

### 반영내용
- 공개 조회 SQL에서도 보호자 이름, 이메일, 주소를 제외해 화면 외 데이터 노출 범위까지 줄였다.
- 안심번호 전화 연결과 보호자 메시지·음성 기능은 유지했다.
- 위치공유는 기기 위치권한 승인 후 위도, 경도, 정확도만 전송한다.
- 위치 알림에서 입력값 삭제로 생기던 `연락처 미입력` 문구도 제거했다.
- 사진은 112px 원형에서 최대 190px, 4:5 비율의 반응형 라운드 사각형으로 변경했다.

### 공식 산출물
- `deliverables/QR_MANAGEMENT.md`
- `deliverables/LOCATION_SHARE_MANAGEMENT.md`
- `deliverables/USER_MANUAL.md`
- 공개 QR 페이지 이미지 생성 프롬프트 누적

### 검증결과
- Next.js 프로덕션 빌드 성공.
- 실제 활성 QR 페이지 HTTP 200 확인.
- 공개 HTML에서 보호자 이름, 이메일, 주소와 삭제 대상 버튼·입력 라벨 미노출 확인.
- 안심번호, 위치공유, 확대 사진 프레임 클래스 출력 확인.
- 브라우저 자동 캡처 세션은 사용할 수 없어 HTML과 반응형 CSS 기준으로 최종 검증했다.

### 반영시간
- 개인정보 검토, 화면·기능 수정, 반응형 스타일, 검증, 문서화 및 배포: 약 25분.

## 2026-07-24 - 광고결제 가격정책 관리

### 요구내용
- 관리자페이지에 광고결제 관리 메뉴를 추가한다.
- 과금 일수와 금액, 기본 광고범위, 초과 범위 단위와 추가금액을 설정한다.
- 사용자는 광고 시작일을 오늘로 고정하고 종료일, 지역, 범위만 선택한다.
- 관리자 설정값으로 계산한 기간금액, 범위금액, 총 결제금액을 안내한다.

### 반영내용
- `광고결제 관리` 전용 메뉴와 실시간 계산 예시가 있는 설정 화면을 추가했다.
- 계산식을 `과금 묶음 수 × (기본 금액 + 초과 범위 단위 수 × 추가 금액)`으로 통일했다.
- 예시 설정에서 2일, 7km 선택 시 기간 2만원과 범위 2만원을 합산해 4만원으로 계산한다.
- 시작일은 한국시간 오늘 날짜를 서버에서 강제하고 최대 365일까지 허용한다.
- 광고 신청, DB 저장, 토스 결제 화면이 동일한 가격 스냅샷과 계산 결과를 사용한다.
- Turso 스키마 버전을 `21`로 갱신했다.

### 공식 산출물
- `deliverables/AD_PRICING_MANAGEMENT.md`
- `deliverables/ADVERTISING_SETUP.md`
- `deliverables/DATABASE_SCHEMA.md`
- 광고결제 설정 흐름 이미지 생성 프롬프트 누적

### 검증결과
- Next.js 프로덕션 빌드 성공.
- 요청 예시 계산 결과 40,000원 확인.
- Turso 설정/광고 스냅샷 컬럼과 기본 정책값 확인.

### 반영시간
- 가격정책 설계, DB 마이그레이션, 관리자·사용자 화면, 결제검증, 테스트, 문서화 및 배포: 약 45분.

## 2026-07-24 - 사용자 개인정보처리방침 페이지

### 요구내용
- 기존 제자리 개인정보처리방침을 참고해 현재 서비스용 페이지를 만든다.
- 사용자 메인 화면 맨 아래에 `개인정보취급방침` 링크를 제공한다.

### 반영내용
- `/privacy` 페이지에 처리 목적, 보유·파기, 공개 QR 정보, 외부 서비스, 위치정보, 이용자 권리, 보안조치 및 문의처를 구성했다.
- 현재 공개 QR 정책에 맞춰 보호자 이름, 이메일, 주소, 원전화번호는 공개하지 않고 050 안심번호만 표시한다고 안내했다.
- 로그인 전 온보딩·로그인 화면과 로그인 후 보호자 대시보드 아래에 공통 정책 링크를 배치했다.
- 모바일 목차와 개인정보 처리표는 화면 폭에 맞춰 재배치하거나 가로 스크롤로 확인할 수 있게 했다.

### 공식 산출물
- `deliverables/PRIVACY_POLICY_PAGE.md`
- 개인정보 처리흐름 이미지 생성 프롬프트 누적

### 검증결과
- Next.js 프로덕션 빌드 성공 및 `/privacy` 정적 경로 생성 확인.
- 로컬 메인과 개인정보처리방침 페이지 HTTP 200 확인.
- 로그인 전 메인 HTML에서 링크 문구와 `/privacy` 주소 확인.
- 데스크톱 1440×1200, 모바일 390×844 화면 확인.
- 정책 페이지와 산출물에 `.env.local` 비밀값이 포함되지 않은 것 확인.
- GitHub `main`에 `4a699dc` 커밋 반영.
- Vercel 프로덕션 배포 `READY` 및 `https://zezari.vercel.app/privacy` HTTP 200 확인.

### 반영시간
- 기존 정책 대조, 현재 구현 분석, 화면·링크·반응형 스타일, 문서화, 테스트 및 배포: 약 25분.

## 2026-07-24 - 관리자 결제패스

### 요구내용
- 관리자에게 상품, 구독, 광고 결제화면의 `결제패스` 버튼을 제공한다.
- 실제 결제 없이 결제 완료 이후 기능과 화면을 테스트한다.

### 반영내용
- 상품 단독구매, 구독 상품, 온라인 광고 결제화면에 관리자 전용 버튼을 추가했다.
- 서버에서 관리자 이메일 및 DB 관리자 권한을 다시 확인한다.
- 화면 이동만 수행하지 않고 기존 결제 완료 함수를 실행해 주문·구독·광고 상태를 실제 테스트 상태로 변경한다.
- 실제 Toss 승인과 과금은 호출하지 않는다.
- 테스트 거래는 `관리자 결제패스`와 `is_test_payment = 1`로 구분한다.
- 기능 및 운영목록에는 반영하되 실제 매출 합계와 추이에서는 제외한다.

### 공식 산출물
- `deliverables/ADMIN_PAYMENT_PASS.md`
- `deliverables/DATABASE_SCHEMA.md`
- `deliverables/USER_MANUAL.md`
- 관리자 결제패스 흐름 이미지 생성 프롬프트 누적

### 검증결과
- Next.js 프로덕션 빌드 성공.
- 비로그인 결제패스 요청은 상품·구독·광고 API 모두 HTTP 401 차단 확인.
- 로그인한 일반회원 요청은 각 API의 서버 관리자 검증에서 HTTP 403으로 차단되도록 구현 확인.
- Turso 스키마 버전 `22`, 테스트 결제 컬럼 2개 및 기존 데이터 기본값 확인.
- 테스트 결제를 제외하는 실제 매출 조회 쿼리 실행 확인.
- GitHub main 및 Vercel 운영 배포 완료.

### 반영시간
- 결제흐름 분석, 권한 검증, 완료처리 연동, 테스트 매출 분리, 화면, 테스트, 문서화 및 배포: 약 40분.

## 2026-07-24 - Meta 실종광고 전체 발행 연동

### 요구내용
- `.env.local`의 Meta 접근정보가 맞는지 확인하고 Vercel에 반영한다.
- 실종신고에서 선택한 광고기간, 지도 위치·반경, 생성된 미리보기 이미지를 Meta API로 발행한다.
- 실제 API 쓰기 테스트를 수행한다.

### 반영내용
- 토큰 유효성, 앱 일치, `ads_management`, 광고계정 활성 상태를 Graph API로 검증했다.
- 값이 있는 Meta 비밀변수 4개를 Vercel Production에 갱신했다.
- 결제 전 실종광고 미리보기를 1080px JPEG로 캡처해 별도 DB 테이블에 저장한다.
- 관리자 광고승인 시 이미지 업로드, 캠페인, 광고세트, 광고소재, 광고를 순차 생성한다.
- 광고세트에는 결제금액을 전체 예산으로, 사용자 선택 날짜와 지도 좌표·반경을 타기팅으로 적용한다.
- 전체 객체 생성이 끝난 뒤에만 활성화하고, 정지·재개도 캠페인·광고세트·광고에 함께 반영한다.
- Meta 객체별 ID, 이미지 해시, 최근 오류, 발행시각을 저장하고 관리자 상세카드에서 확인할 수 있게 했다.
- Facebook 페이지가 없으면 캠페인 생성 전 설정 안내 오류로 차단한다.

### 확인된 운영 조건
- 앱, 토큰, 광고계정 정보는 정상이다.
- 현재 광고계정에 연결된 Facebook 페이지가 없고 `META_PAGE_ID`가 비어 있다.
- 페이지를 광고계정에 연결하고 Page ID를 설정하면 구현된 전체 발행 흐름을 바로 사용할 수 있다.

### 검증결과
- Next.js 프로덕션 빌드 성공.
- Turso 스키마 버전 `23` 적용.
- 테스트 이미지 업로드 성공.
- 3km 지도 타기팅, 기간, 전체예산이 포함된 일시정지 캠페인과 광고세트 생성 성공.
- 테스트 객체와 이미지 즉시 삭제, 실제 광고 과금 없음.
- 페이지 미연결 상태에서는 광고소재·광고 생성을 실행하지 않도록 확인.
- GitHub `main` 커밋 `02dd260` 반영.
- Vercel 프로덕션 배포 `dpl_8Zro5FbdhuCNC7Vv1nzVRevNsogR` `READY` 확인.
- 운영 메인·개인정보처리방침 HTTP 200, 비로그인 광고소재 API HTTP 403 확인.

### 공식 산출물
- `deliverables/META_AD_PUBLISHING.md`
- `deliverables/ADVERTISING_SETUP.md`
- `deliverables/DATABASE_SCHEMA.md`
- Meta 광고 발행구조 이미지 생성 프롬프트 누적

### 반영시간
- Meta 정보·권한 검증, 안전한 쓰기 테스트, 이미지 캡처, DB, 전체 발행 로직, 문서화 및 배포 준비: 약 65분.

## 2026-07-24 - Meta 비즈니스 연결상태 점검

### 요구내용
- 인앱 브라우저에 열린 Meta 비즈니스 설정의 전체 연결상태를 확인한다.

### 확인결과
- 광고계정은 활성 상태이며 통화 KRW, 시간대 Asia/Seoul, 사용자 1명이 연결되어 있다.
- 광고용 Facebook 페이지, 사용자 페이지, Instagram 계정은 모두 0개다.
- 광고계정 결제수단도 설정되지 않은 상태다.
- 현재 토큰에는 광고관리 권한은 있지만 페이지 목록과 비즈니스 관리 권한이 없다.
- `META_PAGE_ID`도 아직 비어 있다.
- 활성 캠페인 1개가 있으나 광고세트와 광고는 0개이고 노출·클릭·지출도 없다.
- 인앱 브라우저 탭이 개발 세션에 연결되지 않아 화면 클릭 대신 Graph API로 상태를 검증했다.

### 다음 설정
- 공식 Facebook 페이지 생성 또는 연결
- 페이지·광고계정 자산 권한 부여
- 광고계정 결제수단 등록
- 토큰 권한 추가 및 `META_PAGE_ID` 설정

### 공식 산출물
- `deliverables/META_AD_PUBLISHING.md` 갱신

### 반영시간
- 브라우저 연결 재확인, API 자산·권한·결제·캠페인 점검 및 문서화: 약 15분.

## 2026-07-24 - Meta 앱 연결 캡처 재확인

### 확인내용
- 캡처에 표시된 연결은 `qr-find-ads 앱 → ZEZARI 광고계정` 연결이다.
- 광고 발행에 필요한 Facebook 페이지 연결 화면은 아니다.
- API 재확인에서도 광고용 페이지, 사용자 페이지, Instagram 계정이 모두 0개이며 `META_PAGE_ID`도 비어 있다.
- 다음 설정 위치는 `계정 > 앱`이 아니라 `계정 > 페이지`다.

### 반영시간
- 화면 판독, API 재확인 및 기록: 약 5분.

## 2026-07-24 - Meta 실제 광고계정·페이지 발행 설정

### 요구내용
- 연결된 인앱 브라우저의 Meta 비즈니스와 개발자센터를 이용해 운영 사이트의 광고 API 발행 설정을 완료한다.

### 반영내용
- 비즈니스 포트폴리오 `제자리zezari`, Facebook 페이지 `제자리`, 실제 광고계정 `ZEZARI`, 앱 `qr-find-ads`를 확인했다.
- 기존 시스템 사용자에 페이지, 광고계정, 앱 권한을 할당했다.
- Meta 앱에 Facebook 로그인을 추가하고 비즈니스 소유 자산용 시스템 사용자 토큰을 새로 생성했다.
- 샌드박스 광고계정 정보를 실제 광고계정으로 교체하고 페이지 ID를 설정했다.
- 로컬과 Vercel Production의 Meta 환경변수를 비밀값 노출 없이 갱신했다.
- Meta 최신 규격에서 폐기된 광고소재 필드를 제거했다.

### 검증결과
- 실제 광고계정은 활성 상태이며 KRW, Asia/Seoul, 결제수단 데이터가 확인됐다.
- 테스트 이미지 업로드, 일시정지 캠페인, 지도 반경이 적용된 일시정지 광고세트, Facebook 페이지 기반 광고소재 생성에 성공했다.
- 최종 광고 생성 요청은 Meta 차별 금지 정책 운영자 동의 한 건만 남아 중단됐다.
- 테스트용 캠페인·광고세트·소재·이미지는 즉시 삭제해 노출과 과금이 발생하지 않았다.
- Next.js 프로덕션 빌드가 성공했다.

### 남은 운영자 확인
- Meta 차별 금지 정책은 비즈니스 운영자가 직접 검토하고 동의해야 한다.
- 동의 후 일시정지 광고 생성 최종 테스트와 운영 배포를 완료한다.

### 공식 산출물
- `deliverables/META_AD_PUBLISHING.md` 갱신

### 반영시간
- Meta 자산 설정, 시스템 사용자 토큰 교체, 실제 계정 검증, 호환성 수정, 안전한 쓰기 테스트, 정리 및 문서화: 약 45분.

## 2026-07-24 - Meta 테스트 광고 실제 게재 확인

### 요구내용
- Meta 차별 금지 정책 동의 후 실제 광고계정에 테스트 광고를 게재하고 게재 여부를 확인한다.

### 테스트 조건
- 실제 광고계정 `ZEZARI`, Facebook 페이지 `제자리` 사용
- 서울 중심 1km 반경, 만 18세 이상
- 총예산 2,000원으로 제한
- 운영 사이트로 연결되는 테스트용 광고소재 사용

### 검증결과
- 이미지, 캠페인, 광고세트, 페이지 기반 광고소재, 광고 생성이 모두 성공했다.
- 광고 활성 요청 후 `검토 중`을 거쳐 API 상태가 `ACTIVE`로 변경됐다.
- Meta 광고 관리자에서 테스트 광고가 `활동 중`으로 표시되는 것을 확인했다.
- 정책 경고, 검수 거절, API 오류가 없었다.
- 실제 게재 확인 직후 불필요한 지출 방지를 위해 광고·광고세트·캠페인을 일시정지했다.
- 정지 시점까지 노출·지출 데이터는 집계되지 않았다.
- 테스트 객체는 Meta 광고 관리자에서 이력 확인이 가능하도록 삭제하지 않고 보존했다.

### 결론
- 관리자 광고승인에서 사용하는 Meta Marketing API 운영 발행 경로가 정상 작동한다.
- 페이지, 실제 광고계정, 결제수단, 시스템 사용자, 정책 동의, 광고소재 규격 관련 차단사항이 모두 해소됐다.

### 공식 산출물
- `deliverables/META_AD_PUBLISHING.md` 갱신

### 반영시간
- 정책 확인, 실제 광고 생성·활성화, 검수상태 추적, 광고 관리자 확인, 안전 정지 및 문서화: 약 15분.

## 2026-07-24 - 사용자 요청 광고 AD-50934FB359 발행 확인

### 요구내용
- 사이트에서 사용자가 신청하고 관리자가 승인한 광고가 Meta에 게재됐는지 확인하는 방법을 안내한다.

### 확인결과
- 서비스 광고 상태는 `active`, Meta 저장 상태는 `ad_active`다.
- 캠페인, 광고세트, 광고소재, 광고 ID가 모두 정상 저장됐다.
- Meta API에서 캠페인·광고세트·광고가 모두 `ACTIVE`로 조회됐다.
- 오류나 검수 반려 내용은 없다.
- 광고 시작 시각은 2026-07-24 22:41:50, 종료 시각은 23:59:59다.
- 총예산은 10,000원, 광고 범위는 지정 좌표 중심 5km다.
- 확인 시점은 시작 전이므로 광고 관리자에는 `예약됨`으로 표시되고 노출·도달·지출 데이터는 아직 없다.

### 운영 확인방법
- 시작 전에는 광고 스위치가 켜져 있고 `예약됨`인지 확인한다.
- 시작 시각 이후에는 게재 상태가 `활동 중`으로 바뀌는지 확인한다.
- 노출수, 도달수, 지출금액이 집계되면 실제 노출까지 진행된 것으로 확정한다.

### 공식 산출물
- `deliverables/META_AD_PUBLISHING.md` 갱신

### 반영시간
- 서비스 DB, Meta API, 광고 관리자 대조 및 문서화: 약 10분.

## 2026-07-24 - 광고 지역 도시명 표시·자동 발행·예산 분리

### 요구내용
- 광고 지역을 GPS 숫자 대신 대략적인 도시명으로 표시한다.
- 보호자가 광고를 결제하면 관리자 승인 없이 Meta 광고 발행을 시작한다.
- 보호자 결제금액은 서비스 매출로 남기고 Meta 집행예산은 지역·반경·기간으로 별도 계산한다.

### 반영내용
- 지도 검색, 지도 클릭, 현재 위치 선택 모두 한국어 도시·구·동 단위 명칭으로 저장한다.
- 위도·경도는 화면 표시용이 아니라 Meta 위치 타겟팅 전용 데이터로 분리했다.
- 수도권, 광역시·세종, 일반지역 가중치와 반경·기간을 조합하는 Meta 예산 계산식을 추가했다.
- 광고 결제 완료 직후 서버가 Meta 발행을 자동으로 시작한다.
- 자동 발행 실패 시 결제는 유지하고 관리자 화면에서 `광고발행 재시도`가 가능하다.
- 사용자 결제화면과 관리자 광고화면에서 `보호자 결제금액`과 `Meta 집행예산`을 구분해 표시한다.
- 기존 광고는 기존 금액을 유지하는 `legacy` 예산으로 보존했다.
- 기존 광고 6건의 좌표형 지역 문구도 도시·구·동 명칭으로 변환하고 타겟팅 좌표는 그대로 보존했다.

### 검증결과
- Next.js 운영 빌드 성공
- `논현동` 검색 결과: `서울특별시 강남구 논현1동`
- 서울 좌표 역조회 결과: `서울특별시 중구 명동`
- 서울 2일·7km Meta 예산 14,400원 계산 확인
- 운영 Turso 스키마 버전 24 및 신규 컬럼 확인
- 운영 DB 광고 7건 모두 Meta 예산 보유, 좌표형 표시 문구 0건 확인
- GitHub `main` 커밋 `e8780f3` 푸시 완료
- Vercel 배포 `dpl_hbiQeiAkWHhjohBNnGKtUr13ML5S` READY 및 `https://zezari.vercel.app` 200 확인

### 공식 산출물
- `deliverables/META_AD_AUTOMATION.md` 생성
- Meta 발행, 광고 가격, DB 스키마 산출물 갱신

### 반영시간
- 분석, 구현, DB 마이그레이션, 검증 및 문서화: 약 35분.

## 2026-07-24 - 모바일 광고 직접 링크 및 보호자 광고 확인

### 요구내용
- 휴대폰에서 광고를 보는 사용자가 같은 화면의 QR을 스캔할 수 없으므로 관리대상 페이지 링크를 광고에 함께 제공한다.
- 보호자가 결제 및 자동 게재된 광고를 광고 대시보드에서 열어볼 수 있게 한다.

### 반영내용
- Meta 광고 본문과 `자세히 보기` CTA가 관리대상 공개 페이지로 연결된다.
- 광고 캡처 이미지의 QR 아래에도 관리대상 페이지 URL을 텍스트로 표시한다.
- Meta가 제공하는 공유 가능한 광고 미리보기 링크와 광고 스토리 ID를 DB에 저장한다.
- 보호자 광고 대시보드에 `관리대상 페이지`, `광고 피드 보기` 버튼을 추가했다.
- 광고 미리보기 링크 조회 실패는 실제 광고 발행을 중단시키지 않도록 분리했다.

### 검증결과
- Next.js 운영 빌드 성공
- 운영 Turso 스키마 버전 25 적용
- 기존 Meta 광고 2건의 공유 링크 역입력 성공
- 신규 광고 생성이나 추가 광고비 지출 없이 기존 데이터로 확인

### 공식 산출물
- `deliverables/META_AD_LINK_ACCESS.md` 생성
- Meta 발행, DB 스키마, 산출물 목록, 이미지 프롬프트 갱신

### 반영시간
- 분석, 구현, 운영 DB 반영, 기존 광고 링크 보강, 검증 및 문서화: 약 25분.

### 배포결과
- GitHub `main` 커밋 `3ea5728` 푸시
- Vercel 운영 배포 `dpl_GYbMdYEbe3JZdBPVwBTSHenPzZ6a` READY
- `https://zezari.vercel.app` 운영 별칭 연결 및 홈·광고 대시보드 경로 HTTP 200 확인

## 2026-07-25 - 관리대상 광고 팝업의 광고 피드 확인

### 요구내용
- 사용자 대시보드에서 관리대상의 진행 중 광고를 열었을 때 일시정지·종료 버튼 옆에 광고 피드 확인 버튼을 제공한다.

### 반영내용
- 최신 광고의 Meta 공유 링크를 관리대상 데이터에 포함했다.
- 공유 링크가 저장된 광고에만 `광고 피드 보기` 버튼을 표시한다.
- 버튼은 새 창에서 Meta 공유 광고 미리보기를 열며, 기존 광고 상태 버튼과 같은 행에서 반응형으로 정렬된다.

### 검증결과
- Next.js 운영 빌드 성공
- 공백 오류 검사 성공
- 기존 운영 광고의 Meta 공유 링크 저장 상태 확인

### 공식 산출물
- `deliverables/META_AD_LINK_ACCESS.md` 및 이미지 프롬프트 갱신

### 반영시간
- 분석, 구현, 검증 및 문서화: 약 10분.

### 배포결과
- GitHub `main` 커밋 `076a658` 푸시
- Vercel 운영 배포 `dpl_CQQj6jK9k4pEAW1b8aNCMGDBPzt7` READY
- 운영 홈 및 대시보드 경로 HTTP 200 확인

## 2026-07-30 - 관리대상 우선 상품·12간지 디자인 선택

### 요구내용
- 상품카드를 먼저 고르는 방식 대신 관리대상, 상품, 디자인을 셀렉트박스로 순서대로 선택한다.
- 팔찌, 목걸이, 키링, 3개 조합상품, 스티커를 제공한다.
- 디자인은 12간지 동물 전체를 선택할 수 있게 한다.

### 반영내용
- 상품구매 첫 화면 상단에 관리대상·상품·디자인 셀렉트박스를 배치했다.
- 팔찌&목걸이, 목걸이&키링, 팔찌&목걸이&키링을 별도 상품으로 추가했다.
- 7개 상품 각각에 12간지 디자인 데이터를 연결했다.
- 선택한 상품·디자인 요약과 수량 조절을 추가하고 기존 미리보기, 배송지, 쿠폰, Toss 결제 흐름을 유지했다.
- 기존 주문과 상품 이미지 데이터는 삭제하지 않았다.

### 검증결과
- Next.js 운영 빌드 성공
- 운영 Turso 스키마 버전 26 적용
- 요청한 상품 7개 존재 확인
- 각 상품의 활성 12간지 디자인 12개 확인

### 공식 산출물
- `deliverables/SHOP_PRODUCT_SELECTION.md` 생성
- 상품 카탈로그, DB 스키마, 산출물 목록, 이미지 프롬프트 갱신

### 반영시간
- 분석, 구현, 운영 DB 반영, 검증 및 문서화: 약 30분.

### 배포결과
- GitHub `main` 커밋 `87c43a7` 푸시
- Vercel 운영 배포 `dpl_GumWVzpyDT6dXpverJ1Lgihznztj` READY
- `https://zezari.vercel.app` 홈 및 `/shop` HTTP 200 확인
- 비로그인 브라우저에서 `/shop` 접근 시 온보딩으로 이동하는 접근 제어 확인

## 2026-07-30 - 상품 구매 시 QR 서비스 계속 이용

### 요구내용
- 상품구매 화면의 이용기간과 상품 단독구매 선택을 없앤다.
- 상품을 구매하면 별도의 이용권 구매 없이 관리대상 QR 페이지를 계속 이용할 수 있게 한다.

### 반영내용
- 상품구매 화면에서 구매방식 탭과 1·3·6개월 기간 선택을 제거했다.
- 선택한 상품·디자인 가격과 수량으로 결제금액을 계산한다.
- 상품 결제, 배송, QR 활성화가 완료되면 만료일 없는 `계속 이용` 서비스 권한이 활성화된다.
- 기존 기간제 구독 데이터는 삭제하지 않고 호환되도록 유지했다.
- 공개 QR 페이지, 위치공유, 마이페이지 결제현황, 관리자 주문·매출 집계를 새 정책에 맞게 변경했다.

### 검증결과
- Next.js 운영 빌드 성공
- 운영 Turso 스키마 버전 27 적용
- 신규 `access_type` 컬럼과 기존 기간제 데이터 보존 확인
- 마이그레이션 후 공개 QR 페이지 HTTP 200 확인

### 공식 산출물
- `deliverables/PRODUCT_INCLUDED_QR_SERVICE.md` 생성
- 상품선택, DB 스키마, 산출물 목록, 이미지 프롬프트 갱신

### 반영시간
- 분석, 구현, DB 반영, 호환성 검증 및 문서화: 약 25분.

### 배포결과
- GitHub `main` 커밋 `10514ab` 푸시
- Vercel 운영 배포 `dpl_8R2gi56J4EdM2xLVMsBfsn5NxkCW` READY
- 운영 홈, 상품구매, 공개 QR 페이지 HTTP 200 확인
- 제거된 구형 단독주문 API 2곳 HTTP 410 확인

## 2026-07-30 - 운영 로그인 통합 브라우저 테스트

### 요구내용
- 로그인된 관리자 계정으로 사용자 화면부터 관리자 전체 메뉴까지 운영 사이트 기능을 꼼꼼하게 점검한다.

### 반영내용
- 사용자·공개 영역 12개와 관리자 메뉴 17개를 실제 운영 브라우저에서 점검했다.
- 사용자 대시보드, 정보입력, 마이페이지, 결제/쿠폰/결제수단, 광고, 상품/쿠폰/Toss 준비, 실종광고 지도, 공개 QR과 개인정보처리방침을 확인했다.
- 관리자 전체 그리드, 필터, 상세 탭, 팝업, 스크롤, 엑셀 버튼, 메뉴 숨김 기능을 확인했다.
- 운영 데이터와 비용이 바뀌는 결제·환불·광고 발행·알림 발송·저장·삭제 작업은 수행하지 않았다.

### 검증결과
- 서버 오류 화면이나 치명적인 화면 실패 없이 주요 조회·전환 흐름이 동작했다.
- 결제 관리와 대시보드 매출 집계 불일치, 일괄 푸시 10명 중 1명 성공, 미활성 QR 비율 100% 초과를 우선 개선 항목으로 확인했다.
- 고객문의 사용자 접수 미구현, 중복 계정 정책, 일부 과거 문구와 상품관리 렌더링 부담을 후속 항목으로 분류했다.
- PWA manifest와 service worker HTTP 200을 확인했다.

### 공식 산출물
- `deliverables/PRODUCTION_AUTHENTICATED_BROWSER_TEST_2026-07-30.md` 생성
- `deliverables/README.md` 갱신

### 반영시간
- 브라우저 점검, 소스 대조, 위험도 분류, 문서화: 약 70분.

### 배포결과
- GitHub `main` 커밋 `727b526` 푸시
- Vercel 운영 배포 `dpl_BbPaYAGxaiPi1Beb9dx31bxPX8K2` READY
- `https://zezari.vercel.app` 별칭 연결 및 홈·개인정보취급방침 HTTP 200 확인

## 2026-07-30 - `zezari.family` 운영 도메인 연결

### 요구내용
- Vercel에서 구매한 `zezari.family`을 프로젝트 운영 URL로 사용한다.
- 기존 `https://zezari.vercel.app` 주소와 이미 제작된 QR도 계속 사용할 수 있게 한다.

### 반영내용
- 도메인 소유권, Vercel 네임서버, CDN, 자동 갱신 SSL 인증서를 확인했다.
- `zezari.family`을 기존 `zezari` 프로젝트에 연결하고 Vercel 도메인 별칭은 유지했다.
- 운영 `NEXTAUTH_URL`, `PUBLIC_APP_URL`과 QR·Meta 광고·지도 API의 기본 URL을 새 도메인으로 변경했다.
- DB의 QR 40개 URL을 `zezari.family/find/{키}`로 변경하되 QR 키와 대상자 매칭은 그대로 보존했다.
- 기존 인쇄 QR은 `zezari.vercel.app` 별칭을 통해 계속 접근할 수 있게 했다.

### 검증결과
- Next.js 운영 빌드 성공
- QR URL 40개 새 도메인 반영, 구 도메인 DB URL 0개 확인
- 두 도메인 병행 구조와 SNS 로그인 콜백 추가 목록 문서화

### 공식 산출물
- `deliverables/CUSTOM_DOMAIN_SETUP.md` 생성
- 산출물 목록 및 이미지 프롬프트 갱신

### 반영시간
- 도메인 확인, 설정, QR 이전, 소스 반영, 검증, 문서화: 약 25분.

### 배포결과
- GitHub `main` 커밋 `3b996c7` 푸시
- Vercel 운영 배포 `dpl_PrQLdYZpbRUYdPWxUrEpyHraihpD` READY
- `zezari.family`과 `zezari.vercel.app`이 동일 배포를 가리키는 상태 확인
- 두 도메인의 홈·공개 QR HTTPS 200, 새 도메인의 개인정보처리방침·PWA manifest·service worker HTTPS 200 확인
- 두 도메인에서 Google·Kakao·Naver·Facebook 인증 공급자와 도메인별 콜백 URL 생성 확인

## 2026-07-31 - Google·Naver 새 도메인 로그인 콜백 설정

### 요구내용
- Google Cloud와 Naver Developers에 `zezari.family` 로그인 콜백을 직접 등록한다.
- 기존 `zezari.vercel.app` 로그인 호환성을 유지한다.

### 반영내용
- Google OAuth 웹 클라이언트에 새 도메인 원본과 콜백을 추가하고 기존 Vercel·로컬 개발 설정을 보존했다.
- Naver 서비스 URL을 `zezari.family`으로 변경하고 새 도메인과 기존 Vercel 도메인 콜백을 함께 등록했다.
- 민감한 클라이언트 비밀값과 토큰은 산출물 및 로그에 기록하지 않았다.

### 검증결과
- Google 계정 선택 화면이 새 도메인 콜백으로 정상 열리고 리디렉션 불일치 오류가 없었다.
- Naver `zezari` 동의 화면이 새 도메인 콜백으로 정상 열리고 콜백 오류가 없었다.
- Naver 앱은 현재 개발 중이므로 등록 멤버는 테스트 가능하며 전체 사용자는 검수 승인 후 이용 가능하다.

### 공식 산출물
- `deliverables/OAUTH_CALLBACK_SETUP.md` 생성
- 커스텀 도메인 산출물, 산출물 목록, 이미지 프롬프트 누적 갱신

### 반영시간
- 콘솔 확인, 설정 저장, 운영 OAuth 진입 테스트, 문서화: 약 15분.

## 2026-07-31 - 관리자 연동형 동적 상품 카탈로그

### 요구내용
- Google 외 SNS 로그인은 추후 보완 목록으로 남긴다.
- 기존 디자인 관리는 유지하고, 구매 selectbox의 상품명·가격·상세페이지를 관리자가 관리한다.
- 세로로 긴 상세페이지 이미지를 원본 비율로 보여준다.

### 반영내용
- Google 완료와 Naver·Kakao·Facebook 후속 작업을 별도 누적 체크리스트로 정리했다.
- 관리자 상품관리에 새 상품 추가 폼과 기존 상품 편집 항목을 구성했다.
- 상품명, 가격, 정렬, 설명, 노출, 대표 이미지, 긴 상세페이지 이미지를 DB로 관리하도록 확장했다.
- 새 상품에는 기존 12간지 디자인 항목을 자동 생성해 디자인 선택 흐름을 유지했다.
- 사용자 구매화면의 고정 상품 목록을 제거하고 활성 상품 전체가 관리자 정렬 순서대로 selectbox에 표시되게 했다.
- 긴 이미지는 상품 목록에 포함하지 않고 선택 시 전용 API에서 불러와 초기 화면 부담을 줄였다.
- 기존 12간지 디자인과 디자인별 이미지·가격·상세자료는 그대로 유지했다.

### 검증결과
- Next.js 운영 빌드 성공
- Turso 상품 상세 이미지 컬럼 2개 생성 확인
- 기존 상품 7개와 활성 상태 보존 확인
- 상세 이미지가 없는 상품의 전용 API가 정상 404를 반환하고 서버 오류가 없음을 확인
- 임시 비노출 상품의 PNG로 상세 이미지 API HTTP 200·이미지 형식·원본 바이트 응답을 확인한 뒤 테스트 데이터를 삭제

### 공식 산출물
- `deliverables/DYNAMIC_PRODUCT_CATALOG.md` 생성
- `deliverables/FOLLOW_UP_TASKS.md` 생성
- 산출물 목록과 이미지 프롬프트 누적 갱신

### 반영시간
- 분석, DB·API·관리자·사용자 화면 구현, 빌드, DB 검증, 문서화: 약 30분.

### 운영 검증 및 배포
- GitHub `main` 커밋 `cf5928b` 푸시
- Vercel 운영 배포 `dpl_APPQHV7Suscu1kNS5bK21mkJGtCv` READY
- `zezari.family`과 `zezari.vercel.app` 홈 HTTP 200 확인
- 로그인된 관리자 상품관리에서 새 상품 폼, 긴 상세이미지, 노출 설정, 기존 디자인 편집 화면 확인
- 임시 활성 상품과 12간지 디자인을 생성해 사용자 상품 selectbox 노출, 12,300원 가격·설명 변경 반응 확인
- 임시 데이터를 모두 삭제하고 기존 전체 7개·활성 7개 상품으로 복원

## 2026-07-31 - 관리자 상품 그리드 및 선택형 상세 편집

### 요구내용
- 사용자 상품 selectbox에 표시되는 상품을 관리자페이지에서 그리드로 관리한다.
- 상품명과 가격을 수정하고 상품 썸네일과 세로형 상세페이지 이미지를 업로드한다.

### 반영내용
- 상품별로 반복되던 대형 편집 카드를 하나의 상품 그리드와 오른쪽 상세 편집 패널 구조로 변경했다.
- 그리드에서 썸네일, 상품명, 가격, 노출 상태, 정렬 순서, 상세페이지 등록 여부를 한눈에 확인하도록 구성했다.
- 선택한 상품만 오른쪽에서 수정하고 `상품 정보`, `디자인 관리` 탭으로 기능을 분리했다.
- 그리드와 상세 패널에 독립 스크롤을 적용해 상품·디자인·긴 이미지가 늘어나도 전체 화면 높이와 다른 상품 배치가 변하지 않게 했다.
- 새 상품 등록도 오른쪽 패널에 통합했으며 저장 직후 생성한 상품이 선택된 상태로 돌아오도록 개선했다.
- 기존 12간지 디자인, 상품별 가격·이미지, 사용자 구매 selectbox 연동은 그대로 유지했다.

### 검증결과
- Next.js 운영 빌드 성공
- Git 공백 오류 검사 통과
- 운영 브라우저 검증 및 배포 결과는 배포 완료 후 본 항목에 추가 기록

### 공식 산출물
- `deliverables/DYNAMIC_PRODUCT_CATALOG.md` 갱신
- 누적 이미지 생성 프롬프트 갱신

### 반영시간
- 구조 분석, 화면 구현, 반응형 스타일, 빌드, 문서화: 약 25분.

### 운영 검증 및 배포
- GitHub `main` 커밋 `571a773` 푸시
- Vercel 운영 배포 `dpl_7YtMvwc4KVPGNPtT4TWsAfoBHFEX` READY
- `zezari.family`, `zezari.vercel.app` 홈과 관리자 상품관리 HTTP 200 확인
- 운영 관리자 화면에서 상품 7개 그리드 조회 확인
- 1440px 화면에서 상품 그리드와 360px 상세 패널이 겹침 없이 나란히 배치되고 각각 독립 스크롤되는 것을 확인
- `팔찌` 행 선택 시 상품명, 5,000원 가격, 선택 표시, URL 상품 ID가 즉시 함께 변경되는 것을 확인
- 디자인 탭에서 팔찌 12간지 디자인 12개와 내부 세로 스크롤 확인
- 새 상품 화면에서 상품명, 가격, 노출, 썸네일, 긴 상세페이지 업로드 항목 확인
- 사용자 상품 구매 selectbox의 활성 상품 7개와 선택 상품의 12간지 디자인 유지 확인
- 관리자 상품관리와 사용자 상품구매 화면 콘솔 오류 없음

## 2026-07-31 - 네이버 비밀번호 없는 가입 및 재검수 요청

### 요구내용
- 네이버 로그인 검수 반려 사유 2건을 수정한다.
- 기존 승인 앱과 신규 앱이 회원정보를 이관·공유하지 않는 별도 회원체계임을 소명한다.
- 네이버 최초 가입에서 별도 비밀번호를 받지 않는다.
- 수정 화면을 캡처해 네이버 재검수를 요청한다.

### 반영내용
- SNS 최초 가입의 아이디·비밀번호 입력란을 제거했다.
- 네이버 계정 식별값으로 로그인하고 휴대폰 인증, 보호자 필수정보, 약관 동의만 진행하도록 변경했다.
- 일반 아이디 회원가입의 비밀번호 규칙은 유지했다.
- 기존 SNS 가입자는 추가 가입 절차 없이 대시보드로 이동하도록 유지했다.
- 신규 화면에 `별도 아이디와 비밀번호를 만들지 않고 네이버 계정으로 로그인합니다.` 안내를 추가했다.
- 기존 `zezari.com` 회원정보와 신규 `zezari.family` 회원정보를 이관·공유하지 않는 별도 체계임을 소명 내용에 기재했다.

### 검증결과
- 실제 네이버 동의 화면과 등록된 로컬 콜백을 통과했다.
- 격리 DB에서 휴대폰 인증과 비밀번호 없는 정보입력 단계까지 확인했다.
- 로그인, 휴대폰 인증, 정보입력 화면 3장을 개인정보 없는 검수 자료로 등록했다.
- 제공정보 확인 3개, 신규 회원가입 적용 형태, 추가서류 없음 상태를 확인했다.
- 재검수 제출 후 상태가 `승인거부`에서 `검수요청`으로 변경되고 요청일 `2026.07.31`이 표시되었다.
- Next.js 운영 빌드와 Git 공백 검사를 통과했다.

### 공식 산출물
- `deliverables/NAVER_LOGIN_REVIEW_RESUBMISSION.md` 생성
- `deliverables/naver-review/` 검수 캡처 3장 생성
- 후속 작업 목록, 산출물 목록, 이미지 프롬프트 누적 갱신

### 반영시간
- 반려 분석, 소스 수정, OAuth 검증, 캡처 준비, 재검수 제출, 문서화: 약 50분.

## 2026-07-31 - 비즈콜 안심번호 가능 여부 및 계정 준비상태 확인

### 요구내용
- 비즈콜 HTTPS 연동규격서와 로그인된 비즈콜 사이트를 확인한다.
- 보호자가 입력한 실제 전화번호를 공개 페이지에서 안심번호로 제공할 수 있는지 판단한다.

### 확인내용
- 실제 전화번호 자체를 변환하는 것이 아니라, 비즈콜 050 가상번호를 보호자의 실제 착신번호에 연결하는 방식이다.
- 신규 자동 연결은 `/link/auto_mapp.do`, 기존 번호 재연결과 해지는 `/link/set_vn.do`를 사용한다.
- 기존 REAL_QR_FIND 소스에 자동 발급, 재연결, 상태 저장, 공개 페이지 원전화번호 차단 로직이 이미 구현되어 있다.
- 비즈콜 계정에는 전체 가상번호 100개와 할당 가상번호 5개가 표시되고 050 번호 관리 화면도 활성화되어 있다.
- 로컬과 Vercel에는 계약별 API 기본 URL과 Interface ID가 아직 등록되어 있지 않다.

### 결론
- 안심번호 적용은 가능하고 비즈콜 계약 회선도 준비되어 있다.
- 비즈콜에서 API 기본 URL과 Interface ID를 확인해 서버 환경변수에 등록한 뒤 실제 1건 발급 및 통화 시험을 해야 운영 적용이 완료된다.
- 실제 전화번호와 인증정보는 로그에 기록하지 않았다.

### 공식 산출물
- `deliverables/BIZCALL_SAFE_PHONE_INTEGRATION.md` 계정 준비상태 갱신

### 반영시간
- 규격서 검토, 소스 대조, 계정·환경변수 확인, 문서화: 약 15분.

## 2026-08-01 - 위치기반서비스 사업계획서 및 한글 산출물 완성

### 요구내용
- `reference` 폴더의 위치기반서비스 사업계획서 HWPX 양식을 기준으로 나머지 내용을 작성한다.
- 제출·수정에 사용할 수 있도록 HWP 파일도 함께 만든다.

### 반영내용
- 양식의 사업자 현황, 서비스 내용, 위치정보 보호조치, 주요설비와 설치장소 순서를 유지했다.
- 별첨으로 위치정보 관리지침 20개 조문, 기술적 보호조치 증빙자료 목록, 사업자 확인서류 목록, 제출 전 최종 점검표를 구성했다.
- 현재 서비스의 QR 스캔, 발견자 1회 위치공유, 좌표 검증·저장, 카카오맵 푸시, 관리자 이력 조회를 실제 구현 내용으로 설명했다.
- 브라우저·운영체제 위치서비스를 좌표 산출 주체로 구분하고 카카오맵·네이버지도는 좌표 표시용 외부 지도서비스로 정리했다.
- 회사정보는 개인정보처리방침에 공개된 제자리, 대표자, 사업자등록번호, 주소, 대표전화와 이메일만 사용했다.
- 신고 전 보완할 별도 동의이력, 이용·제공사실 원장, 좌표 자동파기·저장보호, 관리자 감사로그를 숨기지 않고 명시했다.

### 공식 산출물
- `deliverables/location-service/REAL_QR_FIND_위치기반서비스_사업계획서.hwp`
- `deliverables/location-service/REAL_QR_FIND_위치기반서비스_사업계획서.hwpx`
- `deliverables/location-service/REAL_QR_FIND_위치기반서비스_사업계획서.pdf`
- `deliverables/location-service/REAL_QR_FIND_위치기반서비스_사업계획서_Word편집본.docx`
- 동일 원고 Markdown, 생성 스크립트, 구조도 5종과 재생성 README

### 검증결과
- 한컴오피스에서 HWP와 HWPX 저장 및 HWP 재열기 성공
- HWPX 본문과 구조도 5개 내장, 주요 장·별첨 텍스트 확인
- A4 PDF 19쪽 전체 렌더링 후 한글, 표, 링크, 구조도 잘림 없음 확인
- Word 편집본 제목 스타일, 그림, 섹션, 표 폭 구조감사 통과
- 문서와 PDF에 환경변수·API 키·토큰·시크릿 미포함 확인

### 발표자료 핵심 포인트
- 서비스는 상시 위치추적이 아니라 발견자가 동의하고 누른 시점의 단발성 위치공유다.
- 보호자에게는 QR 관리대상과 연결된 발견 위치를 카카오맵 링크로 즉시 전달한다.
- 실제 구현 기능과 신고 전 보호조치 보완계획을 분리해 사업계획서의 신뢰성을 높였다.
- 운영·보안·법적 증빙까지 하나의 제출 체크리스트로 연결했다.

### 반영시간
- 양식 분석, 법령·소스 대조, 원고·구조도 작성, 4개 문서형식 생성, 전체 페이지 검수와 로그 작성: 약 45분.

### GitHub 및 운영 반영
- GitHub `main` 문서 커밋 `d4d0f84` 푸시
- Vercel 운영 배포 `dpl_8HFijjJ1nW9XZ6bevdiC1mb47ygr` READY
- 새 배포 주소, `zezari.family`, `zezari.vercel.app` 모두 HTTP 200 확인
## 2026-08-01 - 실종광고 거리·기간 선택 방식 개편

### 요구내용
- 광고대상 선택 후 노출 거리와 오늘부터의 광고 일수를 선택하도록 변경
- 선택 내용을 요약한 뒤 결제 화면으로 이동
- 관리자가 거리와 기간 옵션을 각각 그리드에서 추가·수정·삭제
- 첨부 예시의 10/20/40/80km/전국과 1/3/7/14/30일을 초기값으로 적용

### 반영내용
- 보호자 광고 설정을 거리 선택, 기간 선택, 선택 요약·결제의 3단계 화면으로 재구성
- 반경 광고는 휴대폰 현재 위치를 중심으로 설정하고 전국 광고는 대한민국 전체 타겟으로 처리
- 관리자 광고결제 관리에 거리 옵션과 기간 옵션 그리드 추가
- 옵션별 명칭, 거리/일수, 안내문, 금액, 노출 여부를 관리 가능
- 저장한 활성 옵션은 사용자 화면에 바로 반영
- 기존 광고 결제 검증은 유지하고 신규 광고만 가격 버전 3으로 구분
- 사용자 결제금액과 Meta 집행예산은 계속 분리

### 검증결과
- Next.js 운영 빌드 성공
- SQL 자리표시자 및 결제금액 스냅샷 검증 완료
- 실제 결제·Meta 신규 광고 발행은 비용 발생 방지를 위해 실행하지 않음

### 산출물
- `deliverables/AD_DISTANCE_DURATION_OPTIONS.md`

### 반영시간
- 분석, DB·관리자·사용자 화면 구현, 호환성 검증과 문서화: 약 45분

### 운영 검증 및 배포
- GitHub `main` 구현 커밋: `6db5e1c`, `b1f985f`, `a4a5b6e`
- Vercel 운영 배포: `dpl_2vcUBe5XWsZMZATPkfgea6GpvteP` READY
- 관리자 화면에서 거리 5개와 기간 5개, 추가·삭제·노출·금액·저장 기능 확인
- 보호자 운영 화면에서 대상자 선택, 거리 선택, 기간 선택, 요약·결제 단계 확인
- 전국 확산과 3일 선택 시 기간 30,000원 + 거리 100,000원 = 총 130,000원 계산 확인
- 검증 중 브라우저 콘솔 오류 없음
- 실제 결제와 Meta 광고 발행은 비용 발생 방지를 위해 수행하지 않음

## 2026-08-01 - 광고결제 관리 저장영역 정렬 개선

### 요구내용
- 관리자 광고결제 관리 하단에서 안내문이 한 글자씩 줄바꿈되고 저장 버튼이 길게 늘어나는 현상 수정

### 반영내용
- 공통 버튼의 전체 폭 스타일과 가로 배치가 충돌한 원인 확인
- 데스크톱은 안내문과 220~320px 저장 버튼의 2열 구조로 고정
- 모바일은 안내문과 버튼이 한 열로 배치되도록 반응형 유지
- 안내문은 한국어 단어 단위로 자연스럽게 줄바꿈

### 검증결과
- Next.js 운영 빌드 성공
- 거리·기간 데이터와 저장 서버 기능에는 변경 없음

### 반영시간
- 원인 분석, CSS 수정, 빌드 및 문서화: 약 10분

### 운영 검증 및 배포
- GitHub `main` 구현 커밋: `d75d81c`
- Vercel 운영 배포: `dpl_5v5SL9g748esp5w1fHvf1WkBPq4o` READY
- 1280px 운영 화면에서 안내문 637px, 저장 버튼 320px로 분리 배치 확인
- 안내문 한 줄 표시, 페이지 가로 넘침 없음, 브라우저 콘솔 오류 없음 확인

## 2026-08-01 - 안심번호 24시간 공유 풀 전환

### 요구내용
- 보호자마다 안심번호를 영구 발급하지 않고 10~20개 번호를 공유해 비용 절감
- QR 공개 페이지 접근 시 보호자 번호와 즉시 매칭하고 약 24시간 후 회수
- 이미 매칭된 번호의 중복 사용 방지
- 모든 번호 사용 중이면 가장 오래된 매칭을 회수해 새 보호자에게 배정
- 관리자 안심번호 추가·삭제, 상세정보, 매칭일시, 수동 해제, 보호자 조회·수동 매칭 화면 제공

### 반영내용
- 비즈콜 특정 번호 자동 만료 연결 API를 사용하는 24시간 임대 구조 구현
- 유효한 기존 매칭 재사용, 빈 번호 우선, 풀 소진 시 최장 사용 번호 회수 순서 적용
- 보호자 단위 잠금과 번호별 버전 잠금으로 동시 접근 중복 배정 차단
- 안심번호 풀, 매칭 이력, 동시성 잠금 DB 구조 추가
- 관리자 왼쪽 메뉴에 `안심번호 관리` 추가
- 현황 카드, 검색·필터, 번호 그리드, 오른쪽 상세, 보호자 수동 매칭, 해제·삭제, 이력, CSV 다운로드 구현
- QR 공개 페이지는 24시간 풀 번호만 보여주며 실패 시 실제 보호자 번호를 절대 대신 노출하지 않음
- 기존 영구 발급 데이터는 호환용으로만 보존

### 기대효과
- 적은 수의 계약 회선을 반복 사용해 보호자 수 증가에 따른 회선 비용을 억제
- 발견자에게는 050 안심번호만 제공해 보호자 원전화번호 보호
- 관리자 화면에서 번호 부족, 장기 매칭, API 오류와 재사용 이력을 한 곳에서 운영

### 검증결과
- Next.js 운영 빌드 성공
- 실제 통화 연결은 비즈콜 API 기본 URL, Interface ID, 계약 050 번호 등록 후 최종 시험 필요
- 민감한 키, 보호자 원전화번호, 실제 계약 번호는 로그와 산출물에 기록하지 않음

### 산출물
- `deliverables/BIZCALL_SAFE_PHONE_POOL.md`
- 구조도 이미지 생성 프롬프트 누적

### 반영시간
- 규격 검토, 구조 설계, DB·서버·관리자·공개 화면 구현, 빌드와 문서화: 약 55분

### GitHub 및 운영 검증
- GitHub `main` 구현 커밋: `4bdedec`
- Vercel 소스 배포: `dpl_AXUTHXVngivxsxT2MMMngVwzzpfa` READY
- `https://zezari.family`, `https://zezari.vercel.app` 모두 신규 배포 연결 및 HTTP 200 확인
- 운영 관리자 화면에서 메뉴, 현황, 설정 경고, 추가 폼, 검색·필터, 그리드, 상세 빈 상태 확인
- 활성 공개 QR에서 보호자 개인정보 없이 `안심번호 준비중`만 표시되는 것 확인
- 브라우저 콘솔 오류 없음
- 비즈콜 API 주소·Interface ID 및 계약 번호 등록 전이므로 실제 통화 연결 시험은 보류

## 2026-08-02 - QR 접근 안심번호 준비중 원인 분석 및 보강

### 요구내용
- QR 또는 링크로 관리대상 공개 페이지에 들어오면 미매칭 050 번호를 보호자 연락처와 즉시 연결
- 공개 페이지에 실제 연결 가능한 안심번호 표시

### 확인내용
- QR 접근과 서버 배정 로직은 정상 실행됐으나 운영 서버의 `BIZCALL_INTERFACE_ID` 누락으로 공급자 호출 전 실패
- 비즈콜 포털에는 사용 가능한 가상번호가 남아 있어 번호 부족 문제는 아님
- 표준 API 주소 응답을 확인했으며 포털 로그인 ID와 API Interface ID가 다른 값임을 확인
- 실패 중에도 보호자 원전화번호는 공개 페이지와 로그에 노출되지 않음

### 반영내용
- 표준 비즈콜 API 주소를 서버 기본값으로 적용하고 계약별 주소 덮어쓰기 지원 유지
- 신규 번호를 선점하기 전에 필수 설정을 검사해 설정 누락으로 번호 상태가 실패 처리되는 현상 방지
- 이미 유효하게 연결된 24시간 번호는 API 설정이 일시적으로 빠져도 계속 재사용
- 관리자 안심번호 화면에 델피콤 발급 Interface ID가 남은 필수 설정임을 명확히 안내
- 산출물과 이미지 생성 프롬프트 갱신

### 남은 외부 작업
- 델피콤에서 발급한 `BIZCALL_INTERFACE_ID`를 로컬 및 Vercel 서버 환경변수에 등록해야 실제 통화 연결 시험 가능

### 검증결과
- Next.js 전체 운영 빌드 성공
- `git diff --check` 성공
- Interface ID 미설정 시 표준 API 주소는 기본 적용되고 남은 설정값은 `BIZCALL_INTERFACE_ID` 하나로 표시됨
- GitHub `main` 구현 커밋 `e2da700` 및 Vercel 운영 배포 `dpl_BjpPx24rqTTPHTQvXvAd2i2CLyvf` READY 확인
- `zezari.family`, `zezari.vercel.app` HTTP 200 확인
- 운영 QR 페이지에서 보호자 개인정보 비노출 유지 및 Interface ID 미설정에 따른 `안심번호 준비중` 상태 확인

### 반영시간
- 운영 진단, 포털·규격 확인, 서버 보강 및 문서화: 약 40분

## 2026-08-03 - 실종광고 고정 양식 적용

### 요구내용
- 기존 실종광고 캡처 이미지를 제공된 `실종광고 양식.png` 디자인으로 교체
- 왼쪽에 관리대상 사진, 오른쪽에 이름·나이·성별, 노란 영역에 보호자 메시지, 하단 빈칸에 QR 배치
- 운영 배포 후 실제 Meta 광고까지 확인

### 반영내용
- 제공된 1080 x 1350 원본 양식을 서비스 정적 자산으로 적용
- 사용자 광고 요약 화면에서 실제 양식과 동일한 미리보기 제공
- 화면 크기에 따라 달라지는 캡처 대신 Canvas로 항상 1080 x 1350 광고 이미지를 생성
- 사진은 영역을 채우도록 중앙 기준 자르기, QR은 비율 유지, 긴 이름과 메시지는 영역 안에서 자동 축소·줄바꿈
- 보호자 메시지에 휴대폰 원번호가 포함되면 광고 이미지와 Meta 본문에서 QR 연락 안내로 자동 치환
- 기존 결제, 광고 이미지 저장, Meta 자동발행, 관리대상 공개 페이지 클릭 연결 구조 유지

### 검증결과
- Next.js 전체 운영 빌드 성공
- `git diff --check` 성공
- 사용하지 않게 된 `html-to-image` 의존성 제거 확인
- GitHub 구현 커밋 `bdb51f3`, 개인정보 보강 커밋 `8163aa6` 반영
- Vercel 운영 배포 `dpl_FjdF448eUcsWWy8Jn1ukteRzxuwZ` READY 확인
- `zezari.family`, `zezari.vercel.app`, 실종광고 양식 이미지 HTTP 200 확인
- 실제 Meta 시험에서 캠페인·광고세트·소재·광고 ID 정상 발급
- Meta 저장 이미지를 다시 내려받아 1080 x 1350 양식, 대상자 정보, 메시지, QR이 유지되는 것 확인
- 캠페인과 광고세트는 PAUSED, 광고도 PAUSED 설정 상태로 생성해 노출 및 비용 발생 없음

### 공식 산출물
- `deliverables/MISSING_AD_TEMPLATE_CREATIVE.md`

### 반영시간
- 양식 분석, 합성 구현, 개인정보 보강, 빌드, 배포 및 실제 Meta 시험: 약 75분

## 2026-08-03 - 구버전 문자 인증 및 Solapi 흔적 조사

### 요구내용
- `reference/wp`에서 회원가입 인증번호 문자 발송에 사용된 Solapi API 확인

### 확인내용
- 추출된 폴더에는 WordPress 코어만 있고 `wp-content`와 플러그인 소스가 없음
- `wp.tar.gz`의 3,228개 경로에도 `wp-content`와 플러그인 파일이 없음
- `wp.sql`에서 `mshop-sms-s2`, `mshop-user-certification-s2` 사용 및 휴대폰 인증 필드 흔적 확인
- `solapi`, `coolsms`, 두 서비스 API 호스트와 관련 키 설정 검색 결과는 모두 0건

### 결론
- 구버전은 MShop 문자·본인인증 플러그인으로 인증번호를 처리했으나, 실제 하위 문자 공급자가 Solapi였다는 근거와 호출 코드는 백업에 없음
- 현재 서비스 실연동에는 Solapi에서 새 API Key, API Secret, 등록 발신번호를 발급받아 기존 Next.js 문자 어댑터에 연결해야 함
- 조사 과정에서 키, 시크릿, 전화번호, DB 계정은 산출물에 기록하지 않음

### 반영시간
- 소스, 압축 목록, SQL 메타데이터 조사: 약 20분

## 2026-08-03 - 회원가입 이메일 인증 전환

### 요구내용
- 회원가입 휴대폰 SMS 인증을 Resend 이메일 인증번호 방식으로 변경
- Vercel 환경의 Next.js API에서 발송·검증
- 기존 SMS 코드는 삭제하지 않고 화면에서 숨긴 뒤 비활성화

### 반영내용
- 일반 가입과 SNS 최초 가입 모두 이메일 주소 확인을 첫 단계로 변경
- 이메일로 6자리 인증번호 발송, 3분 내 검증, 15분짜리 1회용 가입 토큰 발급
- 인증번호와 토큰은 원문 대신 해시만 DB에 저장
- 이메일별 시간당 5회 발송 제한과 코드별 오답 5회 제한 적용
- 인증 완료 후 기본정보에서 휴대폰 번호를 연락처로 입력하도록 구성
- 기존 회원은 다시 가입 절차가 나오지 않도록 과거 인증 완료 상태 호환
- SMS 화면과 호출 제거, 기존 서버 코드는 보존하고 기본 HTTP 410 비활성화

### 외부 설정
- Vercel Resend 무료 플랜을 `zezari.family`, 도쿄 리전으로 설치하고 운영·개발 환경에 연결
- Vercel이 암호화된 API Key와 발신 도메인 환경변수를 자동 등록
- 운영 환경에는 개발용 우회 인증번호를 등록하지 않음

### 검증결과
- Next.js 16.2.11 운영 빌드 성공 및 이메일 인증 API 2개 생성 확인
- 격리 DB에서 발송, 오답 거절, 토큰 발급, 토큰 없는 가입 거절, 정상 가입, SMS 비활성 응답 확인
- 모바일 390 x 844에서 이메일 입력과 인증번호 6칸 한 줄 표시, 오류 화면 없음 확인
- Next.js 16.2.9 → 16.2.11, NextAuth 4.24.14 → 4.24.15 보안 패치 적용으로 기존 직접 취약점 제거
- Next.js 내부 PostCSS/Sharp의 간접 high 3건은 현재 호환 가능한 상향 패치가 없어 후속 추적
- GitHub 운영 커밋 `d2b7836` 반영
- Vercel 운영 배포 `dpl_CkVxJSMgepWKW9mg3BirYDj8eoZb` READY 확인
- `zezari.family`, `zezari.vercel.app` HTTP 200 확인
- 운영 이메일 발송 API를 Resend 공식 테스트 수신 주소로 호출해 HTTP 200 및 `devMode:false` 확인
- 운영 SMS 발송 API가 HTTP 410으로 비활성화된 상태 확인
- 운영 회원가입 화면에서 이메일 우선 안내, 인증번호 6칸 한 줄 배치, 콘솔 오류 0건 확인

### 공식 산출물
- `deliverables/AUTH_EMAIL_VERIFICATION.md`

### 반영시간
- 설계, DB·API·화면 구현, 격리 테스트, 모바일 확인 및 문서화: 약 65분

## 2026-08-03 - 메타 실종광고 실제 노출 상태 확인

### 요구내용
- `ZEZARI AD-2B5B7E67EC 나하하 서울특별시 송파구 가락1동` 광고가 실제 페이스북에서 게재·노출 중인지 확인

### 확인결과
- 캠페인, 광고 세트, 광고 모두 Meta 기준 `ACTIVE` 확인
- 광고 세트 기간: 2026-08-03 19:41:36 ~ 2026-08-09 23:59:59
- 전체 기간 예산: 67,200원
- 확인 시각 2026-08-03 19:33:58은 예약 시작 전이므로 아직 노출·도달·클릭·지출 실적 없음
- 광고는 승인 및 게재 준비가 완료됐지만 확인 시점에는 실제 사용자 노출 전 상태로 판정

### 반영시간
- 광고 관리자 및 Meta Graph API 교차 확인과 기록: 약 10분

## 2026-08-03 - 광고 마진율 기반 Meta 예산 편성

### 요구내용
- 보호자 광고 결제금액에서 관리자가 설정한 마진율을 제외해 Meta 광고예산 편성
- 광고결제 관리에서 마진율 설정

### 분석결과
- 기존 70,000원 결제·67,200원 Meta 예산은 4% 차감 계산이 아니라 서울·10km·7일 독립 공식의 결과
- 두 금액의 4% 차이는 우연히 일치

### 반영내용
- 관리자 광고결제 관리에 0%~90% 서비스 마진율과 70,000원 계산 예시 추가
- 기본 마진율 4% 적용
- 신규 광고는 `결제금액 × (100 - 마진율)%`로 Meta 전체 기간 예산 계산
- 광고별 신청 당시 마진율을 저장해 관리자 설정 변경 시 기존 광고 금액 보호
- 보호자 미리보기·결제, 관리자 상세·엑셀에 마진율과 집행예산 표시
- 이미 결제·집행 중인 기존 광고는 저장 예산 유지

### 검증결과
- 70,000원/4% → Meta 67,200원
- 70,000원/20% → Meta 56,000원
- 격리 DB 스키마 33, 기본 마진 4%, 광고별 마진 컬럼 확인
- Next.js 전체 빌드 성공
- GitHub 구현 커밋 `4cfcb15` 반영
- Vercel 운영 배포 `dpl_9VQxMiuLMpYTZ5eBEwiAZ3zZSxaK` READY 확인
- 운영 Turso 스키마 33, 기본 마진율 4%, 광고별 마진율 컬럼 확인
- 기존 테스트 광고는 결제 70,000원·Meta 예산 67,200원·예산 버전 2·마진율 미적용 상태로 보존 확인

### 공식 산출물
- `deliverables/ADMIN_AD_MARGIN_BUDGET.md`

### 반영시간
- 분석, DB·화면 구현, 호환 처리 및 검증: 약 50분

## 2026-08-03 - Solapi SMS 회원가입·보호자 연락처 재인증

### 요구내용
- 회원가입 인증을 이메일에서 SMS로 복원
- 일반 가입과 SNS 최초 가입 모두 휴대폰 인증 필수
- 보호자 연락처 변경 때마다 새 번호 SMS 인증 필수
- Resend 이메일 코드는 삭제하지 않고 화면과 API만 비활성화
- Solapi·Vercel 설정, GitHub 푸시, 운영 배포 및 테스트

### 반영내용
- 공식 Solapi Node SDK를 서버 전용으로 연동
- 회원가입 첫 단계를 휴대폰 번호와 6자리 인증번호 입력으로 복원
- 보호자정보에 연락처 변경용 인증번호 발송·확인 UI 추가
- 번호가 달라졌는데 인증하지 않으면 저장 차단
- 인증번호 3분, 가입 토큰 15분, 발송·오답 제한, 토큰 1회 사용 적용
- 인증정보를 로그인한 보호자와 묶어 다른 계정 토큰 사용 차단
- 이메일 인증 API는 HTTP 410으로 비활성화하고 Resend 구성은 예비 경로로 보존
- DB 스키마 34와 Solapi 발송 추적 필드 적용

### 외부설정
- Solapi 전용 API 인증정보 생성 및 등록된 발신번호 연결
- Vercel 운영·개발 환경에 Solapi 키·발신번호와 SMS/이메일 기능 플래그를 암호화 등록
- 보안값은 소스, 로그, 산출물에 기록하지 않음

### 검증결과
- Next.js 16.2.11 전체 빌드 성공
- 기존 스키마 33 격리 DB의 34 마이그레이션과 신규 컬럼·인덱스 확인
- 일반 가입 발송·오답·성공·일회용 토큰·재사용 거절 확인
- 로그인 보호자의 연락처 변경 인증·저장과 미인증 저장 차단 확인
- 모바일에서 인증번호 6칸 한 줄 표시 확인
- Solapi 실제 문자 1건 처리완료, 성공 1건·실패 0건 확인
- 이메일 인증 API 비활성 응답 확인
- 기존 Next.js 간접 보안경고 3건은 호환 가능한 상향 패치가 없어 강제 변경하지 않음

### 공식 산출물
- `deliverables/AUTH_PHONE_VERIFICATION.md`
- `deliverables/SOLAPI_SMS_PHONE_VERIFICATION.md`

### 반영시간
- Solapi 설정, DB·API·UI 구현, 실문자·격리 테스트, 마이그레이션 보완, 문서화 및 배포 작업: 약 75분

### 운영반영 결과
- GitHub `main` 커밋 `8172f65` 푸시 완료
- Vercel 운영 배포 `dpl_HGbV2Y5p79PHwwu5moLiSZYJrQ2m` READY 및 `zezari.family` 연결 완료
- `zezari.family`, `zezari.vercel.app` 모두 HTTP 200 확인
- 운영 Turso 스키마 34와 보호자·Solapi 추적 컬럼 및 인덱스 확인
- 운영 이메일 인증 API HTTP 410, 잘못된 SMS 번호 HTTP 400 확인
- 운영 회원가입 화면에서 SMS 안내, 인증번호 6칸 한 줄, 가로 넘침 없음, 오류 오버레이 없음 확인
- 기존 `zezari.vercel.app`가 이틀 전 배포에 고정된 상태를 발견해 최신 READY 배포로 별칭 재연결
- 기존 주소에서도 이메일 인증 HTTP 410, SMS 검증 HTTP 400, 보호자정보의 `인증번호 받기` 버튼 확인

## 2026-08-08 - 실제 통화 시점 비즈콜 안심번호 자동 배정

### 요구내용
- 비즈콜 번호 100개를 서비스 DB에 미리 등록하거나 QR 조회 시 선점하지 않음
- 발견자가 실제로 보호자에게 전화할 때만 임시 안심번호 자동 배정
- 공급자 번호가 100개를 넘어 추가되어도 별도 개발 없이 자동 사용
- 컬러링·알림멘트 적용, 운영 환경 반영 및 실제 API 테스트

### 반영내용
- 공개 QR 페이지의 사전 번호 배정 제거
- `안심번호로 보호자에게 전화하기` 버튼을 누를 때만 신규 API 호출
- 비즈콜이 빈 번호를 직접 선택하는 자동 만료 매핑 적용
- 임시 연결시간 1시간, 같은 보호자의 유효 연결 재사용
- 컬러링과 보호자 수신 전 알림멘트 적용 후 만료시간 재확정
- 통화 요청 제한, 보호자 단위 동시성 잠금, 번호 부족 안내, 부분 실패 시 즉시 해제 적용
- 공급자 반환 번호만 DB 운영 이력으로 기록하고 앱의 번호 수 상한 제거
- 관리자 화면을 자동 배정 현황·이력·긴급 해제 중심으로 변경
- DB 스키마 35와 비식별 통화 요청 이력 추가

### 검증결과
- Next.js 전체 빌드 성공
- 운영 DB 스키마 35 마이그레이션 확인
- 페이지 조회만으로 번호가 배정되지 않음 확인
- 실제 비즈콜 자동 배정, 컬러링·알림멘트, 자동 만료 갱신 성공
- 실제 전화는 걸지 않고 시험 번호를 비즈콜·DB에서 즉시 해제
- 보호자 원전화번호와 계약 인증정보 비노출 확인
- 보호자 메시지에 직접 입력된 전화번호 형태도 공개 화면에서 안심번호 버튼 안내로 자동 마스킹

### 공식 산출물
- `deliverables/BIZCALL_SAFE_PHONE_POOL.md`
- `deliverables/BIZCALL_ON_DEMAND_CALL_ASSIGNMENT.md`

### 반영시간
- 규격·포털 분석, API 인증 진단, DB·API·화면 구현, 실연동 검증, 문서화: 약 85분

### 운영반영 결과
- GitHub `main` 구현 커밋 `8b1f978` 푸시 완료
- Vercel 운영 배포 `dpl_2oQUBGrRSUxueNoRGCrU7PNzcNRN` READY
- `zezari.family`, `zezari.vercel.app`, 활성 공개 QR 페이지 HTTP 200
- 운영 API 실제 자동 배정 성공 후 전화 발신 없이 즉시 비즈콜·DB 해제
- 운영 DB 스키마 35와 공급자 자동 배정 이력 확인
- 운영 브라우저에서 통화 시점 버튼과 보호자 메시지 전화번호 마스킹 확인

## 2026-08-08 - 관리자 대상자 QR 수동 테스트 활성화

### 요구내용
- 관리자 대상자 상세현황에서 상품 구매와 결제 없이 QR을 수동 활성화
- 공개 관리대상 페이지를 직접 수기 테스트할 수 있는 버튼 제공

### 반영내용
- 대상자 상세 `QR` 탭에 `구매 없이 QR 수동 활성화` 버튼 추가
- 테스트 완료 후 되돌릴 수 있는 `수동 활성화 해제` 버튼 추가
- QR 한 건만 대상으로 공개 페이지, 안심번호, 위치공유 테스트 권한 적용
- 주문·결제·구독·매출 데이터에는 영향이 없도록 별도 활성화 출처로 분리
- 정상 구매 활성화와 관리자 테스트 활성화를 구분하고 오작동 방지
- QR 매칭 해제·변경·폐기 시 테스트 권한 자동 초기화

### 검증결과
- Next.js 16.2.11 전체 빌드 성공
- 관리자 서버 권한, QR 매칭, 폐기 여부, 정상 구매 활성화 보호 조건 확인
- 공개 페이지·안심번호·위치공유의 관리자 테스트 접근 조건 연결 확인

### 공식 산출물
- `deliverables/ADMIN_SUBJECT_QR_MANUAL_ACTIVATION.md`
- `deliverables/QR_MANAGEMENT.md` 갱신

### 반영시간
- 데이터·서버액션·관리자화면·공개흐름·문서 반영: 약 25분

### 운영반영 결과
- GitHub `main` 커밋 `4b8a8c9` 푸시 완료
- Vercel 운영 배포 `dpl_64vzmo6sKREkz61S4ZgNgo2m7Pth` READY
- `zezari.family`, `zezari.vercel.app`, 매칭된 공개 QR 페이지 HTTP 200
- 운영 Turso 스키마 36 및 QR 활성화 출처 컬럼 반영 확인

## 2026-08-09 - 위치정보 보안 기능 및 사업계획서 보완

### 요구내용
- 위치기반서비스 사업계획서 검토 요청사항에 맞춘 실제 보안 기능 보완
- 로그인 실패 제한, 비밀번호 규칙, 단계별 최소권한, 취급대장·접근기록 구현
- 주요설비·설치장소·기술적 보호조치와 구현 화면을 문서에 누적

### 반영내용
- 로그인 3회 연속 실패 시 15분 잠금, 비밀번호 변경 시 조합 규칙 검증
- 위도·경도·정확도·위치설명 AES-256-GCM 저장 암호화
- 별도 동의 버전·시각 저장, 시간당 요청 제한, 원본 위치 24시간 자동 파기
- 시스템 자동처리, 위치정보관리책임자, 위치정보취급자, 일반관리자의 단계별 권한 분리
- 수집·제공·파기 취급대장과 인증·열람·내보내기·권한변경 접근기록 및 무결성 해시 적용
- 관리자 `위치정보 보안` 화면과 권한 기반 CSV 내보내기 추가
- HTTPS/HSTS와 브라우저 보안 응답 헤더, 개인정보처리방침·위치동의 화면 보완
- DB 스키마 37 반영

### 검증결과
- Next.js 16.3.0 전체 빌드와 공백 검사 성공
- 호환 범위 의존성 갱신 후 운영 의존성 보안 감사 0건 확인
- 잘못된 비밀번호 3회 후 15분 잠금 확인
- 암호화·복호화와 동의 없는 위치공유 차단 확인
- 관리자 보안 화면의 권한표·취급대장·파기내역·접근제어 확인
- 24시간을 초과한 기존 원본 위치 16건 자동 파기 및 파기대장 기록
- 보안 보완본 DOCX·PDF 전체 페이지 시각 검수

### 공식 산출물
- `deliverables/location-service/LOCATION_SECURITY_COMPLIANCE.md`
- `deliverables/location-service/REAL_QR_FIND_위치기반서비스_사업계획서_보안보완본.docx`
- `deliverables/location-service/REAL_QR_FIND_위치기반서비스_사업계획서_보안보완본.pdf`
- `deliverables/location-service/evidence/*.png`

### 반영시간
- 참고자료 분석, 보안·DB·관리화면 구현, 시험, 증빙·문서 작성: 약 120분

### 제출 전 운영자 준비사항
- Vercel/Turso 계약·계정·Invoice·리전·백업·암호화·Firewall/WAF 화면
- 제출일 기준 잔여기간 1개월 이상 TLS 인증서
- 관리자 PC 백신·OS 업데이트·화면잠금, 관리자 MFA, 담당자 지정·교육·점검 기록

### 운영반영 결과
- GitHub `main` 구현 커밋 `d4bedc7` 푸시 완료
- Vercel 운영 배포 `dpl_AtAreJjMqyi1AjNmUNA6v9WEP2g1` READY
- `zezari.family`, `zezari.vercel.app`, 개인정보처리방침, 관리자 진입, 활성 공개 QR 경로 HTTP 200
- HSTS·MIME·프레임·리퍼러·위치권한 보안 응답 헤더 확인
- 비로그인 취급대장 내보내기 HTTP 401, 별도 동의 없는 위치전송 HTTP 400 확인
- 운영 Turso 스키마 37, 보안 테이블 7개, 만료 원본 위치 파기 16건 확인

## 2026-08-09 - 실제 운영환경 기준 주요설비내역 및 증빙 작성

### 요구내용
- 현재 REAL_QR_FIND 구성에 맞춰 사업계획서 4.1 주요설비내역과 4.2 설치장소 작성
- Vercel, Turso, GitHub에서 확인 가능한 운영값과 제출용 캡처 준비

### 반영내용
- 자체 물리서버가 없는 관리형 클라우드 논리서버 구조로 정리
- Vercel 웹·애플리케이션 서버의 프로젝트, Next.js·Node.js 버전, 운영배포, 도메인, Function 리전 확인
- Turso DB의 libSQL/SQLite 버전, 리전, 스키마, 테이블 수, 논리용량 확인
- GitHub 소스·변경이력·배포기준 저장소와 Vercel 자동배포 연결 확인
- 관리자 PC·사업장 네트워크와 이용자 스마트폰을 핵심 서버와 구분
- OAuth, Toss, Solapi, Bizcall, Meta, Web Push, Kakao Map을 외부 연동 서비스로 별도 분류
- 인증키·토큰·시크릿과 원본 DB 호스트를 제외한 증빙 이미지 3종 생성
- 보안보완본 DOCX·PDF의 주요설비 표와 증빙 4~6 반영
- 현재 Vercel Function은 미국, Turso DB는 일본에 있어 국외 처리·이전 고지 및 리전 정렬 검토사항 명시

### 검증결과
- Vercel 프로젝트·운영배포·도메인·런타임·리전 확인
- Turso 운영 DB 엔진·스키마 37·업무 테이블 37개·논리용량 6.68 MB 확인
- GitHub `main`, 운영 기준 커밋, `.env.local` 제외, Vercel 배포 연결 확인
- 56쪽 PDF 렌더링 후 주요설비 표와 증빙 6개 페이지의 잘림·겹침·제목 분리 여부 확인
- 생성된 JSON·이미지에 비밀값이 포함되지 않은 상태 확인

### 공식 산출물
- `deliverables/location-service/MAJOR_EQUIPMENT_INVENTORY.md`
- `deliverables/location-service/REAL_QR_FIND_위치기반서비스_사업계획서_보안보완본.docx`
- `deliverables/location-service/REAL_QR_FIND_위치기반서비스_사업계획서_보안보완본.pdf`
- `deliverables/location-service/evidence/04-vercel-project-deployment.png`
- `deliverables/location-service/evidence/05-turso-database-status.png`
- `deliverables/location-service/evidence/06-github-repository-deployment.png`

### 반영시간
- 운영 인프라 확인, 공식자료 대조, 마스킹 증빙 생성, 문서 보완, 시각 검수: 약 55분

### 제출 전 추가 준비
- 제출기관이 제공자 원본 화면을 요구하면 Vercel·Turso·GitHub 로그인 상태의 계정·리전·청구 화면을 추가
- TLS 인증서, Vercel·Turso Invoice, 백업·암호화 확인, 관리자 PC 보안, 관리자 MFA 자료 첨부

### 운영반영 결과
- GitHub `main` 커밋 `a04fcbe` 푸시 완료
- Vercel 운영 배포 `dpl_AHxyf7e7YPsbpD6RbATeL9eUYKAa` READY, Function 리전 `iad1` 확인
- `zezari.family`, `zezari.vercel.app` 최신 배포 연결 및 HTTP 200 확인

## 2026-08-10 - 위치정보 암호화 저장 증빙 화면

### 요구내용
- 암호화 위치 컬럼과 암호문 샘플 2~3건을 인앱 브라우저 및 캡처 이미지로 확인

### 반영내용
- 운영 연결 DB를 읽기 전용으로 조회하여 위치공유 누적 16건, 보존기간 만료 파기 16건, 현재 보존 중 암호문 0건 확인
- `latitude_encrypted`, `longitude_encrypted`, `accuracy_encrypted`, `address_encrypted` 컬럼과 AES-256-GCM 보호 방식을 표로 정리
- 실제 이용자 위치 대신 동일 암호화 함수로 만든 비식별 테스트 암호문 3건을 증빙용으로 표시
- 실제 사용자 정보, 복호화 좌표, 키와 토큰은 증빙에서 제외

### 산출물 및 반영시간
- `deliverables/location-service/evidence/07-location-encryption-evidence.html`
- `deliverables/location-service/evidence/07-location-encryption-evidence.png`
- 확인 및 캡처: 약 15분

## 2026-08-14 - 클라우드 계정·논리설비 증빙 캡처

### 요구내용
- 인앱 브라우저에 로그인된 Vercel과 Turso 계정·서비스 화면 캡처

### 반영내용
- Vercel 팀 `zezariVercel`, Hobby 플랜, 프로젝트 `zezari`, 운영배포 Ready, 운영 도메인과 GitHub `main` 연결 화면 캡처
- Turso 계정 `zezarigit`, Free 플랜, 그룹 `default`, DB `zezariturso`, Active 상태 화면 캡처
- 환경변수, API 키, 인증 토큰, DB 접속정보, 결제수단과 개인정보는 캡처에서 제외

### 산출물 및 반영시간
- `deliverables/location-service/evidence/08-turso-account-database.png`
- `deliverables/location-service/evidence/09-vercel-account-project.png`
- 확인 및 캡처: 약 15분

## 2026-08-14 - Vercel 호스팅 계정·서비스 제공사업자 증빙

### 요구내용
- 현재 호스팅 서비스가 Vercel인지 확인하고, 호스팅 계정과 서비스 제공사업자 정보를 포함한 화면을 캡처

### 반영내용
- Vercel이 현재 웹 애플리케이션의 운영 호스팅·배포 서비스임을 확인
- 팀 `zezariVercel`, Hobby 플랜, 프로젝트 `zezari`, 운영상태 Ready, 도메인 `zezari.family`·`real-qr-find.vercel.app`, 배포 브랜치 `main` 화면 캡처
- Vercel 공식 이용약관에서 서비스 제공 주체 `Vercel Inc.` 확인 및 캡처
- 공식 약관상 통지 주소 `440 N Barranca Ave #4133, Covina, CA 91723` 확인 및 캡처
- 환경변수, API 키, 인증 토큰, 결제수단과 로그인 자격증명은 캡처에서 제외

### 산출물 및 반영시간
- `deliverables/location-service/evidence/10-vercel-hosting-account.png`
- `deliverables/location-service/evidence/11-vercel-service-provider-info.png`
- `deliverables/location-service/evidence/12-vercel-service-provider-address.png`
- 확인 및 캡처: 약 10분

### 운영반영 결과
- GitHub `main` 커밋 `1b0eac8` 푸시 완료
- Vercel 운영 배포 `dpl_2yrLpXeDUGBr9mDiRULsT572CSsh` READY 확인
- `zezari.family`, `zezari.vercel.app`, `real-qr-find.vercel.app` HTTP 200 확인

## 2026-08-14 - 위치정보 이용·제공 및 열람·고지 취급대장

### 요구내용
- 위치공유 발생 시 대상, 취득경로, 제공 서비스, 제공받는 자, 이용일시를 전자적으로 자동 기록·보존
- 위치정보주체 열람·고지 처리 시 취급자, 요청자, 목적, 일시를 취급대장에 기록

### 반영내용
- 위치공유와 보호자 알림 결과가 자동 기록되는 `location_use_ledger`를 요청 양식 5개 열로 관리자 화면에 표시
- DB 스키마 38 및 수정·삭제 UI가 없는 `location_disclosure_ledger` 추가
- 열람·고지 요청자 식별값과 목적을 입력하면 취급자와 일시를 서버에서 자동 기록
- 두 원장 CSV 다운로드와 조회·내보내기 접근기록 연동
- 원장에는 실제 좌표를 저장하지 않고 연속 무결성 해시를 보존

### 산출물 및 반영시간
- `deliverables/location-service/LOCATION_USE_PROVISION_LEDGER.md`
- 빌드 및 검증: 약 25분

### 운영반영 결과
- GitHub `main` 커밋 `1e73260` 푸시 완료
- Vercel 배포 `dpl_5Efa3B3MRgnADyqLUWkZgY4Ge7eG` READY 확인
- 운영 도메인 3개 HTTP 200 확인

## 2026-08-14 - 운영 위치공유·원장 저장 검증

### 요구내용
- 공개 관리대상 페이지에서 수행한 위치공유가 운영 DB와 이용·제공 원장에 정상 저장됐는지 조회하고 캡처

### 확인내용
- 공개키 `zrf-kbg9hhmcsn7rm3`의 신규 위치공유 1건 확인
- 저장일시 `2026-08-14 16:11:51 KST`, 24시간 보존만료 적용 확인
- 원문 좌표는 0, 위도·경도·정확도·위치설명 암호문은 모두 저장 확인
- 수집 원장 1건, 보호자 제공 원장 1건 성공 확인
- 수집 원장 해시와 제공 원장 이전 해시가 일치하여 원장 연결 무결성 확인
- 실제 좌표, 보호자 개인정보, DB 인증정보는 증빙에서 제외

### 산출물 및 반영시간
- `deliverables/location-service/evidence/13-location-ledger-production-evidence.html`
- `deliverables/location-service/evidence/13-location-ledger-production-evidence.png`
- 조회 및 캡처: 약 15분

## 2026-08-14 - 서버·관리자 PC 보안프로그램 증빙

### 요구내용
- 서버와 관리자 PC의 보안프로그램 운영 여부 확인

### 확인내용
- 관리자 PC Windows Defender 백신·실시간 보호·행동 감시·네트워크 검사·다운로드 검사 활성 확인
- Domain, Private, Public Windows 방화벽 활성 확인
- 빠른 검사는 6일 전, 전체 검사 완료 기록 없음 확인
- Vercel 관리형 서버는 고객의 OS 백신 직접 확인이 불가능하여 공식 플랫폼 방화벽, DDoS 방어, 지속 스캔, SOC 2 Type 2, ISO 27001:2022 자료로 증빙

### 산출물 및 반영시간
- `deliverables/location-service/SECURITY_PROGRAM_EVIDENCE.md`
- `deliverables/location-service/evidence/14-admin-pc-security-evidence.html`
- `deliverables/location-service/evidence/14-admin-pc-security-evidence.png`
- `deliverables/location-service/evidence/15-vercel-managed-security-evidence.png`
- 확인 및 캡처: 약 15분

## 2026-08-14 - 외부 테스터 유지보수 요청서

### 요구내용
- 개발 발주자와 테스터가 테스트 중 발견한 오류와 추가 요구사항을 화면 캡처와 함께 전달할 수 있는 Google Sheets 양식 제작

### 반영내용
- 화면·URL·캡처 링크, 현재 상황, 재현 절차, 기대 결과와 요구사항을 한 행에서 관리하는 22개 항목 구성
- 오류·개선·추가 요구사항·문의 구분과 심각도, 우선순위, 처리 상태, 재테스트 결과 드롭다운 적용
- 고정 헤더, 필터형 표, 상태별 색상, 예시 요청 1건 적용
- 별도 `작성안내`, `코드목록` 탭과 개인정보 마스킹 기준 추가
- 프로젝트 이메일 `general@zezari.com` 편집 권한 및 Google Sheets 실제 화면 확인

### 산출물 및 반영시간
- Google Sheets: https://docs.google.com/spreadsheets/d/1i8gbKgnTgBiMXSaEFXrTn7u7iv9YxA8q6G3DqJRubu4/edit
- `deliverables/MAINTENANCE_REQUEST_SHEET.md`
- 제작 및 검증: 약 20분

## 2026-08-14 - 운영 소스 보안 점검 및 개선

### 요구내용
- `reference/`를 제외한 현재 운영 소스의 취약점 점검
- 기존 기능을 최대한 유지하면서 발견 사항 개선

### 반영내용
- 이미지 MIME·실제 파일 시그니처 검증으로 저장형 실행 파일 차단
- 보호자 음성 형식·용량 제한과 기존 미디어 hidden 필드 신뢰 제거
- 푸시 구독 공급자 주소·암호화 키 검증 및 알림 URL 안전화
- 인증번호 발송, 공개 보호자 알림, 지도 검색 요청 제한
- 인증 완료 토큰의 동시 재사용 차단과 운영 전용 시크릿 강제
- 비밀번호 해시 강화와 기존 회원 호환 유지
- 관리자 활성 계정 확인, 내부 리다이렉트 제한, CSV 수식 주입 방어
- 민감 API 캐시 방지와 CSP 등 브라우저 보안 헤더 추가
- 보안 회귀 검사 명령 추가

### 검증 및 결과
- 운영 npm 취약점 0건
- 보안 회귀 검사·Next.js 프로덕션 빌드·공백 검사 통과
- 로컬 운영 서버에서 홈 200, 보안 헤더, 알림함 비로그인 401 및 no-store 확인
- 기능·DB 스키마 변경 없이 서버 경계 중심으로 보강

### 산출물 및 반영시간
- `deliverables/SECURITY_HARDENING_REVIEW_2026-08-14.md`
- 점검·개선·검증·문서화: 약 40분

### 배포결과
- GitHub `main` 보안 소스 커밋 `a034371` 반영
- Vercel 운영 배포 `dpl_73DwtxGAFjb6ZzGM1NgcyiVYXtKm` READY
- 구형 `zezari.vercel.app` 별칭을 최신 운영 배포에 재연결
- `zezari.family`, `zezari.vercel.app`, `real-qr-find.vercel.app` HTTP 200 및 보안 헤더 확인
- 배포 직후 운영 오류 로그 없음

## 2026-08-19 - 대시보드 상단·페이지 표시·관리대상 등록 개선

### 요구내용
- 검은색 상단 바와 `Z 제자리` 문구 제거
- 알림 종은 왼쪽 위, 마이페이지는 오른쪽 위에 배치
- 실종신고·상품구매·내 정보 세 버튼을 같은 스타일로 통일
- 관리대상 스와이프 페이지 수와 현재 위치를 원형 점으로 표시
- 보호자당 관리대상 최대 4명 제한을 제거하고 계속 등록 가능하도록 변경

### 반영내용
- 상단 앱 바 없이 좌우 모서리 아이콘만 표시하도록 대시보드 구조 변경
- 실종신고 버튼의 검은 배경 예외를 제거하여 세 버튼 모두 흰 배경·검은 테두리로 통일
- 스와이프 위치를 추적하는 캐러셀 컴포넌트와 클릭 가능한 페이지 점 추가
- 관리대상 3명 단위 페이지 구성과 마지막 페이지 `+` 등록 버튼 유지
- 관리대상정보 화면에 항상 신규 등록 양식을 표시하고 서버의 4명 저장 제한 제거
- 최대 4명 안내, `/4명` 카운터와 관리자 화면의 최대 인원 표기 제거

### 검증 및 반영시간
- Next.js 프로덕션 빌드 통과
- 보안 회귀 검사 통과
- 공백 오류 검사 통과
- 로컬 운영 서버 콘솔 오류 없음
- 구현·검증·문서화: 약 30분

### 배포결과
- GitHub `main` 커밋 `6b44e6f` 반영
- Vercel 운영 배포 `dpl_JBjTqZM91duksEqaTNUz7TbmCfbK` READY
- `zezari.vercel.app`를 최신 운영 배포에 재연결
- 관리대상 4명이 등록된 운영 계정에서 2페이지·점 2개·현재 점 전환과 다섯 번째 신규 입력 양식 확인
- 알림 왼쪽 위, 마이페이지 오른쪽 위, 상단 바 제거와 빠른 메뉴 3개 동일 스타일 확인
- 세 운영 주소 HTTP 200·HSTS 확인, 배포 직후 운영 오류 로그 없음

## 2026-08-19 - 관리대상 한 명 단위 등록·수정 개편

### 요구내용
- 신규 관리대상 추가 시 빈 입력 화면 한 개만 표시
- 기존 대상자는 선택한 한 명만 수정
- 마이페이지에 등록 대상자를 계속 누적 표시하고 각 대상의 수정 메뉴 제공
- 이름으로 대상자 검색 기능은 제외

### 반영내용
- 신규 등록과 대상자별 수정 URL을 분리하고 대상자정보 화면에는 양식 한 개만 렌더링
- 마이페이지 대상자 정보 영역을 사진·이름·상태·성별·생년월일 목록으로 변경
- 각 대상자 행을 해당 대상자 수정 화면에 연결
- 목록 아래 `대상자 추가` 버튼과 대시보드 `+` 버튼을 신규 한 명 등록 화면에 연결
- 수정 성공·오류 시 같은 대상자 화면 유지, 삭제 후 마이페이지 목록으로 복귀
- 검색 입력창과 검색 처리 로직은 추가하지 않음

### 검증 및 반영시간
- Next.js 프로덕션 빌드 통과
- 보안 회귀 검사와 공백 오류 검사 통과
- 구현·라우팅 검토·반응형 스타일·문서화: 약 25분

### 배포결과
- GitHub `main` 커밋 `f90f21c` 반영
- Vercel 운영 배포 `dpl_B6kFRmmSZoems2y91V2LxzJZAFkF` READY 및 `zezari.vercel.app` 최신 연결
- 390px 운영 화면에서 마이페이지 대상자 4명 목록, 검색창 없음, 신규 양식 1개, 수정 양식 1개 확인
- 모바일 가로 넘침과 브라우저 콘솔 오류 없음

## 2026-08-19 - 운영 구성·서비스·계정 통합대장

### 요구내용
- Vercel, Turso DB, GitHub와 현재 서비스 구조를 한눈에 확인할 수 있도록 정리
- 안심번호, 인증번호, Toss 결제, Meta 광고, OAuth, Push 등 외부 연동 관리
- 계정 로그인·비밀번호 정보의 안전한 인수인계 체계 마련

### 반영내용
- GitHub → Vercel → Turso와 OAuth·Solapi·Toss·Meta·Bizcall·Push·지도 연결 구조도 작성
- 운영 도메인, 프로젝트·조직 식별정보, 37개 DB 테이블의 업무영역 정리
- 서비스별 관리 콘솔, 환경변수 이름, 현재 상태, 코드 위치, 키 교체 영향과 정기점검 주기 작성
- 계정별 로그인 URL, 확인된 계정 식별자, 권한, 비밀번호 관리자 항목명, MFA·복구수단 확인란 작성
- 비밀번호·토큰 원문은 문서화하지 않고 Vercel 암호화 변수와 조직용 비밀번호 관리자에서만 관리하도록 기준 확정
- 로컬 비밀번호 내보내기·자격정보 메모 파일의 Git 제외 규칙 추가

### 산출물 및 반영시간
- `deliverables/operations/OPERATIONS_ARCHITECTURE.md`
- `deliverables/operations/SERVICE_CONFIGURATION_REGISTER.md`
- `deliverables/operations/ACCOUNT_ACCESS_REGISTER.md`
- 확인·작성·보안검토: 약 35분

## 2026-08-19 - 검정색 대시보드 CSS 사전 시안

### 요구내용
- 최신 제자리 서비스 와이어프레임을 참고하여 전체 CSS 변경 전 대시보드 시안을 먼저 확인
- 기존 보라색 중심 색상을 검정색 중심으로 변경
- 사용자 검토 후 수정사항을 받아 실제 화면에 반영

### 반영내용
- 와이어프레임 전체와 대시보드 영역을 렌더링하여 화면 구성 분석
- 보호자 인사, 주요 탭, 현재 상태, 대상자 목록, 실종신고·상품구매·내 정보 행동 구조를 시안에 반영
- 검정·흰색·중성 회색을 기본으로 하고 안전·주의·실종 상태에만 의미 색상 적용
- PC 화면과 390px 모바일 화면을 나란히 비교할 수 있는 독립 미리보기 제작
- 탭 선택과 관리대상 선택 반응까지 구현하여 단순 이미지가 아닌 동작 가능한 시안으로 구성

### 검증 및 결과
- PC·모바일 렌더링 정상
- 모바일 가로 넘침 없음
- 브라우저 콘솔 오류 없음
- 탭 전환 및 관리대상 선택 상태 정상

### 산출물 및 반영시간
- `deliverables/previews/BLACK_DASHBOARD_STYLE_PREVIEW.html`
- `deliverables/previews/BLACK_DASHBOARD_STYLE_PREVIEW.png`
- `deliverables/previews/BLACK_DASHBOARD_STYLE_PREVIEW_MOBILE.png`
- 분석·제작·검증: 약 25분
- 운영 소스와 Vercel에는 아직 반영하지 않은 검토용 시안

## 2026-08-19 - 검정색 테마 및 사용자 대시보드 구조 개편

### 요구내용
- 승인한 검정색 CSS 시안을 운영 서비스에 적용
- 대시보드 상단 3개 탭 제거
- 관리대상을 3명씩 표시하고 4번째부터 현재 상태 내부에서 가로 스와이프로 조회
- 마지막 대상 아래 `+` 버튼을 제공하여 관리대상정보 화면으로 이동
- 보호자정보 수정은 마이페이지 팝업의 링크로 통합

### 반영내용
- 공통 디자인 토큰, 포커스 효과, 브라우저 및 PWA 테마를 검정·흰색·중성 회색으로 변경
- 사용자 대시보드에 검정색 앱 헤더와 알림·마이페이지 아이콘 구성
- 상단 대시보드·보호자정보·관리대상정보 탭을 제거하고 편집 URL은 숨김 경로로 유지
- 편집 화면에서 대시보드로 돌아갈 수 있는 링크 추가
- 관리대상 목록을 3명 단위 `scroll-snap` 가로 스와이프로 구성
- 최대 4명 미만일 때 마지막 목록 아래 원형 `+` 버튼 표시
- 마이페이지의 보호자·주소·대상자 정보 수정 링크를 각 편집 화면에 연결
- 중복되던 관리대상 요약 패널 제거

### 검증 및 결과
- Next.js 프로덕션 빌드 통과
- 보안 회귀 검사 통과
- PC 및 390px 모바일 로컬 운영 화면에서 콘솔 오류와 가로 넘침 없음

### 산출물 및 반영시간
- `deliverables/USER_DASHBOARD_BLACK_THEME.md`
- 구현·반응형 보정·검증: 약 45분

### 배포결과
- GitHub `main` 소스 커밋 `e10ba48` 반영
- Vercel 운영 배포 `dpl_CCwZ8K3wtehLEkmuFUXYxyC91Z5f` READY
- `zezari.vercel.app` 별칭을 최신 운영 배포에 재연결
- `zezari.family`, `zezari.vercel.app`, `real-qr-find.vercel.app` HTTP 200, 검정 테마와 HSTS 확인
- 로그인된 운영 계정에서 상단 탭 제거, 관리대상 3명, 추가 버튼, 보호자·관리대상 편집 경로 확인
- 배포 직후 운영 오류 로그 없음

## 2026-08-19 - 외부 판매용 미배정 QR 가입·자동 매칭

### 요구내용
- 미배정 QR 화면에 일반 회원가입과 Google·카카오·네이버·Facebook 가입 버튼 제공
- 기존 휴대폰 인증과 보호자 정보 입력 유지
- 신규 회원은 관리대상을 최소 한 명 필수 등록
- QR 스캔으로 시작한 가입은 방금 스캔한 QR을 신규 대상자와 반드시 연결
- 일반 가입은 남은 미배정 QR을 기존 방식으로 자동 배정

### 반영내용
- 미배정 QR을 가입 시작 시점부터 2시간 동안 예약하는 서버 API와 HTTP 전용 쿠키 구현
- 요청자 원문 IP를 남기지 않는 해시 기반 10분당 10회 남용 방지 제한 적용
- 예약 토큰 원문 대신 SHA-256 해시와 만료시각만 Turso DB에 저장
- 일반 자동 배정에서 가입 예약 중인 QR을 제외하여 외부 판매 QR 선점 보호
- 일반·SNS 가입 완료 후 대상자 등록을 건너뛰지 못하도록 화면 흐름 변경
- 첫 신규 대상자 저장 시 스캔한 QR의 공개키·예약해시·만료시간을 원자적으로 검증하여 정확 매칭
- 만료 또는 중복 배정이면 임의 QR로 바꾸지 않고 재스캔 안내

### 검증 및 산출물
- 빌드, 보안 회귀, QR 예약·매칭 자동 테스트 통과
- 운영 DB 스키마 버전 39 및 예약 컬럼 생성 확인
- PC·375px 모바일에서 버튼 6개, 동일 폭, 가로 넘침 없음 확인
- `deliverables/QR_EXTERNAL_SALES_ONBOARDING.md`
- `scripts/qr-signup-claim-regression.mjs`
- 분석·구현·검증: 약 55분

### 배포결과
- GitHub `main` 커밋 `8e4ec29` 반영
- Vercel 운영 배포 `dpl_C8R2trr5mD6Wz9nLJpwFTeMoPNVU` READY
- `zezari.vercel.app` 최신 운영 배포 재연결
- `zezari.family`, `zezari.vercel.app`, `real-qr-find.vercel.app` HTTP 200 및 HSTS 확인
- 375px 운영 화면에서 가입 진입 버튼 6개, 가로 넘침 없음, 브라우저 오류 없음 확인

## 2026-08-19 - 관리자 지정 스토어 판매용 QR 선점

### 요구내용
- 관리자가 QR 관리 상세의 관리 탭에서 미배정 QR을 스토어 판매용으로 선점
- 배정된 QR의 선점 버튼은 비활성화
- 선점 QR은 해당 QR 페이지의 가입·로그인과 신규 대상자 필수 등록으로만 매칭
- 일반 가입 자동배정은 선점 QR을 사용하지 않음

### 반영내용
- 관리자 선점 상태와 선점일시를 DB 스키마 버전 40에 추가
- QR 목록·상세·이력·CSV에 스토어 선점 상태 표시
- 관리 탭에 `QR선점 - 스토어판매용` 및 선점 해제 기능 추가
- 배정·폐기·비활성 QR은 선점 버튼 비활성화
- 선점 QR은 관리자 수동 매칭과 일반 자동배정에서 제외
- 관리자 선점 QR에만 일반·SNS 가입 버튼과 2시간 가입자 임시 예약 제공
- 신규 대상자 저장 시 관리자 선점과 가입자 예약을 함께 검증한 뒤 정확한 QR을 매칭

### 검증 및 산출물
- 선점 전 예약 차단, 선점 QR 정확 매칭, 일반 배정 제외 자동 테스트 통과
- Next.js 전체 빌드와 보안 회귀 검사 통과
- 독립 로컬 DB 화면에서 미선점 QR 가입 버튼 0개, 선점 QR 가입 진입 버튼 6개 확인
- 375px 모바일에서 동일 버튼 폭, 가로 넘침 없음, 브라우저 오류 없음 확인
- `deliverables/QR_EXTERNAL_SALES_ONBOARDING.md` 갱신
- 분석·구현·검증: 약 40분

### 배포결과
- GitHub `main` 기능 커밋 `856e221` 반영
- Vercel 운영 배포 `dpl_7wxBumdJTvT7oHcMLktnLBdxYd4J` READY 및 `zezari.vercel.app` 재연결
- 운영 Turso 스키마 버전 40과 선점 컬럼 생성 확인
- 임시 선점 QR로 세 운영 도메인의 가입 진입 6개, HTTPS 응답, 가입 예약 성공 확인
- 테스트 QR을 즉시 미선점 상태로 복구하고 예약값 제거, 가입 버튼 미노출과 예약 거부 확인
- 테스트 과정에서 회원·대상자·주문·결제·영구 QR 매칭 데이터는 생성하지 않음

## 2026-08-20 - 이미지 업로드 관리 및 팝업 개선

### 요구내용
- 보호자·관리대상 사진 업로드 제한 확인 및 관리자 변경 기능
- 제한 초과 이미지 선택 시 사용자 알림
- 알림과 내정보 팝업 바깥 터치 닫기
- 내정보 열기 속도 개선
- 모바일 실종광고 팝업 닫기 버튼 세로 늘어짐 수정

### 반영내용
- 기존 관리대상 제한 1MB 확인, 기존에 없던 보호자 사진 업로드 추가
- 두 제한을 DB에서 각각 관리하며 기본 1MB, 설정 범위 1~4MB로 구성
- 관리자 왼쪽 메뉴에 `이미지업로드 관리`와 저장 화면 추가
- 파일 선택 즉시 제한 MB를 알림으로 안내하고 서버에서도 동일 값으로 재검증
- 보호자 사진을 로그인 사용자 전용 경로로 제공하고 내정보 사진에 표시
- 내정보를 전체 페이지 재조회 없이 즉시 여는 팝업으로 변경
- 전체 대상 QR 생성 작업을 필요한 대상만 생성하도록 축소
- 알림·내정보 바깥 터치 및 Esc 닫기 적용
- 광고 팝업의 grid 높이와 닫기 버튼 높이를 콘텐츠 기준으로 수정

### 검증 및 산출물
- Next.js 전체 빌드와 보안 회귀 검사 통과
- 독립 DB에서 스키마 41, 보호자 사진 컬럼, 두 1MB 기본값 확인
- 375px 기본 화면 가로 넘침 없음 확인
- `deliverables/IMAGE_UPLOAD_MANAGEMENT.md`
- 분석·구현·검증: 약 50분

## 2026-08-20 - 상품구매 진입 속도 개선

### 요구내용
- 대시보드에서 상품구매 화면을 여는 데 걸리는 3~5초 지연 원인 확인
- 기존 상품선택·쿠폰·결제 기능을 유지하면서 화면 전환 속도 개선

### 원인 및 반영내용
- 상품과 디자인 Base64 이미지 약 3.3MB가 초기 화면 데이터에 포함된 것을 확인
- 쿠폰 조회 과정의 보호자 데이터 중복 조회 확인
- 초기 화면에는 이미지 존재 여부와 버전만 전달하고 원본 이미지는 전용 URL로 지연 로드
- 보호자·관리대상·쿠폰을 한 번의 DB 배치로 통합
- 상품·디자인 메타데이터를 한 번의 DB 배치로 통합
- 화면 이동 즉시 표시되는 상품 로딩 화면 추가

### 검증 및 산출물
- 상품·디자인 초기 데이터 약 3.3MB에서 33KB로 감소
- 운영 DB 직접 계측 약 2.5초에서 약 0.44초로 감소
- 상품·디자인 이미지 API HTTP 200 확인
- Next.js 전체 빌드, 보안 회귀, 공백 검사 통과
- `deliverables/SHOP_LOADING_PERFORMANCE.md`
- 분석·구현·검증: 약 35분

## 2026-08-20 - 상품구매 운영 오류 및 Google 로그인 복귀 개선

### 요구내용
- 상품구매 버튼 클릭 시 발생하는 운영 서버 오류 수정 및 실사용 테스트
- 세션 종료 후 Google 로그인 과정에서 앱 로그인 화면이 한 번 더 보이는 듯한 흐름 개선

### 원인 및 반영내용
- 성능 개선용 상품 조회문이 운영 DB에 없는 우편번호 컬럼을 조회한 것이 서버 오류 원인임을 확인하고 제거
- SNS 로그인과 로그아웃의 기본 복귀 주소를 서비스 루트로 통일
- 동일 도메인 또는 내부 상대경로만 콜백으로 허용하고 잘못된 주소는 루트로 복귀하도록 서버 검증 추가
- NextAuth 기본 로그인 화면 대신 서비스 로그인 화면을 사용하도록 설정

### 검증 및 배포
- 전체 빌드, 보안 회귀, 공백 검사 통과
- GitHub `main` 커밋 `246ee0d` 반영
- Vercel 배포 `dpl_5KFMofPfVnHzo1XjzJNeR11g6kTE` READY 및 `zezari.family` 연결
- 운영 상품구매 화면 약 0.73초 내 표시, 상품·디자인 변경과 요약 반영 정상
- 운영 `/shop` 함수 요청 HTTP 200 확인
- 로그아웃 후 Google 버튼 1회 클릭과 계정 선택만으로 루트 대시보드 복귀, 추가 앱 로그인 버튼 및 내정보 팝업 재등장 없음
- 분석·수정·배포·운영 검증: 약 25분

## 2026-08-23 - Google 로그인 속도 및 세션 유지 개선

### 요구내용
- Google 로그인 지연 원인 파악 및 개선
- 로그인 후 짧은 시간 안에 다시 로그인해야 하는 현상 개선

### 원인 및 반영내용
- 미국 Vercel 함수와 도쿄 Turso DB 사이 장거리 통신을 확인하고 함수 실행 지역을 도쿄 `hnd1`로 변경
- 운영 기준 도메인을 `zezari.family`로 통일하고 기존 Vercel 주소는 자동 이동 처리
- JWT/세션 유지 기간을 활동 기준 90일로 설정
- 앱 실행, 6시간 주기, 장시간 후 화면 복귀·온라인 복귀 시 세션을 가볍게 갱신
- 명시적 로그아웃과 장기 미사용 만료는 그대로 유지

### 검증 및 배포
- 대시보드 표시 약 2.865초에서 약 0.568초로 개선
- Google 계정 선택 후 약 3.436초 내 대시보드 직접 복귀, 중간 앱 로그인 화면 없음
- 상품 화면 왕복 후 로그인 유지 확인
- 기존 `zezari.vercel.app` 접근 시 `zezari.family` 이동 및 로그인 유지 확인
- GitHub 기능 커밋 `327b7ab`, Vercel 배포 `dpl_8WbNSVKiNcvvdmvE1aVwcFxuVE9y` READY
- `deliverables/AUTH_SESSION_PERFORMANCE.md` 생성
- 분석·구현·배포·운영 검증: 약 40분

## 2026-08-24 - 개발·운영 계정 목록 점검

### 요구내용
- Google, Kakao, Facebook, GitHub, Vercel 등 제자리 개발과 운영에 필요한 로그인 정보 전체 나열

### 확인내용
- GitHub·Vercel·Turso·도메인, SNS OAuth 4종, 문자·결제·광고·안심번호·이메일 서비스 계정을 분류
- GitHub `zezariGit`, Vercel `zezari / zezari`, Turso 조직 `zezarigit` 등 확인된 식별자 재점검
- 공급자별 로그인 URL, 용도, 권한, 비밀번호 관리자 항목명을 계정 접근 대장에서 확인
- 비밀번호·API 키·토큰·MFA 복구코드는 문서와 로그에서 제외
- `deliverables/operations/ACCOUNT_ACCESS_REGISTER.md` 점검 요약 갱신
- 점검·문서화: 약 10분

## 2026-08-24 - 개발 계정 통합 전환 프로그램 설계

### 요구내용
- 제자리 종료 후 다른 프로젝트를 개발할 때 GitHub, Vercel, DB, Google, Codex 등 개발 계정을 한 번에 전환
- 구현 전 구조를 설계하고 쉽게 설명

### 설계내용
- 프로젝트마다 인증 저장소가 분리된 `개발 프로젝트 서랍` 방식 채택
- Codex `CODEX_HOME`, GitHub `GH_CONFIG_DIR`, Vercel global config, VS Code user-data와 브라우저 프로필을 프로젝트별 분리
- 최초 가입·MFA만 사용자가 진행하고 이후에는 `전환 후 열기` 한 번으로 개발환경 실행
- 토큰·쿠키 복사와 Windows 전역 환경변수 변경 금지
- GitHub 저장소, Vercel Project, 운영 DB가 다른 경우 Push·배포 차단
- PowerShell MVP 검증 후 .NET 8 WPF 단일 실행파일로 완성하는 단계 제안

### 산출물
- `deliverables/operations/DEVELOPMENT_PROFILE_SWITCHER_DESIGN.md`
- 이번 단계는 설계만 수행하고 실행 프로그램은 아직 구현하지 않음
- 조사·설계·문서화: 약 25분

## 2026-08-24 - 개발환경 계정 전환 프로그램 구현

### 요구내용
- 기존 개발 프로젝트는 기존 계정을 유지하고 새 프로젝트는 기존 계정 선택 또는 새 계정 생성
- Codex CLI 중심으로 최대한 간단하게 프로젝트 전환

### 반영내용
- 바탕화면 GUI, Windows EXE, `dev` 명령 설치
- 프로젝트와 재사용 가능한 계정 묶음 분리
- Codex, GitHub CLI, Git Push, Vercel, 브라우저 계정을 프로젝트 창별로 격리
- DB와 API는 프로젝트 `.env.local`을 우선하고 다른 터미널의 환경변수 혼입 차단
- 기존 프로젝트 자동 분석과 새 프로젝트/새 계정 묶음 생성 지원
- 비밀번호·토큰·환경변수 값을 프로필과 로그에 저장하지 않도록 처리

### 실제 적용
- `dev zezari`: 현재 ZEZARI 개발환경으로 실행
- `dev stock`: STOCK 전용 개인 개발환경으로 실행
- STOCK Git 작성자 `soonsuboy <soonsuboy10@gmail.com>` 로컬 적용
- STOCK GitHub·Vercel·Codex는 `선택 계정 연결`에서 최초 로그인 후 계속 재사용

### 검증 및 산출물
- 자동 회귀 테스트, EXE/GUI 실행, 프로필 분리, 비밀값 미저장 검증 통과
- `tools/dev-profile-switcher/`
- `deliverables/operations/DEVELOPMENT_PROFILE_SWITCHER_IMPLEMENTATION.md`
- 구현·설치·프로필 이관·검증: 약 45분

## 2026-08-24 - 개발환경 전환기 실제 실행 오류 보완

### 확인된 문제
- STOCK 실행 시 Git 인증 helper 중복 오류
- 일반 Windows Terminal에서 Microsoft Store Codex 내부 실행파일 탐색·실행 불가

### 개선내용
- Git helper를 실행마다 초기화하고 STOCK GitHub CLI 인증만 사용하도록 고정
- 공식 독립 Codex CLI `@openai/codex` 설치 및 설치기 자동 확인 추가
- 프로젝트별 Codex·GitHub·Vercel 경로를 시작 화면에 명시
- 대화형 화면을 막는 상위 `TERM=dumb` 환경값 제거

### 검증결과
- ZEZARI Codex 기존 로그인 유지
- STOCK Codex 별도 미로그인 상태 확인
- `dev stock` 실행 시 오류 없이 ChatGPT 계정 로그인 선택 화면 진입
- 분석·보완·실행 검증: 약 20분

## 2026-08-24 - STOCK 자식 프로세스 계정 완전 분리

### 요구내용
- STOCK Codex에서 제자리 Git·Vercel 계정이 표시되는 원인 확인 및 분리

### 반영내용
- STOCK 저장소의 실제 Git 작성자와 커밋 기록이 `soonsuboy <soonsuboy10@gmail.com>`임을 확인
- 계정별 전역 Git 설정 파일을 추가하여 `git config --global`도 STOCK 계정으로 표시
- Codex 내부에서 실행되는 Vercel 명령에도 STOCK 전용 인증만 전달
- STOCK Vercel 미로그인 상태에서 `zezarigit` 계정으로 fallback하는 경로 차단
- 실행할 때 프로젝트 진단정보를 자동 갱신하도록 개선

### 검증결과
- STOCK 전역·유효 Git 작성자 모두 개인계정으로 확인
- STOCK Codex 개인계정 로그인 유지 확인
- GitHub·Vercel은 최초 로그인 필요 상태로 분리되어 제자리 계정을 사용하지 않음
- 구현·설치·검증: 약 25분

## 2026-08-24 - ZEZARI 개발계정 로그인 재확인

### 요구내용
- 현재 개발환경이 STOCK이 아닌 제자리 계정으로 연결되어 있는지 확인

### 확인 및 반영
- 작업 폴더와 GitHub 저장소가 ZEZARI 프로젝트임을 확인
- Git 작성자 `zezariGit <general@zezari.com>` 확인
- GitHub CLI를 `zezariGit`으로 재로그인
- Vercel `zezarigit`, Codex `soonsuboy10@gmail.com`, Turso 연결정보 확인
- GitHub 원격 저장소 접근 성공 확인
- 확인·재연결: 약 5분

## 2026-08-24 - 대상자 정보 등록 화면 문구 정리

### 요구내용
- 대상자 신규 등록 화면의 기존 중복 제목과 안내 문구를 제거하고 제공된 문구로 교체

### 반영내용
- 제목을 `대상자 정보 등록`으로 통일
- 보호 대상 정보 등록 목적과 QR 스캔 시 활용 목적을 두 줄로 안내
- `보호자 대시보드`, `관리대상정보`, `대상자 등록`, `신규` 등 중복 표현 제거
- 수정 화면은 `대상자 정보 수정`으로 구분
- 구현·검증: 약 10분

## 2026-08-24 - 대상자 등록 입력 편의 및 필수값 강화

### 요구내용
- 성별 기타 및 현재 상태 선택 제거
- 사진 미리보기, 사진과 이름의 가로 배치, 생년월일 연·월·일 선택 적용
- 사진·이름·생년월일·성별·보호자 메시지 필수화와 누락 항목 안내
- iOS 입력칸 너비 불일치 개선 및 안내 문구 교체

### 반영내용
- 사진 선택 즉시 미리보기와 `변경하기` 문구가 표시되도록 개선
- 업로드 전에는 `사진 등록하기`와 제한 용량을 겹침 없이 표시
- 필수 항목 누락 시 항목명을 한 번에 보여주는 경고창 추가
- 서버 저장 단계에서도 동일 필수값을 재검증하여 우회 입력 차단
- 신규 상태는 내부 기본값, 수정 상태는 기존값을 유지하도록 처리
- 빌드·보안 검사 완료, 구현·검증 약 25분

## 2026-08-25 - 대상자 등록 완료 화면 QR 보호

### 요구내용
- 등록 완료 화면에서 실제 QR 이미지와 QR 코드값을 숨기고 재사용할 수 없는 아이콘으로 교체
- 완료 화면의 기존 헤더·안내·뒤로가기 제거 및 문구 변경

### 반영내용
- 등록 완료 대상은 실제 QR 이미지 생성 대상에서 제외
- URL이나 코드값이 없는 CSS 장식용 QR 아이콘 적용
- 상단 제목을 `대상자 등록 완료`로 구성하고 완료 전용 간결한 화면 적용
- `대상자 등록이 완료되었습니다`, `이제 대상자 전용 QR이 적용된 상품을 구매할 수 있어요.` 문구 적용
- 상품 구매와 대시보드 이동 버튼에 아이콘 적용
- 빌드·보안 검사 완료, 구현·검증 약 15분

## 2026-08-25 - 모바일 앨범·파일 선택기 분리 보강

### 요구내용
- Android에서 앨범은 사진 선택기, 파일은 내 파일 선택기로 바로 연결
- iPhone에서도 지원 범위 안에서 동일한 흐름 제공

### 반영내용
- 앨범 입력을 단일 `image/*` 유형으로 정리하여 Android Chrome 사진 선택기 조건 적용
- 지원 브라우저에서는 File System Access API로 문서 선택기를 직접 호출
- iOS 등 미지원 브라우저에는 표준 이미지 파일 선택기를 호환 경로로 유지
- 카메라 직접 촬영, 미리보기, 용량 제한, 기존 저장 로직 유지
- 빌드·보안 검사 완료, 구현·검증 약 15분

## 2026-08-25 - 대상자 사진 업로드 방식 선택

### 요구내용
- 대상자 사진 선택 시 사진 촬영 앱, 앨범 앱, 파일 앱의 세 가지 경로 제공

### 반영내용
- 사진 영역을 누르면 모바일 하단 선택 메뉴가 열리도록 개선
- 카메라 직접 촬영, 앨범 이미지 선택, 파일 이미지 선택 입력을 각각 분리
- 실제로 선택된 파일 하나만 기존 사진 저장 로직으로 전달하여 DB 구조와 서버 검증 유지
- 사진 미리보기, 변경하기, 제한 용량 경고, 필수 사진 검증 기능 유지
- 빌드·보안 검사 완료, 구현·검증 약 15분

## 2026-08-25 - 대시보드 대상자 상태 및 수정 동선 통합

### 요구내용
- 마이페이지 아이콘을 톱니바퀴로 변경
- 대상자별 상태를 상품구매필요, QR활성화필요, 안전, 찾는중 중 하나만 표시
- 카드에서 QR·광고상태를 제거하고 대상자 미리보기에서 수정하도록 동선 통합
- 마이페이지 대상자 메뉴 제거 및 알림 새로고침 아이콘 적용

### 반영내용
- 기존 4단계 DB 상태와 구매·QR 활성화 전환 구조를 확인하고 실제 구매·QR 데이터를 함께 반영하는 단일 상태 우선순위 적용
- 대상자 카드에는 이름, 생년월일, 상태 한 개만 표시하고 카드 전체를 보호자 전용 미리보기 링크로 변경
- 보호자 미리보기에서 사진, 기본정보, 보호자 메시지, 음성을 확인한 뒤 `수정하기`로 기존 정보가 채워진 등록 폼에 진입
- 공개 QR 페이지에는 수정 기능을 추가하지 않아 발견자 화면과 보호자 화면의 권한 경계를 유지
- 마이페이지의 대상자 목록·추가·수정 영역 삭제, 톱니바퀴와 새로고침 아이콘 적용
- 빌드, 보안, QR 가입 회귀 및 로컬·운영 브라우저 렌더링 검사 완료
- 운영 대상자 8명 카드의 상태 단일 표시, 미리보기·수정 기존값, 마이페이지·알림·공개 QR 권한 경계 확인
- 첫 운영 캡처에서 발견한 확인 버튼 정렬 문제를 즉시 보완하고 `https://zezari.family`에 재배포
- 구현·문서화·검증: 약 30분

## 2026-08-25 - 초록색 테마 및 대시보드 상태 액션

### 요구내용
- 검정·흰색 중심 화면을 초록색 중심 팔레트로 변경
- 하단 중복 `내 정보`를 카카오톡 채널 `고객지원`으로 교체
- 상품구매필요·QR활성화필요 상태 배지에서 해당 처리 화면으로 이동
- 알림 상세의 `관련 화면 열기` 버튼 제거

### 반영내용
- 공통 디자인 토큰, 버튼, 강조 아이콘, 로그인·온보딩·관리자 선택 강조색과 PWA 테마 색상을 초록색으로 통일
- 상단 마이페이지 톱니바퀴는 유지하고 하단 고객지원 버튼은 상담 아이콘과 외부 새 창 카카오톡 링크로 구성
- 대상자 카드의 미리보기 영역과 상태 액션 영역을 분리하여 클릭 목적을 명확하게 개선
- 상품구매필요는 해당 대상자가 미리 선택된 상품 페이지, QR활성화필요는 해당 대상자 QR 활성화 페이지로 연결
- 안전·찾는중 배지는 정보 표시만 유지
- 알림 목록의 중복 범용 이동 버튼과 라우팅 헬퍼 제거, 메시지 본문의 카카오맵 등 명시적 링크는 유지
- 모바일 운영 화면에서 초록 테마, 3개 퀵 메뉴, 상태 단일 표시, 페이지 점 표시와 글자 넘침 없음 확인
- 상품구매필요 배지에서 해당 대상자가 자동 선택된 상품 화면, 정상 QR활성화필요 배지에서 QR 활성화 버튼 노출 확인
- 관리자 비활성 QR은 기존 보안 안내를 유지하고 안전·찾는중은 클릭되지 않는 정보 배지로 유지
- 알림 팝업의 관련 화면 열기 버튼 제거 확인, 스와이프용 네이티브 스크롤바는 숨기고 페이지 점은 유지
- GitHub 및 `https://zezari.family` 운영 배포 완료, 구현·검증 약 30분

## 2026-08-25 - 대시보드 이미지형 메뉴 및 안전 안내 배너

### 요구내용
- 인사말 아래 보호자 안내 문구 제거
- 현재 상태 제목 아래 가로선과 대상자 카드 왼쪽 강조선 제거
- 첨부 이미지 그대로 실종신고, 상품구매, 고객지원 버튼 구성
- 대상자 목록과 하단 버튼 사이에 제자리 안전 안내 이미지 배치

### 반영내용
- 가입 완료 보호자 대시보드에서만 설명 문구를 숨기고 다른 정보입력 안내문은 유지
- 현재 상태 구분선, 카드 왼쪽 초록선, 카드 음영·이동 효과를 제거하여 평면형 목록으로 정리
- 전달받은 이미지 4개를 프로젝트 정식 자산으로 등록하고 원본 비율을 유지해 표시
- 기존 화면 이동과 카카오톡 새 창 연결, 접근성용 버튼 이름은 그대로 유지
- 빌드 및 공백 검사 완료, 구현·초기 검증 약 15분

## 2026-08-25 - 대시보드 하단 메뉴 아이콘·텍스트 분리

### 요구내용
- 실종신고, 상품구매, 고객지원 버튼의 새 아이콘 이미지를 사용
- 이미지에 포함된 글자 대신 실제 화면 텍스트로 버튼명을 표시

### 반영내용
- 새로 전달된 그림 전용 이미지 3개로 정식 자산 교체
- 각 메뉴를 아이콘과 실제 텍스트로 분리하고 기존 이동 경로 유지
- 모바일·데스크톱에서 세 메뉴의 높이, 아이콘 크기, 글자 정렬이 일정하도록 반응형 스타일 적용
- 구현·초기 검증 약 10분

## 2026-08-25 - 대시보드 하단 메뉴 글자 강조

### 요구내용
- 메뉴 글자 뒤의 원형 배경 제거
- 메뉴명을 `실종신고`, `상품구매`, `고객지원`으로 표시
- 글자 크기를 기존보다 약 1.8배 확대

### 반영내용
- 과거 아이콘용 공통 `span` 규칙이 메뉴 글자에 적용되던 충돌을 해소
- 라벨의 원형 크기, 배경색, 테두리를 제거하고 실제 텍스트만 표시
- 데스크톱 27px, 모바일 25px로 확대하고 기존 링크 기능 유지
- 구현·초기 검증 약 5분

## 2026-08-25 - 신규 카카오 계정 로그인 연동

### 요구내용
- 새로운 카카오 개발자 계정에서 제자리 운영 서비스의 카카오 로그인을 전체 설정
- `zezari.family`에서 카카오 가입·로그인이 가능하도록 키와 콜백을 배포

### 반영내용
- 새 카카오 개발자 앱 `ZEZARI` 생성 및 카카오 로그인 활성화
- 운영 콜백 `https://zezari.family/api/auth/callback/kakao` 등록
- 닉네임 필수 동의와 Client Secret 활성 상태 구성
- 새 REST API 키와 Client Secret을 로컬 및 Vercel 운영·개발 환경에 안전하게 교체
- 비밀값은 Git과 산출물에서 제외하고 비민감 설정만 인증 산출물에 기록
- 운영 로그인 화면에서 실제 카카오 동의와 콜백을 완료하고 SNS 신규가입 휴대폰 인증 화면 진입 확인
- 빌드·보안 검사 통과, GitHub 푸시 및 `zezari.family` 운영 배포 완료
- 설정·배포·운영 검증 약 35분

## 2026-08-25 - 상품 결제 배송지·쿠폰·뒤로가기 개선

### 요구내용
- 배송지 기본주소는 주소 검색 결과로만 입력하고 상세주소는 별도 입력
- 배송지 아래 쿠폰 보유 수와 선택 상태를 첨부 화면 형태로 표시
- 쿠폰 선택 또는 휴대폰 뒤로가기 시 결제 흐름을 벗어나는 현상 개선

### 반영내용
- 결제 화면의 기본주소를 읽기 전용으로 제한하고 카카오 주소 검색 팝업과 연결
- `할인쿠폰`, 보유 수, 선택 내용과 화살표를 갖춘 쿠폰 행 및 선택 팝업 구현
- 쿠폰 팝업과 상품 설정·미리보기·결제 단계별 브라우저 히스토리를 구성해 뒤로가기 순서를 보존
- 서버 쿠폰 유효성 및 최종 결제금액 재검증 구조 유지
- 운영에서 주소 팝업, 읽기 전용 기본주소, 상세주소 입력 분리 확인
- 3,000원 쿠폰 적용 시 주문요약과 Toss 결제금액이 5,000원에서 2,000원으로 함께 변경되는 것 확인
- 팝업 상단 및 브라우저 뒤로가기 모두 쿠폰을 유지한 채 결제 화면으로 복귀하는 것 확인
- GitHub 푸시 및 `zezari.family` 운영 배포 완료, 구현·배포·검증 약 35분

## 2026-08-25 - 상품·12간지 이미지 선택 적용

### 요구내용
- 상품구매 화면의 상품과 디자인 텍스트 셀렉트박스를 이미지 선택 방식으로 변경
- `reference/icon`의 상품 7종과 12간지 12종 이미지 사용

### 반영내용
- 상품 7종·12간지 12종 이미지를 운영용 정식 자산으로 등록
- 선택 행을 누르면 4열 이미지 그리드가 열리고 현재 항목에 체크가 표시되는 UI 구현
- 닫기, 바깥 영역 터치, Esc 닫기와 접근성용 선택 상태 추가
- 기존 상품·디자인 ID, 관리자 가격·상세페이지, 수량·쿠폰·결제 연동 유지
- 운영 상품구매 화면에서 상품 7종과 12간지 12종 이미지 표시 확인
- `키링 + 호랑이` 선택 후 주문 요약과 상품 미리보기까지 값·수량·가격이 정상 유지되는 것 확인
- 빌드·보안·공백 검사, GitHub 푸시 및 `zezari.family` 운영 배포 완료
- 구현·배포·운영 검증 약 35분

## 2026-08-25 - 상품 선택 이미지 가독성 개선

### 요구내용
- 선택 행 상품 아이콘 아래에 원본 이미지 글자가 일부 보이는 현상 제거
- 복합상품 3종 이미지를 단일상품보다 넓게 표시

### 반영내용
- 선택 행에서는 원본 하단 글자를 잘라내고 상품 아이콘만 표시하도록 개선
- 단일상품 4종은 첫 행 4열, 복합상품 3종은 다음 행 3열의 넓은 칸으로 구성
- 기존 상품 식별값·가격·주문 흐름과 12간지 4열 구성 유지
- 운영 화면에서 하단 글자 제거와 복합상품 확대 배치를 시각적으로 확인
- 빌드·보안·공백 검사, GitHub 푸시 및 `zezari.family` 운영 배포 완료
- 구현·배포·운영 검증 약 20분

## 2026-08-25 - 상품 결제 흐름·QR 정보 보호·주문완료 개선

### 요구내용
- 상품 선택 다음에 표시되는 미리보기 화면 생략
- 보호자 대상자 수정 화면에서 QR 이미지·코드값·링크 제거
- 제공된 주문완료 아이콘 4개로 완료 화면 재구성

### 반영내용
- 상품·디자인 선택 후 바로 배송지·쿠폰·결제수단 입력 화면으로 이동
- 보호자 수정 화면에서는 대상자 정보와 음성만 관리하고 QR 원본 정보는 노출하지 않도록 변경
- 상품 수령, QR 스캔, 대상자 확인, 활성화 완료 순서를 제공 이미지로 구성
- 상품 결제 성공 경로 두 곳을 동일한 주문완료 화면으로 통합
- 운영 상품구매 화면에서 미리보기 없이 결제 화면으로 바로 이동하는 흐름 확인
- 운영 대상자 수정 화면에서 음성은 유지되고 QR 이미지·코드값·링크는 노출되지 않는 것 확인
- 운영 주문완료 화면에서 제공 이미지 4개와 안내·대시보드 이동 버튼이 정상 표시되는 것 확인
- 빌드·보안·공백 검사, GitHub 커밋 `9be2f79` 푸시 및 Vercel 배포 `dpl_DRtBnjYMucfYQQkHZ9jcTm3i9tem`의 `READY` 상태 확인
- 구현·배포·운영 검증 약 35분

## 2026-08-25 - SNS 계정 연결 및 가입 경험 정리

### 요구내용
- 기존 회원이 Google·Kakao·Naver·Facebook 중 다른 로그인 수단을 추가로 사용할 수 있게 개선
- 대표번호 인증문자, 마이페이지 팝업, 보호자 사진, PWA 안내, 가입 예시 문구 정리

### 반영내용
- 휴대폰 번호 중복 감지 후 사용자 동의와 SMS 재인증을 거쳐 동일 보호자 계정에 SNS를 연결하는 절차 구현
- 보호자·관리대상·주문·구독·광고 데이터는 하나의 보호자 계정에 그대로 유지
- 이메일만으로 자동 연결하지 않고 15분 유효·1회 사용 인증 토큰으로 소유권 확인
- 기존 SNS 회원 데이터도 새 연결 테이블에 자동 이관되는 스키마 적용
- 마이페이지 정보수정 시 팝업 닫기, 보호자 사진 업로드 숨김, 설치 안내 문구 삭제, 가입 예시 변경
- Solapi 발신번호를 제자리 대표번호 `1668-1290`으로 변경
- 격리 SNS 연결 회귀, QR 가입 회귀, 보안 검사, 운영 빌드 통과
- GitHub 커밋 `db9eea6`, `90d0e14` 푸시 및 Vercel 배포 `dpl_7pA6mZaGfpu6ZHFxDZR8U3XpjEvF` 운영 반영 완료
- 운영에서 정보수정 팝업 닫기·보호자 정보 이동, 사진 업로드 숨김, PWA 문구 삭제, 콘솔 오류 없음 확인
- 운영 DB 스키마 42와 기존 SNS 식별자 13건 이관 확인
- 실제 계정 연결과 대표번호 문자 수신은 운영 데이터 변경을 피하기 위해 자동 테스트하지 않고 격리 회귀로 검증
- 구현·배포·운영 검증 약 60분

## 2026-08-25 - 기존 회원 확인 후 인증번호 발송 개선

### 요구내용
- 인증코드 요청 시 기존 회원 여부를 먼저 판단하고 계정 연결 동의 후 문자 발송

### 반영내용
- 회원 확인 요청과 실제 SMS 발송 요청의 제한 카운터 분리
- 회원 확인은 5분 10회·1시간 30회, 실제 발송은 5분 3회·1시간 10회로 운영
- 같은 전화번호·인증 목적의 실제 발송은 1시간 5회 제한 유지
- SNS 연결 한 번이 문자 발송 횟수 한 번만 소비하도록 개선
- 기존 회원 확인 세 번 후에도 문자 발송·인증·계정 연결이 완료되는 회귀 테스트 통과
- QR 가입·보안 검사·운영 빌드 통과, 구현 및 로컬 검증 약 25분
- GitHub 커밋 `ee8a842` 및 Vercel 배포 `dpl_SfDZFDtphbCQfqbDgbLVw8X4S3Aq` 운영 반영, `zezari.family` HTTP 200 확인
- 전체 구현·배포·검증 약 35분

## 2026-08-25 - SOLAPI 인증문자 장애 개선
- 요구: 잔여 발송 한도가 있는데 신규 번호 인증문자가 실패하는 원인 분석 및 개선
- 원인: 대표번호 `1668-1290`이 SOLAPI 발신번호로 등록되지 않아 상태코드 `1062`로 접수 실패
- 확인: 동일 시간대 SOLAPI 발송 이력 3건 모두 실패, 기존 등록 발신번호는 한 개만 활성 상태
- 조치: 대표번호 승인 전까지 등록된 발신번호로 임시 복구하고 공급자 실패코드 판별·안전한 운영 로그·구체적인 오류 안내 추가
- 검증: SMS 공급자, SNS 연결, QR 가입, 보안 회귀 테스트와 Next.js 운영 빌드 통과
- 운영 확인: 배포 `dpl_BDRETYBCfp6Tx6wQKVwGkofbiTyB` READY, 운영 API 성공, SOLAPI 성공 1·실패 0 확인
- 일일 발송 한도 상향 500건 반영 확인
- 소요시간: 약 45분

## 2026-08-25 - 온보딩·QR 관리·활성화 UI 단순화
- 로그인 전 서비스 소개의 이전/다음 버튼을 제거하고 `로그인하기` 버튼 하나로 정리
- 서비스 소개 3페이지의 스와이프와 페이지 점 탐색은 유지
- 관리자 QR 관리의 활성/미활성 필터·열·상세 배지를 숨기고 대표 상태를 한 개만 표시
- 상품 수령 후 활성화 화면을 큰 사진 위, 대상자 정보 아래의 모바일 세로형 카드로 변경
- 일반 로그인과 스토어 선점 QR 가입 화면에서 Facebook 버튼 제거
- 기존 Facebook 연결 계정 데이터와 서버 호환 설정은 유지
- 운영 빌드·보안·QR 선점 가입·SNS 계정 연결 회귀 검사 통과
- 로컬 브라우저에서 온보딩 단일 버튼과 Kakao·Naver·Google 3개 로그인 버튼 확인
- GitHub 커밋 `0a95abc` 푸시 및 Vercel 배포 `dpl_F5feL3JFSnKLiKLtevNr7wgRRri1` 운영 반영 완료
- 운영에서 QR 상태 단일 표기와 대상자 세로형 활성화 카드 확인, 두 운영 도메인 HTTP 200 확인
- 구현·배포·운영 검증 약 35분

## 2026-08-26 - 관리대상 사진 업로드 플랫폼별 옵션 개선
- iOS: 사진 촬영 앱·앨범 앱·파일 앱 3개 옵션 제공
- Android·웹: 동작하지 않는 앨범 앱을 제거하고 사진 촬영 앱·파일 앱 2개 옵션 제공
- iPadOS 데스크톱 모드도 터치 여부를 확인해 iOS 흐름으로 처리
- 등록과 수정 화면이 공유하는 사진 컴포넌트에 한 번에 적용
- 사진 제한 용량·미리보기·필수값 검증은 기존 동작 유지
- 운영 빌드·보안·QR 선점 가입 회귀 검사 통과
- GitHub 커밋 `4c4b6d8` 푸시 및 Vercel 배포 `dpl_93CuTh7PhkhdbecV8oJUVfzfDqWu` 운영 반영 완료
- 운영 일반 웹에서 촬영·파일 두 옵션만 표시되고 앨범 옵션이 제거된 것을 확인
- 구현·배포·운영 검증 약 25분

## 2026-08-26 - 온라인 실종 광고 결제 화면 정보 단순화
- 보호자 광고 미리보기와 결제 화면에서 Meta 집행예산 및 서비스 마진 정보를 제거
- 보호자에게는 광고 기간, 거리, 최종 결제금액과 예상 노출수만 안내
- 결제 클라이언트 데이터에서도 내부 예산·마진 필드를 제외해 노출 경계를 강화
- 관리자·서버의 예산 계산과 Meta 자동 광고 발행 기능은 기존대로 유지
- Next.js 운영 빌드·보안 회귀·공백 검사 통과
- GitHub 커밋 `bbefa50` 푸시 및 Vercel 배포 `dpl_DAykHUd7CKidMxdadKYAAfnNHJDp` 운영 반영 완료
- 운영 결제 화면에서 내부 예산·마진 비노출과 Toss 결제위젯 정상 로드를 확인
- 대표 도메인 HTTP 200 및 기존 Vercel 주소의 대표 도메인 리디렉션 확인
- 구현·배포·운영 검증 약 30분

## 2026-08-26 - SOLAPI 대표 발신번호 전환
- SOLAPI에서 제자리 대표번호 `1668-1290`의 활성화·번호인증 완료 및 2027-08-26 만료 상태 확인
- 기존 개인 발신번호는 비활성화 상태 확인
- 로컬과 Vercel Production·Development 발신번호를 `16681290`으로 교체
- 회원가입, SNS 계정 연결, 보호자 연락처 변경 인증에 공통 적용
- SMS 공급자 회귀, Next.js 빌드, 보안 검사 통과
- GitHub 커밋 `d01f13d` 및 Vercel 배포 `dpl_6DmvCjd4cGoz95YPpLf5VKGad8PS` 운영 반영 완료
- 기존 오류 원인은 비활성 개인번호를 포함한 이전 배포의 SOLAPI `1062` 응답으로 확인
- 대표 도메인이 새 배포를 가리키고 인증 API 입력 검증이 정상 동작함을 확인
- 실제 단말 수신·대표 발신번호 표시는 사용자 재요청으로 최종 확인 예정
- 승인 확인부터 운영 배포까지 약 35분

## 2026-09-04 - 전 화면 초록 디자인 시스템 표준화
- 기존 대표색 `#26963F`와 `Noto Sans KR`을 서비스 공통 기준으로 확정
- 배경·표면·텍스트·비활성·성공·경고·오류·정보 색상을 전역 CSS 토큰으로 통합
- 카드·입력창 모서리를 4px·8px로 통일하고 일반 카드 그림자와 장식용 그라데이션 최소화
- 버튼·링크·입력창의 키보드 포커스를 초록색 외곽선으로 통일
- 로그인·온보딩·대시보드·위치보안·실종신고·관리자 UI의 기존 색상 편차를 공통 토큰에 연결
- 외부 SNS 제공자의 공식 브랜드 색상은 로그인 식별성을 위해 유지
- 모바일·데스크톱 브라우저 렌더링, Next.js 빌드, 보안·QR 회귀, CSS 토큰 검사 통과
- WCAG 2 A/AA 자동 검사 위반 0건
- GitHub 기능 커밋 `7d91f1b`, Vercel 기능 배포 `dpl_27X4woZSGZfAqoerxLz3jwgPjsir` 운영 반영 완료
- 구현·문서화·배포·운영 검증 약 45분

## 2026-09-04 - 설정 명칭 및 푸시 알림 자동 복구
- 우측 상단 톱니바퀴 팝업의 `내 정보` 명칭을 `설정`으로 변경
- 운영 로그인 유지기간이 90일임을 확인해 하루·이틀 뒤 알림 중단이 로그인 만료 때문이 아님을 확인
- 브라우저가 푸시 주소를 갱신하거나 폐기했을 때 서버의 기기 연결이 복구되지 않던 원인 확인
- 앱 실행, 화면 복귀, 인터넷 복귀, 서비스 워커 교체, 최초 연결 및 6시간 주기로 현재 기기 연결을 자동 복구
- 브라우저가 지원하는 경우 푸시 주소 변경 시 서비스 워커가 즉시 새 주소를 저장
- 개인화 화면과 API를 서비스 워커 캐시에서 제외하고 최신 워커를 항상 확인하도록 변경
- 푸시 자동 복구·보안·QR 회귀검사와 Next.js 운영 빌드 통과
- 운영체제에서 알림 권한을 끄거나 앱·브라우저 데이터를 삭제한 경우에는 사용자가 다시 권한을 허용해야 함
- GitHub 기능 커밋 `7dd3070` 푸시 및 Vercel 배포 `dpl_B9DLDcYLT78MibCo3zXsX7epCeme` 운영 반영 완료
- 운영 로그인 화면에서 톱니바퀴·팝업 제목·닫기 라벨이 모두 `설정`으로 표시되고 브라우저 오류 0건 확인
- 운영 서비스 워커 `zezari-v17`, 캐시 금지 헤더, 푸시 서버 키 구성 및 대표 도메인 HTTP 200 확인
- 구현·문서화·배포·운영 검증 약 45분

## 2026-09-05 - 제자리 앱·푸시 알림 아이콘 교체
- 제공한 검정 `제자리` 워드마크를 흰색 정사각형 앱 아이콘으로 제작
- Android 일반·마스커블 아이콘, Apple touch icon, 브라우저 바로가기 아이콘을 새 버전 경로로 적용
- 안드로이드 푸시 알림의 큰 아이콘과 투명 단색 배지를 제자리 워드마크로 교체
- 기존 설치 앱이 이전 초록 `Z` 이미지를 계속 캐시하지 않도록 아이콘 URL과 서비스 워커를 `zezari-v18`로 갱신
- 삼성 인터넷 일반 탭으로 받은 알림의 왼쪽 삼성 인터넷 마크는 운영체제가 표시하는 브라우저 발신자 아이콘이라 사이트에서 강제로 교체할 수 없음을 확인
- 설치된 제자리 앱에서는 새 매니페스트 아이콘을 앱 아이콘으로 사용하도록 반영
- 이미지 크기·투명도·매니페스트·푸시 경로 회귀검사, 보안·QR 회귀검사 및 Next.js 운영 빌드 통과
- GitHub 기능 커밋 `00c73fe` 푸시 및 Vercel 배포 `dpl_4QXikeF5XT8k7XfeBY9QhRRJMFn5` 운영 반영 완료
- 운영 홈·매니페스트·서비스 워커·아이콘 6개 HTTP 200 확인
- 운영 아이콘의 크기와 파일 해시가 커밋 원본과 모두 일치하고 Vercel 오류 로그 0건 확인
- 구현·문서화·배포·운영 검증 약 40분

## 2026-09-05 - 관리자 그리드 행 전체 선택 통일
- 오른쪽 상세 패널이 있는 관리자 목록을 전수 조사해 선택 방식을 통일
- 결제·쿠폰 목록은 마지막 상세보기 칸 외에도 번호·이름·상태 등 어느 셀을 눌러도 상세 갱신
- 상품 목록은 맨 앞 선택 버튼뿐 아니라 썸네일·상품명·가격·상태 등 행 전체에서 선택 가능
- 상품 행에 키보드 Enter·Space 선택과 초록색 포커스 표시 추가
- 상세 패널이 없는 요약·감사 표와 빈 행은 잘못된 이동을 막기 위해 비선택 상태 유지
- 관리자 그리드 회귀검사, 보안·푸시 회귀검사, Next.js 운영 빌드 통과
- GitHub 기능 커밋 `bbb3e30` 푸시 및 Vercel 배포 `dpl_6MS6zX2UX6c6Mj5FqmVfqRKN9KeU` 운영 반영 완료
- 운영 관리자 계정으로 결제 거래구분 셀, 쿠폰 코드 셀, 상품 가격 셀 클릭 시 URL 선택값과 오른쪽 상세정보 동시 갱신 확인
- 운영 상세형 메뉴의 실제 데이터 행 전체 클릭 구조 전수 확인
- 대표 관리자 URL HTTP 200 및 Vercel 운영 오류 로그 0건 확인

## 2026-09-06 - 관리자 회원가입 인증문자 재테스트 예외
- 이미 가입된 일반 회원 번호의 회원가입 인증문자 차단은 그대로 유지
- 활성 관리자 번호만 회원가입 화면의 `인증코드받기`로 인증문자를 다시 받을 수 있도록 예외 처리
- DB 관리자 권한 또는 설정된 관리자 이메일에 해당하는 활성 계정만 허용
- 관리자도 인증 후 실제 중복 회원가입은 할 수 없도록 최종 가입 단계의 중복 차단 유지
- SNS 계정 연결·연락처 변경 흐름과 요청자·번호별 발송 제한은 기존대로 유지
- 관리자 예외·일반 회원 차단·중복 가입 차단·SNS 연결·문자 공급자·보안 회귀검사 및 Next.js 운영 빌드 통과
- 운영 관리자 역할 계정과 등록 전화번호 존재 여부를 개인정보 출력 없이 확인
- GitHub 기능 커밋 `f29b0b7` 푸시 및 Vercel 배포 `dpl_4fjNC7MHCbRxDqCkVHhtY1PaTiGY` 운영 반영 완료
- 자동 검증 중 실제 관리자 인증문자는 발송하지 않음

## 2026-09-07 - 휴대폰 인증 비밀번호 찾기 구현
- 로그인 화면의 준비중 안내를 실제 `비밀번호 찾기` 기능으로 교체
- 휴대폰 번호 입력 시 하이픈 자동 적용 및 유효한 번호에서만 `인증코드 받기` 활성화
- 6자리 인증번호 입력, 3분 타이머, 6자리 완성 후 `확인` 활성화 구현
- 인증번호 재전송 시 이전 코드와 이전 인증 토큰을 모두 무효화하고 타이머 초기화
- 인증 성공 후 같은 화면에서 새 비밀번호와 확인 입력 영역으로 전환
- 영문·숫자·특수문자·8자 이상 조건과 비밀번호 일치 여부를 입력 중 표시
- 변경 완료 후 아이디를 채운 로그인 화면으로 복귀
- 비밀번호 재설정 전용 API 3개, 15분 일회용 토큰, 관리자·SNS·가입 인증과 분리된 목적값 적용
- 비밀번호 재설정·문자 공급자·관리자 인증·SNS 연결·보안 회귀검사 및 Next.js 운영 빌드 통과
- 로컬 390px와 운영 브라우저에서 버튼 상태·레이아웃·오류 없음 확인
- GitHub 기능 커밋 `38d628f` 푸시 및 Vercel 배포 `dpl_8UXzr7YeUzqa3WCe6mHEVVbZrQ6k` 운영 반영 완료
- 운영 검증 중 실제 인증문자 발송이나 비밀번호 변경은 수행하지 않음

## 2026-09-07 - 서비스 소개 3단계 화면 개편
- 제공된 서비스 소개 이미지 3장을 기준으로 그림·문구·여백을 모바일 화면에 맞춰 재구성
- 이전·다음 버튼을 제거하고 좌우 스와이프만으로 1·2·3단계 이동하도록 변경
- 첫 단계와 마지막 단계에서 추가 스와이프가 범위를 벗어나지 않도록 제한
- 현재 단계 표시점 3개를 유지하고 활성 표시점을 초록색으로 즉시 동기화
- 화면별 콘텐츠 높이와 관계없이 표시점 위치를 모두 `742px`로 통일
- `로그인하기`는 로그인 화면으로 이동하고, `다시 보지 않기`만 브라우저에 영구 숨김 상태 저장
- 온보딩·보안 회귀검사와 Next.js 운영 빌드 통과
- 로컬 운영 모드에서 실제 포인터 스와이프, 360px 가로 맞춤, 다시 보지 않기 재접속 차단 확인
- GitHub 커밋 `b469ef8`, `581fd1d` 및 Vercel 배포 `dpl_C7aV1X4m3kQVz6Cr89AyAiKgMHJA` 운영 반영 완료

## 2026-09-11 - AI·개발자 인수인계 체계 정비
- 다른 AI가 기존 대화 없이도 작업을 이어갈 수 있도록 루트 README와 현재 상태 기준 문서를 작성
- 로그인·온보딩·대시보드·알림·대상자·설정·쿠폰·광고·관리자·공개 QR 화면의 누적 요구사항과 구현 상태를 화면별 표로 통합
- GitHub 저장소, Vercel 프로젝트·대표 도메인, Turso와 외부 서비스, 로컬 실행, 미리보기 주소, 테스트 및 배포 절차를 한 문서에 정리
- Meta 실광고 권한, SNS 운영 검증, iOS 음성 실제 기기 점검 등 완료 기능과 외부 대기 항목을 명확히 분리
- 이후 모든 작업에서 현재 상태 문서와 기술·발표 로그를 함께 갱신하도록 프로젝트 규칙에 반영

## 2026-09-11 - 실종신고 광고 설정 진입 오류 복구
- 온라인 실종신고에서 대상자를 선택한 뒤 발생하던 서버 오류를 운영 로그로 재현
- 원인은 Meta 권한이 아니라 광고 모달에 전달할 `forceNewAd` 값이 내부 컴포넌트에서 누락된 코드 오류로 확인
- 대상자 선택 후 기존 광고 거리·위치 범위·기간·요약·미리보기·결제 흐름으로 다시 연결
- Meta API는 기존과 동일하게 결제 이후 서버에서만 실행되도록 분리 유지
- 로컬 광고 설정 미리보기와 재발 방지 회귀 테스트 추가
- 거리 선택부터 3일 기간, 선택 요약, 광고 미리보기, 결제 버튼까지 브라우저 검증 완료

## 2026-09-11 - 광고 대시보드 상태별 기능 보강
- 광고 검토 중과 광고 완료 카드에는 버튼이 표시되지 않고, 진행 중 카드에만 `추가/종료`가 표시되도록 확인
- `추가` 선택 시 같은 대상자가 선택된 온라인 실종신고 화면을 거쳐 새 지역·거리·기간 광고 설정으로 연결
- `종료` 선택 시 즉시 종료하지 않고 환불 불가 안내와 최종 확인 버튼이 있는 팝업 표시
- 화면 표시뿐 아니라 서버에서도 진행 중 또는 일시정지 광고만 조기 종료할 수 있도록 제한
- 광고 대시보드·대시보드 UI·대상자 흐름·보안 회귀검사와 Next.js 운영 빌드 통과
- 로컬 미리보기에서 상태별 카드, 필터, 버튼, 종료 팝업 및 브라우저 오류 없음 확인
- 기능 커밋 `0206956` 생성, 이번 요청에는 배포 지시가 없어 GitHub/Vercel 반영은 대기

## 2026-09-11 - 광고 대시보드 상태별 기능 운영 배포
- 기능 커밋 `0206956`과 인수인계 커밋 `7f7faea`를 GitHub `main`에 반영
- Vercel 운영 배포 `dpl_CvC4VyKr31PubgnVN78t4He3ahkK`가 `Ready` 상태이며 대표 도메인 연결 확인
- 운영 광고 대시보드에서 검토 중·완료 광고가 정상 표시되고 두 상태에 버튼이 나타나지 않는 것 확인
- 운영 계정에 진행 중 광고가 없어 실제 종료는 실행하지 않았으며, 진행 중 전용 버튼과 환불 안내 팝업은 동일 빌드의 로컬 미리보기에서 검증

## 2026-09-11 - 광고 결제 완료 화면 개편 및 112 경찰 신고 연계
- 제공된 확정 그래픽 이미지를 광고 결제 완료 화면의 상단 완료 그래픽으로 적용
- 이미지 하단에 '경찰 신고도 함께 진행하시겠어요?' 밑줄 안내 및 [예], [아니오] 버튼 구성
- 문구 또는 [예] 버튼 클릭 시 '112로 전화할까요?' 확인 팝업 모달을 띄우고 [전화하기] 선택 시 112로 즉시 전화 연결 기능 구현
- [아니오] 선택 시 광고 대시보드로 이동하도록 연결
- 관리자 결제패스 테스트 링크 및 광고내역·대시보드 이동 링크 유지
- 광고 결제완료 로컬 미리보기(`?preview=1`) 및 회귀 테스트 스위트 확장
- 광고 대시보드 회귀, 대시보드 UI 회귀, 보안 검사, Next.js 운영 빌드 통과
- GitHub 커밋·푸시 및 Vercel 운영 배포 반영 완료

## 2026-09-11 - 관리자 테스트 결제 안내 노출 제한
- `관리자 테스트 결제가 완료되었습니다. Meta에 발행하지 않고 광고 검토 중 상태로 생성했습니다.` 안내를 관리자 전용으로 제한
- 문구 끝에 `[해당 문구는 관리자만 볼 수 있습니다]` 추가
- 서버에서 관리자 권한을 확인해 일반 사용자에게 해당 메시지를 전달하지 않고, 화면에서도 관리자 조건을 다시 검사
- 관리자 로컬 미리보기에서 정확한 문구 표시와 브라우저 오류 없음 확인
- 광고 대시보드 회귀검사, 보안검사, Next.js 운영 빌드 통과
- 기능 커밋 `0ef3ebb` 생성, 배포 요청은 없어 운영 반영 대기

## 2026-09-11 - 일반 사용자 광고 결제 완료 미리보기
- 일반 사용자 상태를 직접 확인할 수 있는 개발 전용 `preview=user` 주소 추가
- 일반 사용자 화면에서는 관리자 테스트 결제 안내를 제거하고 결제 완료 이미지와 112 신고 안내만 유지
- 관리자 화면은 `preview=admin`, 기존 `preview=1` 주소로 계속 확인 가능
- 브라우저에서 관리자 안내 미노출과 공통 UI 표시 확인
- 광고 대시보드 회귀검사 및 Next.js 운영 빌드 통과

## 2026-09-11 - 결제 및 서비스 현황 개편
- 상품 구매와 온라인 실종광고 결제 내역을 실제 보호자 결제 데이터 기준으로 한 목록에 통합
- 최신 결제부터 카드로 표시하고 결제 금액·상태·일시와 색상 상태표시 적용
- 결제 내역이 많을 때 화면 전체가 아닌 목록 영역만 세로 스크롤되도록 구성
- 상품 결제는 주문 상품·배송·결제·취소 정보를, 광고 결제는 대상자·광고 기간·결제 정보를 별도 상세 화면에 표시
- URL을 임의 변경해도 로그인한 보호자 본인의 결제만 열리도록 서버 소유권 검사 적용
- 제공된 쇼핑백·확성기·영수증 이미지를 원본 자산으로 반영하고 결제 없음 화면도 구성
- 통합 결제 회귀검사, 보안검사, Next.js 운영 빌드 및 로컬 브라우저 검증 통과
- GitHub 기능 커밋 `9d79166` 및 Vercel 배포 `dpl_4DeUFiDBvVYF5vALrLTcpzY6T2zn` 운영 반영 완료

## 2026-09-11 - 대시보드 알림 확인 방식 및 위치 이동 개선
- 알림 팝오버를 여는 동안 미확인 상태를 유지하고 닫기·뒤로가기·화면 이탈 시 노출된 알림만 자동 확인 처리
- 확인 후 알림을 삭제하지 않고 다음 진입부터 회색 아이콘과 흰 배경으로 계속 표시
- 미확인 알림은 초록 아이콘·초록 점·옅은 초록 배경으로 표시되도록 뒤바뀌어 있던 상태별 이미지 자산 수정
- 여러 알림을 한 번에 처리하는 보호자 소유권 기반 서버 API와 화면 이탈용 `keepalive` 저장 적용
- 위치 공유 알림만 검증된 카카오맵 링크로 연결하여 발견자가 전달한 좌표 화면으로 이동
- 알림·푸시·보안 회귀검사, Next.js 운영 빌드, 로컬 닫기 전후 및 카카오맵 이동 검증 통과
- GitHub 기능 커밋 `cbd6044` 및 Vercel 배포 `dpl_J2sFbHX7LfBm7EaVH7xkS14Ujdrv` 운영 반영 완료

## 2026-09-11 - 관리자 서비스 규정 관리 및 회원가입 약관 연동
- 관리자 페이지에 `서비스 규정 관리` 메뉴와 개인정보·서비스이용·알림 3개 탭 추가
- 탭별 문단 내용 수정, 문단 추가·삭제, 굵기와 14~24px 글씨 크기 설정 및 저장 기능 구현
- 저장 내용을 임의 HTML이 아닌 검증된 문단 서식 데이터로 관리하여 사용자 입력 스크립트 노출 차단
- 일반 및 SNS 회원가입의 세 약관 `자세히` 버튼을 최신 관리자 저장 내용 팝업에 연결
- 팝업은 제공 화면처럼 고정 헤더와 본문 내부 스크롤을 사용하고 열 때마다 no-store API로 최신 내용 조회
- SNS 회원가입의 선택 알림 동의 저장까지 함께 연결
- 서비스 규정·회원가입·SNS·보안 회귀검사와 Next.js 운영 빌드, 로컬 브라우저 검증 통과
- 기능 커밋 `957553b`과 인수인계 커밋을 GitHub `main`에 반영
- Vercel 운영 배포 `dpl_E19dHgcypFn11T8TGpeSo8YnA951`가 `READY` 상태이며 대표 도메인과 세 약관 API 응답 확인 완료
