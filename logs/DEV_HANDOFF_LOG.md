...
## 2026-08-25 KST - 회원 확인과 SMS 발송 제한 분리

### 요구내용
- 사용자가 번호를 입력하고 `인증코드 받기`를 누르면 먼저 기존 회원 여부를 판단하고, 필요한 계정 연결 동의 후 인증번호를 발송

### 원인
- SNS 계정 연결은 기존 회원 확인과 확인 후 SMS 발송의 API 요청 두 번으로 구성되어 있었다.
- 두 요청이 모두 동일한 `3회/5분` SMS 제한을 사용해, 사용자가 한 번 재시도하면 실제 문자 발송 전에 429 제한에 도달할 수 있었다.
- 운영 로그에서 `/api/signup/phone/send` 429 두 건과 같은 브라우저/IP 단기 카운터 3회를 확인했다.

### 반영내용
- 회원 확인 제한과 실제 Solapi 문자 발송 제한을 별도 키로 분리했다.
- 회원 확인은 5분 10회·1시간 30회로 제한해 번호 조회 남용을 방지한다.
- 실제 문자 발송은 기존 5분 3회·1시간 10회, 같은 번호·목적 1시간 5회 제한을 유지한다.
- 기존 회원 확인 응답은 SMS 발송 횟수를 소비하지 않고, 계정 연결 동의 후 실제 문자 발송 때만 발송 횟수가 증가한다.
...
## 2026-08-25 KST - 온보딩·QR 상태·활성화 화면 정리

### 요구내용
- 로그인 전 3단계 소개 화면의 이전/다음 버튼을 없애고 가운데 `로그인하기` 버튼만 표시
- 관리자 QR 관리에서 중복되는 활성화 상태를 숨기고 사용중·미사용·스토어 선점 중심으로 표시
- 상품 수령 후 QR 활성화 대상자 카드를 사진 위, 정보 아래의 세로형으로 변경
- 로그인에서 Facebook 제거

### 반영내용
- 소개 화면의 좌우 스와이프와 페이지 점은 유지하고 하단 명령을 `로그인하기` 하나로 단순화했다.
- 관리자 QR 검색에서 활성/미활성 필터를 제거하고 목록의 활성화상태 열, 상세 헤더 활성화 배지, 관리 탭의 복합 상태 문구를 제거했다.
- 사용중 QR 현황 카드의 활성/미활성 세부 수치도 숨기고 스토어 선점 QR은 단일 상태 배지로 우선 표시한다.
- QR 활성화 대상자 카드에 전용 클래스를 적용해 큰 정사각형 사진과 이름·생년월일·확인 문구를 위아래로 배치했다.
- 일반 로그인과 스토어 선점 QR 가입 화면에서 Facebook 버튼을 제거했다. 기존 연결 계정 호환을 위한 서버 공급자와 DB 데이터는 유지했다.
- 산출물 `deliverables/ONBOARDING_QR_ADMIN_ACTIVATION_UI.md`를 추가했다.
...
- `npm run test:social-link`: 통과
- `git diff --check`: 통과
- 로컬 브라우저에서 소개 화면에 `로그인하기`만 표시되고, 로그인 화면에는 Kakao·Naver·Google 버튼만 표시되는 것을 확인했다.
...
## 2026-09-04 KST - 전 화면 초록 디자인 시스템 표준화

### 요구내용
- 앱 전체를 확정된 초록색 중심 팔레트와 하나의 한글 서체로 통일
- 초록색은 주요 버튼·선택·핵심 정보에만 사용하고 배경은 밝은 중립색으로 유지
- 성공·경고·오류 상태, 입력창, 버튼, 카드, 아이콘, 간격과 모바일 화면 기준을 일관되게 적용
- 그림자·그라데이션·과도한 모서리 효과를 줄이고 상태를 색상만으로 구분하지 않도록 유지

### 반영내용
- 기존 서비스에서 사용 중이던 대표색 `#26963F`와 `Noto Sans KR`을 공식 전역 기준으로 유지했다.
- 대표·보조·표면·텍스트·비활성·성공·경고·오류·정보 색상을 `css/gov-style.css`의 단일 토큰 세트로 확정했다.
- 카드와 입력창 모서리를 4px·8px로 정리하고 공통 그림자를 최소화했다.
- 버튼·링크·입력창의 키보드 포커스를 초록 외곽선으로 통일하고 폼 컨트롤이 공통 서체를 상속하도록 했다.
- 로그인, 온보딩, 대시보드, 위치보안, 실종신고, 관리자 사이드바와 표의 남은 파란·보라·검정 강조값을 공통 토큰에 연결했다.
- 온보딩의 장식용 배경 그라데이션을 제거하고 밝은 배경·얇은 초록 테두리 구조로 단순화했다.
- 카카오·네이버 등 외부 로그인 제공자의 공식 브랜드 색상은 식별성을 위해 유지했다.
- 확정 HEX와 후속 화면 제작 기준을 `deliverables/GLOBAL_GREEN_DESIGN_SYSTEM.md`에 기록했다.
...
- CSS 미선언 공통 토큰 검사: 통과
- `git diff --check`: 통과
- 로컬 `390 x 844`, `1440 x 900` 렌더링에서 텍스트 겹침·화면 잘림·Next.js 오류 오버레이 없음 확인
- WCAG 2 A/AA 자동 검사: 위반 0건
- 운영 `https://zezari.family` 모바일 렌더링, 대표색 `#26963F`, 공통 서체와 브라우저 오류 없음 확인

### 소요시간
- CSS 정리·문서화·회귀검증·배포: 약 45분
