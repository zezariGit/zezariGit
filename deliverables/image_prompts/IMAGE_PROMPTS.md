...
Create a Korean technical onboarding flow diagram for REAL_QR_FIND external QR product sales. Start with a customer scanning one unassigned physical QR product. Show a public mobile page with regular signup and Google, Kakao, Naver, and Facebook signup buttons. After a button press, show a Next.js server reserving that exact QR for two hours using an HTTP-only browser token while Turso stores only its SHA-256 hash and expiry time. Continue through Solapi phone verification, guardian information, and mandatory registration of one managed subject. Finish with one atomic conditional update that connects the exact scanned QR to the first new subject and clears the reservation. Add a parallel normal-signup path that selects the oldest unreserved QR and cannot take an active external-sale reservation. Include failure branches for expired, already assigned, inactive, and concurrently reserved QR codes. Use the project's white, black, and neutral-gray service style with provider brand colors only on SNS buttons, thin connectors, compact Korean labels, corners no larger than 8px, no gradients, and no real names, phone numbers, QR keys, cookies, hashes, or credentials.
