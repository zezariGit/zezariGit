...
# 2026-08-02 - 비즈콜 안심번호 공개 페이지 장애 진단

## 설정 검증과 즉시 매칭 흐름

Create a Korean technical handoff diagram for REAL_QR_FIND's Bizcall safe-phone access flow. Start with an active QR or managed-subject link request, then show a server decision that first reuses a still-valid 24-hour lease. For a new lease, show a mandatory configuration gate labeled `BIZCALL_INTERFACE_ID`, followed by selecting an unmapped 050 number, calling the standard HTTPS API host, and atomically marking the number assigned only after provider success. Add a failure branch that leaves the public page at `안심번호 준비중` without exposing the guardian's raw phone and without corrupting an available pool row. Include a compact administrator warning that the Interface ID is issued by Delphicom and differs from the portal login ID. Use white work surfaces, civic blue, restrained purple, thin gray connectors, compact Korean labels, corners no larger than 8px, no gradients, and no real credentials or phone numbers.

# 2026-08-03 - 실종광고 고정 양식 합성 흐름

## 광고 소재 생성 및 Meta 자동발행

Create a Korean technical workflow diagram for REAL_QR_FIND's fixed-template missing-person Meta advertisement. Show the supplied 1080 by 1350 form as an immutable background, then four server-approved data overlays: the managed-subject photo in the left red frame, name age and gender in the right fields, guardian message in the yellow field, and the subject QR in the lower-left blank. Show a browser Canvas producing one deterministic 4:5 JPEG regardless of desktop or mobile viewport, followed by payment completion, stored creative, Meta image upload, campaign/ad-set/creative/ad creation, and a clickable destination to the same managed-subject public page. Include a privacy note that guardian name, raw phone, email, address, and API credentials never enter the advertisement image. Use a clean technical handoff style, white background, civic blue connectors, restrained red/yellow samples only where the supplied form uses them, compact Korean labels, no gradients, and no real personal data or tokens.

# 2026-08-08 - 비즈콜 통화 시점 안심번호 자동 배정

## 공급자 자동 선택 및 확장 구조

Create a Korean technical architecture diagram for REAL_QR_FIND's on-demand Bizcall safe-phone calling flow. Show an active managed-subject QR page that performs no phone allocation on page view. The finder presses a single call button, the Next.js API validates QR and service access, applies rate limiting and a per-guardian lock, then calls Bizcall `/link/auto_expire_mapp.do`. Show Bizcall atomically selecting any currently unassigned 050 number from the contract inventory, followed by coloring and announcement setup and a one-hour automatic expiry confirmation. Return only the temporary 050 number and a `tel:` action to the finder. Show the database recording request and assignment history without storing raw requester IP. Add a capacity note that 100 or more provider numbers automatically participate without local preload or a code limit. Include failure compensation release and an administrator history panel with emergency release. Use the project's white and civic-blue operations style, thin gray connectors, compact Korean labels, corners no larger than 8px, no gradients, and no real phone numbers, credentials, or IDs.

# 2026-08-19 - 외부 판매 미배정 QR 가입·정확 매칭

## QR 예약과 첫 대상자 연결 흐름

Create a Korean technical onboarding flow diagram for REAL_QR_FIND external QR product sales. Start with an administrator selecting an active unassigned QR and pressing `QR선점 - 스토어판매용` in a dense master-detail QR management screen. Show that this persistent store-sale reservation excludes the QR from normal automatic allocation and manual matching. Continue with a customer scanning the selected physical QR product and seeing regular signup plus Google, Kakao, Naver, and Facebook signup buttons. After a button press, show a separate two-hour browser claim using an HTTP-only token while Turso stores only its SHA-256 hash and expiry. Continue through Solapi phone verification, guardian information, and mandatory registration of one managed subject. Finish with one atomic conditional update that verifies both the administrator reservation and browser claim, connects the exact QR, and clears both states. Add failure branches for unselected, expired, assigned, inactive, and concurrently claimed QR codes. Use the project's white, black, and neutral-gray service style with provider brand colors only on SNS buttons, thin connectors, compact Korean labels, corners no larger than 8px, no gradients, and no real names, phone numbers, QR keys, cookies, hashes, or credentials.

# 2026-08-20 - 이미지 업로드 정책 관리

## 관리자 설정과 이중 검증 흐름

Create a Korean technical operations diagram for REAL_QR_FIND image upload management. On the left, show a compact administrator screen named `이미지업로드 관리` with separate numeric MB controls for guardian photos and managed-subject photos, both defaulting to 1MB and capped at 4MB. In the center, show the setting stored in Turso and read by the Next.js dashboard. On the right, show two upload paths: guardian profile photo and managed-subject photo. Each path first checks the selected file size in the browser and displays a Korean alert containing the configured limit, then repeats the same authoritative validation on the server before storing an approved raster image. Include allowed JPEG, PNG, WebP, and GIF labels, authenticated guardian photo delivery, and a note that oversized or mismatched file content is rejected. Use the project's white, black, and neutral-gray operations style, compact labels, thin connectors, corners no larger than 8px, no gradients, and no real photos, personal data, or credentials.
