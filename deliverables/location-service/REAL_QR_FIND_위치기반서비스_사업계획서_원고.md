...
| 설비·서비스 | 현재 구성·용도 | 주요 보호조치 | 설치·운영 장소 |
| --- | --- | --- | --- |
| Vercel Cloud 웹·애플리케이션 논리서버 | 프로젝트 `zezari`, Next.js 16.3.0, Node.js 24.x. 글로벌 CDN, HTTPS, PWA, SSR, 인증·QR·결제·광고·위치공유 API | TLS·HSTS, 배포 접근통제, 서버 전용 환경변수, 플랫폼 경계 보호 | Vercel 관리형 클라우드. 현재 Function `iad1`(Washington, D.C., USA), 국내 요청은 서울 PoP 경유 가능 |
| Turso Cloud DB 논리서버 | libSQL/SQLite 3.47.0, 스키마 37, 업무 테이블 37개, 확인 당시 논리용량 6.68 MB. 회원·QR·업무·위치 암호문·취급대장 저장 | 인증 토큰, 위치 필드 AES-256-GCM, Turso 볼륨 저장암호화, 접근기록 | Turso 관리형 클라우드. 현재 `aws-ap-northeast-1`(Tokyo, Japan) |
| GitHub Cloud 형상·배포이력 저장소 | `zezariGit/zezariGit`, `main`. 소스·변경·배포 기준점과 Vercel 자동 Production 배포 | 계정 인증, 저장소 권한, 커밋 이력, `.env.local` 제외 | GitHub 관리형 클라우드. 원본 위치좌표 DB로 사용하지 않음 |
...
클라우드 설비는 물리 장비를 제자리 사무실에 설치하지 않고 각 서비스 제공자의 데이터센터에서 운영한다. 자체 물리 서버는 없다. 현재 확인된 Vercel Function은 미국 `iad1`, Turso DB는 일본 `aws-ap-northeast-1`에 있어 국외 처리·이전 고지와 리전 정렬 여부를 함께 검토한다. 제출 시 Vercel 프로젝트·도메인·배포 현황, Turso 데이터베이스 리전·백업·암호화 설정, GitHub 저장소 접근권한, Web Push 키 관리 화면을 캡처한다. 계약서, 이용내역, 청구서 또는 서비스 가입을 확인할 수 있는 자료를 함께 준비한다.
