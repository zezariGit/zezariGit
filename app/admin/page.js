  ...
  return (
    <div className="safe-phone-admin-page">
      ...
      <section className="admin-panel safe-phone-policy-panel">
        <div>
          <h2>통화 시점 자동 안심번호 배정</h2>
          <p>QR 공개 페이지를 보는 것만으로는 번호를 점유하지 않습니다. 발견자가 통화 버튼을 누르면 비즈콜이 비어 있는 번호를 선택해 보호자 연락처에 {config.leaseHours}시간 임시 연결합니다.</p>
          <p>번호 재고는 비즈콜 계정에서 관리합니다. 계약 번호가 100개를 초과해 추가되더라도 애플리케이션 수정이나 번호 사전 등록 없이 자동 배정 대상에 포함됩니다.</p>
        ...
        <section className="admin-panel admin-master-panel">
          ...
          <form className="admin-master-filter safe-phone-filter" action="/admin">
            ...
            <label>
              ...
              <select name="safePhoneStatus" defaultValue={filters.status}>
                ...
                <option value="available">자동 해제</option>
                <option value="assigned">통화 연결중</option>
          ...
          <div className="admin-record-table-wrap">
            <div className="admin-record-table safe-phone-record-table" role="table" aria-label="안심번호 풀 목록">
              ...
              {pool.length === 0 && <p className="empty-text">아직 통화 요청으로 자동 배정된 안심번호 이력이 없습니다.</p>}
            </div>
          </div>
        </section>
            ...
            <>
              ...
              <section className="admin-detail-section">
                ...
                <dl className="admin-detail-list">
                  ...
                  <div><dt>배정 방식</dt><dd>{selectedSafePhone.allocation_source === "provider_auto" ? "비즈콜 자동 선택" : "기존 수동 풀 이력"}</dd></div>
                ...
                <div className="admin-detail-history safe-phone-history">
                  {history.map((item) => (
                    <div key={item.id}>
                      ...
                      <span>{item.source === "finder_call" ? "발견자 통화 요청" : item.source === "admin_manual" ? "기존 관리자 수동 매칭" : "기존 QR 접근 매칭"}</span>
...
function safePhonePoolStatusLabel(status) {
  if (status === "available") return "자동 해제";
  if (status === "assigned") return "통화 연결중";
  if (status === "assigning") return "연결 처리중";
...
function safePhoneReleaseReasonLabel(reason, status) {
  if (!reason && status === "assigned") return "현재 통화 연결 유지 중";
  if (reason === "lease_expired") return "임시 연결시간 만료로 자동 해제";
  if (reason === "provider_reassigned") return "비즈콜 자동 재배정 전 이력 종료";
