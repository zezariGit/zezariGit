...
export async function prepareSubscriptionForGuardian(session) {
  await ensureSchema();
  const db = getDb();
  const { guardian } = await getDashboardData(session);
  ...
  if (existing) {
    await db.execute({
      sql: `UPDATE subscriptions
        SET amount = ?, currency = 'KRW', error_code = NULL, error_message = NULL, updated_at = CURRENT_TIMESTAMP
        WHERE guardian_id = ?`,
      args: [amount, guardian.id],
    });
    return {
      guardian,
      subscription: {
  ...
  await db.execute({
    sql: `INSERT INTO subscriptions (id, guardian_id, customer_key, status, amount)
      VALUES (?, ?, ?, ?, ?)`,
    args: [subscription.id, subscription.guardian_id, subscription.customer_key, subscription.status, subscription.amount],
  });
