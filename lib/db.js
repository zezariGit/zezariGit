...
export async function saveGuardianProfile(session, formData) {
  ...
  await db.execute({
    sql: `UPDATE guardians
      SET name = ?, login_id = ?, password_hash = ?, phone = ?, safe_phone = ?, address = ?, email = ?, google_email = ?, updated_at = CURRENT_TIMESTAMP
      WHERE google_id = ?`,
    args: [name, loginId, passwordHash, phone, safePhone, address, email, session.user?.email || "", googleId],
  });
}
