...
export function normalizeMetaAdBudgetSettings(value = {}) {
  return {
    marginPercent: marginPercentage(
      value.marginPercent ?? value.meta_margin_percent,
...
export function calculateMetaBudgetFromPayment({
  paymentAmount,
  days,
  marginPercent = DEFAULT_META_AD_BUDGET.marginPercent,
}) {
  const normalizedPaymentAmount = nonNegativeInteger(paymentAmount, 0);
  const normalizedDays = boundedPositiveInteger(days, 1, 365);
  const normalizedMarginPercent = marginPercentage(
    marginPercent,
    DEFAULT_META_AD_BUDGET.marginPercent,
  );
  const amount = Math.floor(
    normalizedPaymentAmount * (100 - normalizedMarginPercent) / 100,
  );
  const serviceMarginAmount = normalizedPaymentAmount - amount;
  ...
  return {
    ...
    serviceMarginAmount,
    dailyBudget: Math.floor(amount / normalizedDays),
    amount,
    budgetVersion: 3,
  };
}
...
function marginPercentage(value, fallback) {
  const number = Math.floor(Number(value));
  if (!Number.isFinite(number) || number < 0 || number > 90) return fallback;
  return number;
}
