  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.phone = token.authPhone || "";
        session.user.supportMode = Boolean(token.supportMode);
        session.user.supportSessionId = token.supportSessionId || "";
        session.user.adminActorGuardianId = token.adminActorGuardianId || "";
        session.user.adminActorName = token.adminActorName || "";
        session.user.supportGuardianName = token.supportGuardianName || "";
      }
      return session;
    },
  },
