...
import { sanitizeAdGuardianMessage } from "./ad-creative-text.js";
...
const DEFAULT_DESTINATION_URL = "https://zezari.family";
...
function buildPrimaryText(ad) {
  ...
  const message = sanitizeAdGuardianMessage(ad?.subject_guardian_message);
...
async function metaGet(path, query = {}, { timeoutMs = 0 } = {}) {
  const config = getMetaMarketingConfig();
  assertConfigured(config);
  ...
  return metaFetch(`${path}?${params.toString()}`, {
    method: "GET",
...
function parseCreativeImage(dataUrl) {
  ...
  if (!match) {
    throw new Error("Meta 광고용 포스터 이미지가 없습니다. 활성 포스터 템플릿과 대상자 정보를 확인해 주세요.");
  }
...
function buildSchedule(ad) {
  ...
  const earliestStart = new Date(Date.now() + 60 * 1000);
...
export async function createMetaAdvertisementForSubjectAd(ad, { status = "ACTIVE" } = {}) {
  ...
  if (!Number.isFinite(lifetimeBudget) || lifetimeBudget < 1) {
    throw new Error("Meta 집행예산을 확인해 주세요. 보호자 결제금액과 서비스 마진율로 계산된 예산이 필요합니다.");
  }
  ...
  try {
    ...
    const creative = await metaPost(`${config.adAccountId}/adcreatives`, {
      ...
      object_story_spec: {
        ...
        link_data: {
          ...
          name: `${ad?.subject_name || "관리대상"}님 정보 확인하기`,
          description: "모바일에서는 광고를 눌러 관리대상 페이지를 바로 확인할 수 있습니다.",
          call_to_action: {
            type: "LEARN_MORE",
    ...
    try {
      const links = await getMetaAdvertisementLinks(result.adId);
      result.previewUrl = links.previewUrl;
      result.storyId = links.storyId;
    } catch {
      // Preview links are helpful for operators but must not block delivery.
    }
...
export async function updateMetaAdvertisementStatus(ids, status) {
  ...
  let previewUrl = String(ids?.previewUrl || ids?.meta_preview_url || "").trim();
  let storyId = String(ids?.storyId || ids?.meta_story_id || "").trim();
  if (adId && !previewUrl) {
    try {
      const links = await getMetaAdvertisementLinks(adId);
      previewUrl = links.previewUrl;
      storyId = links.storyId;
    } catch {
      // A missing preview link must not block a status change.
    }
  }
...
export async function getMetaAdvertisementLinks(adId) {
  const id = String(adId || "").trim();
  if (!id) return { previewUrl: "", storyId: "" };

  const data = await metaGet(id, {
    fields: "id,preview_shareable_link,creative{id,effective_object_story_id}",
  });
  return {
    previewUrl: String(data?.preview_shareable_link || "").trim(),
    storyId: String(data?.creative?.effective_object_story_id || "").trim(),
  };
}

export async function getMetaAdvertisementDeliveryStatus(adId) {
  const id = String(adId || "").trim();
  if (!id) throw new Error("Meta 광고 ID가 없습니다.");

  const data = await metaGet(id, {
    fields: "id,effective_status,configured_status,status,updated_time",
  }, { timeoutMs: 8000 });
  const effectiveStatus = String(data?.effective_status || data?.status || "").trim().toUpperCase();
  if (!effectiveStatus) {
    throw new Error("Meta 광고의 현재 상태를 확인할 수 없습니다.");
  }
  return {
    adId: String(data?.id || id),
    effectiveStatus,
    configuredStatus: String(data?.configured_status || data?.status || "").trim().toUpperCase(),
    updatedTime: String(data?.updated_time || "").trim(),
    state: classifyMetaAdvertisementDeliveryStatus(effectiveStatus),
  };
}

export function classifyMetaAdvertisementDeliveryStatus(value) {
  const status = String(value || "").trim().toUpperCase();
  if (status === "ACTIVE") return "active";
  if (["PENDING_REVIEW", "IN_PROCESS", "PREAPPROVED"].includes(status)) return "review";
  if (["DISAPPROVED", "WITH_ISSUES", "PENDING_BILLING_INFO"].includes(status)) return "rejected";
  if (["PAUSED", "ADSET_PAUSED", "CAMPAIGN_PAUSED"].includes(status)) return "paused";
  if (["ARCHIVED", "DELETED"].includes(status)) return "ended";
  return "review";
}

export function buildMetaCustomLocationTargeting(ad) {
  if (String(ad?.coverage_type || "").trim() === "country") {
    return {
      age_min: 18,
      geo_locations: {
        countries: ["KR"],
