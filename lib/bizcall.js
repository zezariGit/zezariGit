...
export function getBizcallConfigStatus() {
  ...
  const coloringId = normalizeRingId(process.env.BIZCALL_COLORING_ID);
  const announcementId = normalizeRingId(process.env.BIZCALL_ANNOUNCEMENT_ID);
  const leaseHours = normalizeLeaseHours(process.env.BIZCALL_ON_DEMAND_LEASE_HOURS);
  const missing = [];

  if (!interfaceId) missing.push("BIZCALL_INTERFACE_ID");
  if (!coloringId) missing.push("BIZCALL_COLORING_ID");
  if (!announcementId) missing.push("BIZCALL_ANNOUNCEMENT_ID");
...
export async function assignBizcallOnDemandSafePhone({
  phone,
  guardianId,
  subjectId,
  leaseHours,
  ...
  const config = requireBizcallConfig();
  const realNumber = normalizeRealNumber(phone);
  const normalizedLeaseHours = normalizeLeaseHours(leaseHours || config.leaseHours);
  const assignmentMemo = [guardianId, subjectId].filter(Boolean).join(":").slice(0, 100);
  const result = await requestBizcall("/link/auto_expire_mapp.do", {
    iid: config.interfaceId,
    rn: realNumber,
    expire_hour: normalizedLeaseHours,
    memo: assignmentMemo,
    memo2: "REAL_QR_FIND_CALL",
    auth: createBizcallAuth(config.interfaceId, realNumber),
  }, config);
  const virtualNumber = normalizeVirtualNumber(result.vn);
  ...
  try {
    await requestBizcall("/link/set_vn.do", {
      ...
      vn: virtualNumber,
      rn: realNumber,
      cr_id: config.coloringId,
      if_id: config.announcementId,
      memo: assignmentMemo,
      memo2: "REAL_QR_FIND_CALL",
      switch_yn: "Y",
    ...
    await releaseMappedBizcallNumber(virtualNumber, config, assignmentMemo).catch(() => null);
    throw error;
  }
...
export async function releaseBizcallSafePhone({ safePhone, guardianId }) {
  const config = requireBizcallConfig();
  const virtualNumber = normalizeVirtualNumber(safePhone);
  await releaseMappedBizcallNumber(
    virtualNumber,
    config,
    String(guardianId || "").slice(0, 100),
  );
...
async function releaseMappedBizcallNumber(virtualNumber, config, memo) {
  return requestBizcall("/link/set_vn.do", {
    iid: config.interfaceId,
    vn: virtualNumber,
    rn: " ",
    memo,
...
function normalizeRingId(value) {
  const normalized = String(value || "").trim();
  return /^\d{1,38}$/.test(normalized) ? normalized : "";
}

function normalizeLeaseHours(value) {
  const parsed = Math.floor(Number(value || 1));
  return Math.min(24, Math.max(1, Number.isFinite(parsed) ? parsed : 1));
}
