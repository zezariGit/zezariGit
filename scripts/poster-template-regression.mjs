...
assert.match(editorSource, /poster-template-preview-overlays/);
assert.match(editorSource, /posterPreviewZoneStyle/);
assert.match(editorSource, /POSTER_FIELD_LABELS\[fieldName\]/);
...
assert.match(dbSource, /UPDATE poster_template_versions SET background_image_url = NULL/);
assert.match(dbSource, /backgroundChanged \? null : current\.background_image_url/);
