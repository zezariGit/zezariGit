...
assert.match(checkout, /configurationReady = Boolean\(subjectId && product && selectedDesign && lengthSelectionReady\)/);
...
assert.match(checkout, /SHOP_CONFIGURATION_DRAFT_KEY/);
assert.match(checkout, /window\.sessionStorage\.setItem\(SHOP_CONFIGURATION_DRAFT_KEY/);
assert.match(checkout, /window\.sessionStorage\.getItem\(SHOP_CONFIGURATION_DRAFT_KEY\)/);
assert.match(checkout, /BRACELET_LENGTH_OPTIONS/);
assert.match(checkout, /NECKLACE_LENGTH_OPTIONS/);
assert.match(checkout, /hasKoreanProductType/);
assert.match(checkout, /className="shop-length-options"/);
assert.match(checkout, /braceletLength,/);
assert.match(checkout, /necklaceLength,/);
assert.doesNotMatch(checkout, /\[braceletLength, necklaceLength\]\.filter\(Boolean\)\.join/, "결제 화면 구매 상품 요약에는 선택 길이를 표시하지 않아야 합니다.");
...
assert.match(serviceControls, /getElementById\("shop-service-top"\)\?\.scrollTo\(\{ top: 0, behavior: "smooth" \}\)/);
assert.match(serviceControls, /scrollContainer\.scrollTop >= scrollContainer\.clientHeight/, "위로가기 버튼은 한 화면 높이 이상 스크롤한 뒤 표시해야 합니다.");
assert.match(serviceControls, /scrollContainer\.addEventListener\("scroll", updateVisibility, \{ passive: true \}\)/, "서비스 소개 내부 스크롤 위치를 감지해야 합니다.");
...
assert.match(adminPage, /productPreview = process\.env\.NODE_ENV === "development"/, "상품 관리 로컬 미리보기를 제공해야 합니다.");
assert.match(adminActions, /saveProductServiceIntroAction/);
assert.match(adminActions, /setGlobalProductDesignCatalogItemAction/);
assert.match(adminActions, /createProductCatalogItemAction/);
assert.match(adminActions, /item=\$\{encodeURIComponent\(`product:\$\{productId\}`\)\}/, "신규 상품 저장 후 통합 카탈로그에서 해당 상품을 선택해야 합니다.");
assert.match(productAdmin, /전체/);
assert.match(productAdmin, /상품\/디자인 목록/);
assert.match(productAdmin, /createProductCatalogItemAction/);
assert.match(productAdmin, /상품 추가/);
assert.match(productAdmin, /name="name"[\s\S]*name="unitPrice"[\s\S]*name="image"/, "상품명, 가격, 이미지를 입력하는 신규 상품 폼이 필요합니다.");
...
assert.match(database, /export async function createProductCatalogItem/);
assert.match(database, /bracelet_length TEXT/);
assert.match(database, /necklace_length TEXT/);
assert.match(database, /requiresBraceletLength/);
assert.match(database, /requiresNecklaceLength/);
assert.match(database, /hasKoreanProductType/);
...
assert.match(styles, /\.shop-help-mark\s*\{[^}]*background:\s*#ffffff;/s, "도움말 아이콘 배경은 흰색이어야 합니다.");
assert.match(styles, /\.shop-length-options\s*\{[^}]*grid-template-columns:\s*repeat\(2/s);
assert.match(styles, /\.shop-length-options\s*\{[^}]*width:\s*min\(100%, 320px\)[^}]*margin-inline:\s*auto/s, "길이 선택 옵션 묶음은 가운데 정렬되어야 합니다.");
assert.match(styles, /\.shop-length-options button\s*\{[^}]*place-content:\s*center[^}]*padding:\s*8px/s, "체크 표시와 무관하게 길이 옵션 텍스트를 가운데 정렬해야 합니다.");
assert.match(styles, /\.shop-length-options button\.selected\s*\{[^}]*background:\s*var\(--c-primary-light\)/s);
assert.match(styles, /\.shop-choice-image img\s*\{[^}]*max-width:\s*100%[^}]*max-height:\s*100%[^}]*object-fit:\s*contain/s);
assert.match(styles, /\.shop-catalog-selection-view\.product \.shop-choice-image\s*\{[^}]*aspect-ratio:\s*4 \/ 3/s, "상품 이미지 표시 영역의 비율을 안정적으로 유지해야 합니다.");
assert.match(styles, /\.shop-catalog-selection-view\.product \.shop-choice-image img\s*\{[^}]*position:\s*absolute[^}]*inset:\s*8px[^}]*width:\s*calc\(100% - 16px\)[^}]*height:\s*calc\(100% - 16px\)[^}]*object-fit:\s*contain/s, "상품 이미지는 원본 비율을 유지한 채 전체가 보여야 합니다.");
...
assert.match(styles, /\.shop-service-page\s*\{[^}]*height:\s*100dvh;[^}]*overflow-y:\s*auto;[^}]*overscroll-behavior-y:\s*contain;/s, "모바일 브라우저 기본 위로가기 버튼이 생기지 않도록 소개 화면만 내부 스크롤해야 합니다.");
