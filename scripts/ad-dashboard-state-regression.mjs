...
assert.doesNotMatch(page, /\["running", "paused"\]\.includes\(stage\) && <AdHistoryActions/);
...
assert.match(serverActions, /광고가 중단되었습니다/);
...
assert.match(database, /export async function endSubjectAd[\s\S]{0,220}allowed: \["active"\][\s\S]{0,120}nextStatus: "paused"[\s\S]{0,120}timestampColumn: "paused_at"/);
assert.match(database, /export async function getGuardianAdDashboardData[\s\S]+WHERE a\.guardian_id = \?[\s\S]{0,80}AND a\.paid_at IS NOT NULL/);
assert.match(database, /function adminAdRowsCte[\s\S]+WHERE a\.paid_at IS NOT NULL[\s\S]+\)`;/);
assert.match(database, /FROM subject_ads\s+WHERE paid_at IS NOT NULL/);
assert.match(database, /SELECT DISTINCT region FROM subject_ads WHERE paid_at IS NOT NULL/);
