...
assert.match(database, /export async function syncSubjectAdLifecycle[\s\S]{0,220}const operational = await syncOperationalMetaAds/);
...
assert.match(database, /async function reconcileSubjectStatusAfterAdChange/);
assert.match(database, /status IN \('ready', 'active'\)/);
assert.match(database, /SET status = '안전'/);
assert.match(database, /subjectId: ad\.subject_id,[\s\S]{0,100}nextStatus/);
