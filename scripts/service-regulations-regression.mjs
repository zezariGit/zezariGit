...
const [dbSource, adminPage, adminWorkspace, adminActions, authActions, socialSignup, modal, editor, apiRoute, shared, css, privacyPage, dashboard, myPageOverlay] = await Promise.all([
  ...
  readFile(new URL("../app/my-page-overlay.js", import.meta.url), "utf8"),
]);

assert.match(dbSource, /CREATE TABLE IF NOT EXISTS service_regulations/, "서비스 규정 저장 테이블이 필요합니다.");
assert.match(dbSource, /seedServiceRegulations\(db\)/, "세 규정의 기본값을 초기화해야 합니다.");
assert.match(dbSource, /export async function saveServiceRegulation/, "관리자 저장 함수가 필요합니다.");
assert.match(dbSource, /submittedLength > 30000/, "관리자 입력 길이를 서버에서 제한해야 합니다.");
...
assert.match(adminPage, /ServiceRegulationEditor key=\{activeType\}/, "탭 전환 시 유형별 편집 상태를 새로 불러와야 합니다.");
...
assert.match(dbSource, /marketing_notifications_agreed_at = CASE WHEN \? = 1/, "SNS 알림 동의 결과를 저장해야 합니다.");
...
process.env.NEXTAUTH_SECRET = "service-regulations-regression-secret";
process.env.NODE_ENV = "test";

const { ensureSchema, getServiceRegulations, saveServiceRegulation } = await import(`../lib/db.js?service-regulations=${Date.now()}`);
await ensureSchema();

for (const [type, text] of [["privacy", "개인정보 전용 내용"], ["service", "이용약관 전용 내용"], ["notification", "알림동의 전용 내용"]]) {
  const form = new FormData();
  form.set("regulationType", type);
  form.set("contentJson", JSON.stringify({ blocks: [{ text, bold: false, fontSize: 16 }] }));
  await saveServiceRegulation(form, { user: { id: "regulation-test-admin" } });
}

const savedRegulations = await getServiceRegulations();
assert.equal(savedRegulations.privacy.blocks[0].text, "개인정보 전용 내용");
assert.equal(savedRegulations.service.blocks[0].text, "이용약관 전용 내용");
assert.equal(savedRegulations.notification.blocks[0].text, "알림동의 전용 내용");
assert.equal(new Set(Object.values(savedRegulations).map((document) => document.blocks[0].text)).size, 3, "세 탭의 저장 내용은 서로 달라야 합니다.");
